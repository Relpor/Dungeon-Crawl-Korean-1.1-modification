/*
 *  File:       message.cc
 *  Summary:    Functions used to print messages.
 *  Written by: Linley Henzell
 *
 *  Change History (most recent first):
 *
 *      <3>      5/20/99        BWR             Extended screen lines support
 *      <2>      5/08/99        JDJ             mpr takes a const char* instead of a char array.
 *      <1>      -/--/--        LRH             Created
 */

#include "AppHdr.h"
#include "message.h"
#include "religion.h"

#include <string.h>

#ifdef DOS
#include <conio.h>
#endif

#include "externs.h"

#include "macro.h"
#include "stuff.h"
#include "view.h"

// hantest.cpp : ÄÜ¼Ö ÀÀ¿ë ÇÁ·Î±×·¥¿¡ ´ëÇÑ ÁøÀÔÁ¡À» Á¤ÀÇÇÕ´Ï´Ù.
//

unsigned short hcode[2350][2] = {
	0xb0a1, 0x8861,	/* °¡ */
	0xb0a2, 0x8862,	/* °¢ */
	0xb0a3, 0x8865,	/* °£ */
	0xb0a4, 0x8868,	/* °¤ */
	0xb0a5, 0x8869,	/* °¥ */
	0xb0a6, 0x886a,	/* °¦ */
	0xb0a7, 0x886b,	/* °§ */
	0xb0a8, 0x8871,	/* °¨ */
	0xb0a9, 0x8873,	/* °© */
	0xb0aa, 0x8874,	/* °ª */
	0xb0ab, 0x8875,	/* °« */
	0xb0ac, 0x8876,	/* °¬ */
	0xb0ad, 0x8877,	/* °­ */
	0xb0ae, 0x8878,	/* °® */
	0xb0af, 0x8879,	/* °¯ */
	0xb0b0, 0x887b,	/* °° */
	0xb0b1, 0x887c,	/* °± */
	0xb0b2, 0x887d,	/* °² */
	0xb0b3, 0x8881,	/* °³ */
	0xb0b4, 0x8882,	/* °´ */
	0xb0b5, 0x8885,	/* °µ */
	0xb0b6, 0x8889,	/* °¶ */
	0xb0b7, 0x8891,	/* °· */
	0xb0b8, 0x8893,	/* °¸ */
	0xb0b9, 0x8895,	/* °¹ */
	0xb0ba, 0x8896,	/* °º */
	0xb0bb, 0x8897,	/* °» */
	0xb0bc, 0x88a1,	/* °¼ */
	0xb0bd, 0x88a2,	/* °½ */
	0xb0be, 0x88a5,	/* °¾ */
	0xb0bf, 0x88a9,	/* °¿ */
	0xb0c0, 0x88b5,	/* °À */
	0xb0c1, 0x88b7,	/* °Á */
	0xb0c2, 0x88c1,	/* °Â */
	0xb0c3, 0x88c5,	/* °Ã */
	0xb0c4, 0x88c9,	/* °Ä */
	0xb0c5, 0x88e1,	/* °Å */
	0xb0c6, 0x88e2,	/* °Æ */
	0xb0c7, 0x88e5,	/* °Ç */
	0xb0c8, 0x88e8,	/* °È */
	0xb0c9, 0x88e9,	/* °É */
	0xb0ca, 0x88eb,	/* °Ê */
	0xb0cb, 0x88f1,	/* °Ë */
	0xb0cc, 0x88f3,	/* °Ì */
	0xb0cd, 0x88f5,	/* °Í */
	0xb0ce, 0x88f6,	/* °Î */
	0xb0cf, 0x88f7,	/* °Ï */
	0xb0d0, 0x88f8,	/* °Ð */
	0xb0d1, 0x88fb,	/* °Ñ */
	0xb0d2, 0x88fc,	/* °Ò */
	0xb0d3, 0x88fd,	/* °Ó */
	0xb0d4, 0x8941,	/* °Ô */
	0xb0d5, 0x8945,	/* °Õ */
	0xb0d6, 0x8949,	/* °Ö */
	0xb0d7, 0x8951,	/* °× */
	0xb0d8, 0x8953,	/* °Ø */
	0xb0d9, 0x8955,	/* °Ù */
	0xb0da, 0x8956,	/* °Ú */
	0xb0db, 0x8957,	/* °Û */
	0xb0dc, 0x8961,	/* °Ü */
	0xb0dd, 0x8962,	/* °Ý */
	0xb0de, 0x8963,	/* °Þ */
	0xb0df, 0x8965,	/* °ß */
	0xb0e0, 0x8968,	/* °à */
	0xb0e1, 0x8969,	/* °á */
	0xb0e2, 0x8971,	/* °â */
	0xb0e3, 0x8973,	/* °ã */
	0xb0e4, 0x8975,	/* °ä */
	0xb0e5, 0x8976,	/* °å */
	0xb0e6, 0x8977,	/* °æ */
	0xb0e7, 0x897b,	/* °ç */
	0xb0e8, 0x8981,	/* °è */
	0xb0e9, 0x8985,	/* °é */
	0xb0ea, 0x8989,	/* °ê */
	0xb0eb, 0x8993,	/* °ë */
	0xb0ec, 0x8995,	/* °ì */
	0xb0ed, 0x89a1,	/* °í */
	0xb0ee, 0x89a2,	/* °î */
	0xb0ef, 0x89a5,	/* °ï */
	0xb0f0, 0x89a8,	/* °ð */
	0xb0f1, 0x89a9,	/* °ñ */
	0xb0f2, 0x89ab,	/* °ò */
	0xb0f3, 0x89ad,	/* °ó */
	0xb0f4, 0x89b0,	/* °ô */
	0xb0f5, 0x89b1,	/* °õ */
	0xb0f6, 0x89b3,	/* °ö */
	0xb0f7, 0x89b5,	/* °÷ */
	0xb0f8, 0x89b7,	/* °ø */
	0xb0f9, 0x89b8,	/* °ù */
	0xb0fa, 0x89c1,	/* °ú */
	0xb0fb, 0x89c2,	/* °û */
	0xb0fc, 0x89c5,	/* °ü */
	0xb0fd, 0x89c9,	/* °ý */
	0xb0fe, 0x89cb,	/* °þ */
	0xb1a1, 0x89d1,	/* ±¡ */
	0xb1a2, 0x89d3,	/* ±¢ */
	0xb1a3, 0x89d5,	/* ±£ */
	0xb1a4, 0x89d7,	/* ±¤ */
	0xb1a5, 0x89e1,	/* ±¥ */
	0xb1a6, 0x89e5,	/* ±¦ */
	0xb1a7, 0x89e9,	/* ±§ */
	0xb1a8, 0x89f3,	/* ±¨ */
	0xb1a9, 0x89f6,	/* ±© */
	0xb1aa, 0x89f7,	/* ±ª */
	0xb1ab, 0x8a41,	/* ±« */
	0xb1ac, 0x8a42,	/* ±¬ */
	0xb1ad, 0x8a45,	/* ±­ */
	0xb1ae, 0x8a49,	/* ±® */
	0xb1af, 0x8a51,	/* ±¯ */
	0xb1b0, 0x8a53,	/* ±° */
	0xb1b1, 0x8a55,	/* ±± */
	0xb1b2, 0x8a57,	/* ±² */
	0xb1b3, 0x8a61,	/* ±³ */
	0xb1b4, 0x8a65,	/* ±´ */
	0xb1b5, 0x8a69,	/* ±µ */
	0xb1b6, 0x8a73,	/* ±¶ */
	0xb1b7, 0x8a75,	/* ±· */
	0xb1b8, 0x8a81,	/* ±¸ */
	0xb1b9, 0x8a82,	/* ±¹ */
	0xb1ba, 0x8a85,	/* ±º */
	0xb1bb, 0x8a88,	/* ±» */
	0xb1bc, 0x8a89,	/* ±¼ */
	0xb1bd, 0x8a8a,	/* ±½ */
	0xb1be, 0x8a8b,	/* ±¾ */
	0xb1bf, 0x8a90,	/* ±¿ */
	0xb1c0, 0x8a91,	/* ±À */
	0xb1c1, 0x8a93,	/* ±Á */
	0xb1c2, 0x8a95,	/* ±Â */
	0xb1c3, 0x8a97,	/* ±Ã */
	0xb1c4, 0x8a98,	/* ±Ä */
	0xb1c5, 0x8aa1,	/* ±Å */
	0xb1c6, 0x8aa2,	/* ±Æ */
	0xb1c7, 0x8aa5,	/* ±Ç */
	0xb1c8, 0x8aa9,	/* ±È */
	0xb1c9, 0x8ab6,	/* ±É */
	0xb1ca, 0x8ab7,	/* ±Ê */
	0xb1cb, 0x8ac1,	/* ±Ë */
	0xb1cc, 0x8ad5,	/* ±Ì */
	0xb1cd, 0x8ae1,	/* ±Í */
	0xb1ce, 0x8ae2,	/* ±Î */
	0xb1cf, 0x8ae5,	/* ±Ï */
	0xb1d0, 0x8ae9,	/* ±Ð */
	0xb1d1, 0x8af1,	/* ±Ñ */
	0xb1d2, 0x8af3,	/* ±Ò */
	0xb1d3, 0x8af5,	/* ±Ó */
	0xb1d4, 0x8b41,	/* ±Ô */
	0xb1d5, 0x8b45,	/* ±Õ */
	0xb1d6, 0x8b49,	/* ±Ö */
	0xb1d7, 0x8b61,	/* ±× */
	0xb1d8, 0x8b62,	/* ±Ø */
	0xb1d9, 0x8b65,	/* ±Ù */
	0xb1da, 0x8b68,	/* ±Ú */
	0xb1db, 0x8b69,	/* ±Û */
	0xb1dc, 0x8b6a,	/* ±Ü */
	0xb1dd, 0x8b71,	/* ±Ý */
	0xb1de, 0x8b73,	/* ±Þ */
	0xb1df, 0x8b75,	/* ±ß */
	0xb1e0, 0x8b77,	/* ±à */
	0xb1e1, 0x8b81,	/* ±á */
	0xb1e2, 0x8ba1,	/* ±â */
	0xb1e3, 0x8ba2,	/* ±ã */
	0xb1e4, 0x8ba5,	/* ±ä */
	0xb1e5, 0x8ba8,	/* ±å */
	0xb1e6, 0x8ba9,	/* ±æ */
	0xb1e7, 0x8bab,	/* ±ç */
	0xb1e8, 0x8bb1,	/* ±è */
	0xb1e9, 0x8bb3,	/* ±é */
	0xb1ea, 0x8bb5,	/* ±ê */
	0xb1eb, 0x8bb7,	/* ±ë */
	0xb1ec, 0x8bb8,	/* ±ì */
	0xb1ed, 0x8bbc,	/* ±í */
	0xb1ee, 0x8c61,	/* ±î */
	0xb1ef, 0x8c62,	/* ±ï */
	0xb1f0, 0x8c63,	/* ±ð */
	0xb1f1, 0x8c65,	/* ±ñ */
	0xb1f2, 0x8c69,	/* ±ò */
	0xb1f3, 0x8c6b,	/* ±ó */
	0xb1f4, 0x8c71,	/* ±ô */
	0xb1f5, 0x8c73,	/* ±õ */
	0xb1f6, 0x8c75,	/* ±ö */
	0xb1f7, 0x8c76,	/* ±÷ */
	0xb1f8, 0x8c77,	/* ±ø */
	0xb1f9, 0x8c7b,	/* ±ù */
	0xb1fa, 0x8c81,	/* ±ú */
	0xb1fb, 0x8c82,	/* ±û */
	0xb1fc, 0x8c85,	/* ±ü */
	0xb1fd, 0x8c89,	/* ±ý */
	0xb1fe, 0x8c91,	/* ±þ */
	0xb2a1, 0x8c93,	/* ²¡ */
	0xb2a2, 0x8c95,	/* ²¢ */
	0xb2a3, 0x8c96,	/* ²£ */
	0xb2a4, 0x8c97,	/* ²¤ */
	0xb2a5, 0x8ca1,	/* ²¥ */
	0xb2a6, 0x8ca2,	/* ²¦ */
	0xb2a7, 0x8ca9,	/* ²§ */
	0xb2a8, 0x8ce1,	/* ²¨ */
	0xb2a9, 0x8ce2,	/* ²© */
	0xb2aa, 0x8ce3,	/* ²ª */
	0xb2ab, 0x8ce5,	/* ²« */
	0xb2ac, 0x8ce9,	/* ²¬ */
	0xb2ad, 0x8cf1,	/* ²­ */
	0xb2ae, 0x8cf3,	/* ²® */
	0xb2af, 0x8cf5,	/* ²¯ */
	0xb2b0, 0x8cf6,	/* ²° */
	0xb2b1, 0x8cf7,	/* ²± */
	0xb2b2, 0x8d41,	/* ²² */
	0xb2b3, 0x8d42,	/* ²³ */
	0xb2b4, 0x8d45,	/* ²´ */
	0xb2b5, 0x8d51,	/* ²µ */
	0xb2b6, 0x8d55,	/* ²¶ */
	0xb2b7, 0x8d57,	/* ²· */
	0xb2b8, 0x8d61,	/* ²¸ */
	0xb2b9, 0x8d65,	/* ²¹ */
	0xb2ba, 0x8d69,	/* ²º */
	0xb2bb, 0x8d75,	/* ²» */
	0xb2bc, 0x8d76,	/* ²¼ */
	0xb2bd, 0x8d7b,	/* ²½ */
	0xb2be, 0x8d81,	/* ²¾ */
	0xb2bf, 0x8da1,	/* ²¿ */
	0xb2c0, 0x8da2,	/* ²À */
	0xb2c1, 0x8da5,	/* ²Á */
	0xb2c2, 0x8da7,	/* ²Â */
	0xb2c3, 0x8da9,	/* ²Ã */
	0xb2c4, 0x8db1,	/* ²Ä */
	0xb2c5, 0x8db3,	/* ²Å */
	0xb2c6, 0x8db5,	/* ²Æ */
	0xb2c7, 0x8db7,	/* ²Ç */
	0xb2c8, 0x8db8,	/* ²È */
	0xb2c9, 0x8db9,	/* ²É */
	0xb2ca, 0x8dc1,	/* ²Ê */
	0xb2cb, 0x8dc2,	/* ²Ë */
	0xb2cc, 0x8dc9,	/* ²Ì */
	0xb2cd, 0x8dd6,	/* ²Í */
	0xb2ce, 0x8dd7,	/* ²Î */
	0xb2cf, 0x8de1,	/* ²Ï */
	0xb2d0, 0x8de2,	/* ²Ð */
	0xb2d1, 0x8df7,	/* ²Ñ */
	0xb2d2, 0x8e41,	/* ²Ò */
	0xb2d3, 0x8e45,	/* ²Ó */
	0xb2d4, 0x8e49,	/* ²Ô */
	0xb2d5, 0x8e51,	/* ²Õ */
	0xb2d6, 0x8e53,	/* ²Ö */
	0xb2d7, 0x8e57,	/* ²× */
	0xb2d8, 0x8e61,	/* ²Ø */
	0xb2d9, 0x8e81,	/* ²Ù */
	0xb2da, 0x8e82,	/* ²Ú */
	0xb2db, 0x8e85,	/* ²Û */
	0xb2dc, 0x8e89,	/* ²Ü */
	0xb2dd, 0x8e90,	/* ²Ý */
	0xb2de, 0x8e91,	/* ²Þ */
	0xb2df, 0x8e93,	/* ²ß */
	0xb2e0, 0x8e95,	/* ²à */
	0xb2e1, 0x8e97,	/* ²á */
	0xb2e2, 0x8e98,	/* ²â */
	0xb2e3, 0x8ea1,	/* ²ã */
	0xb2e4, 0x8ea9,	/* ²ä */
	0xb2e5, 0x8eb6,	/* ²å */
	0xb2e6, 0x8eb7,	/* ²æ */
	0xb2e7, 0x8ec1,	/* ²ç */
	0xb2e8, 0x8ec2,	/* ²è */
	0xb2e9, 0x8ec5,	/* ²é */
	0xb2ea, 0x8ec9,	/* ²ê */
	0xb2eb, 0x8ed1,	/* ²ë */
	0xb2ec, 0x8ed3,	/* ²ì */
	0xb2ed, 0x8ed6,	/* ²í */
	0xb2ee, 0x8ee1,	/* ²î */
	0xb2ef, 0x8ee5,	/* ²ï */
	0xb2f0, 0x8ee9,	/* ²ð */
	0xb2f1, 0x8ef1,	/* ²ñ */
	0xb2f2, 0x8ef3,	/* ²ò */
	0xb2f3, 0x8f41,	/* ²ó */
	0xb2f4, 0x8f61,	/* ²ô */
	0xb2f5, 0x8f62,	/* ²õ */
	0xb2f6, 0x8f65,	/* ²ö */
	0xb2f7, 0x8f67,	/* ²÷ */
	0xb2f8, 0x8f69,	/* ²ø */
	0xb2f9, 0x8f6b,	/* ²ù */
	0xb2fa, 0x8f70,	/* ²ú */
	0xb2fb, 0x8f71,	/* ²û */
	0xb2fc, 0x8f73,	/* ²ü */
	0xb2fd, 0x8f75,	/* ²ý */
	0xb2fe, 0x8f77,	/* ²þ */
	0xb3a1, 0x8f7b,	/* ³¡ */
	0xb3a2, 0x8fa1,	/* ³¢ */
	0xb3a3, 0x8fa2,	/* ³£ */
	0xb3a4, 0x8fa5,	/* ³¤ */
	0xb3a5, 0x8fa9,	/* ³¥ */
	0xb3a6, 0x8fb1,	/* ³¦ */
	0xb3a7, 0x8fb3,	/* ³§ */
	0xb3a8, 0x8fb5,	/* ³¨ */
	0xb3a9, 0x8fb7,	/* ³© */
	0xb3aa, 0x9061,	/* ³ª */
	0xb3ab, 0x9062,	/* ³« */
	0xb3ac, 0x9063,	/* ³¬ */
	0xb3ad, 0x9065,	/* ³­ */
	0xb3ae, 0x9068,	/* ³® */
	0xb3af, 0x9069,	/* ³¯ */
	0xb3b0, 0x906a,	/* ³° */
	0xb3b1, 0x906b,	/* ³± */
	0xb3b2, 0x9071,	/* ³² */
	0xb3b3, 0x9073,	/* ³³ */
	0xb3b4, 0x9075,	/* ³´ */
	0xb3b5, 0x9076,	/* ³µ */
	0xb3b6, 0x9077,	/* ³¶ */
	0xb3b7, 0x9078,	/* ³· */
	0xb3b8, 0x9079,	/* ³¸ */
	0xb3b9, 0x907b,	/* ³¹ */
	0xb3ba, 0x907d,	/* ³º */
	0xb3bb, 0x9081,	/* ³» */
	0xb3bc, 0x9082,	/* ³¼ */
	0xb3bd, 0x9085,	/* ³½ */
	0xb3be, 0x9089,	/* ³¾ */
	0xb3bf, 0x9091,	/* ³¿ */
	0xb3c0, 0x9093,	/* ³À */
	0xb3c1, 0x9095,	/* ³Á */
	0xb3c2, 0x9096,	/* ³Â */
	0xb3c3, 0x9097,	/* ³Ã */
	0xb3c4, 0x90a1,	/* ³Ä */
	0xb3c5, 0x90a2,	/* ³Å */
	0xb3c6, 0x90a5,	/* ³Æ */
	0xb3c7, 0x90a9,	/* ³Ç */
	0xb3c8, 0x90b1,	/* ³È */
	0xb3c9, 0x90b7,	/* ³É */
	0xb3ca, 0x90e1,	/* ³Ê */
	0xb3cb, 0x90e2,	/* ³Ë */
	0xb3cc, 0x90e4,	/* ³Ì */
	0xb3cd, 0x90e5,	/* ³Í */
	0xb3ce, 0x90e9,	/* ³Î */
	0xb3cf, 0x90eb,	/* ³Ï */
	0xb3d0, 0x90ec,	/* ³Ð */
	0xb3d1, 0x90f1,	/* ³Ñ */
	0xb3d2, 0x90f3,	/* ³Ò */
	0xb3d3, 0x90f5,	/* ³Ó */
	0xb3d4, 0x90f6,	/* ³Ô */
	0xb3d5, 0x90f7,	/* ³Õ */
	0xb3d6, 0x90fd,	/* ³Ö */
	0xb3d7, 0x9141,	/* ³× */
	0xb3d8, 0x9142,	/* ³Ø */
	0xb3d9, 0x9145,	/* ³Ù */
	0xb3da, 0x9149,	/* ³Ú */
	0xb3db, 0x9151,	/* ³Û */
	0xb3dc, 0x9153,	/* ³Ü */
	0xb3dd, 0x9155,	/* ³Ý */
	0xb3de, 0x9156,	/* ³Þ */
	0xb3df, 0x9157,	/* ³ß */
	0xb3e0, 0x9161,	/* ³à */
	0xb3e1, 0x9162,	/* ³á */
	0xb3e2, 0x9165,	/* ³â */
	0xb3e3, 0x9169,	/* ³ã */
	0xb3e4, 0x9171,	/* ³ä */
	0xb3e5, 0x9173,	/* ³å */
	0xb3e6, 0x9176,	/* ³æ */
	0xb3e7, 0x9177,	/* ³ç */
	0xb3e8, 0x917a,	/* ³è */
	0xb3e9, 0x9181,	/* ³é */
	0xb3ea, 0x9185,	/* ³ê */
	0xb3eb, 0x91a1,	/* ³ë */
	0xb3ec, 0x91a2,	/* ³ì */
	0xb3ed, 0x91a5,	/* ³í */
	0xb3ee, 0x91a9,	/* ³î */
	0xb3ef, 0x91ab,	/* ³ï */
	0xb3f0, 0x91b1,	/* ³ð */
	0xb3f1, 0x91b3,	/* ³ñ */
	0xb3f2, 0x91b5,	/* ³ò */
	0xb3f3, 0x91b7,	/* ³ó */
	0xb3f4, 0x91bc,	/* ³ô */
	0xb3f5, 0x91bd,	/* ³õ */
	0xb3f6, 0x91c1,	/* ³ö */
	0xb3f7, 0x91c5,	/* ³÷ */
	0xb3f8, 0x91c9,	/* ³ø */
	0xb3f9, 0x91d6,	/* ³ù */
	0xb3fa, 0x9241,	/* ³ú */
	0xb3fb, 0x9245,	/* ³û */
	0xb3fc, 0x9249,	/* ³ü */
	0xb3fd, 0x9251,	/* ³ý */
	0xb3fe, 0x9253,	/* ³þ */
	0xb4a1, 0x9255,	/* ´¡ */
	0xb4a2, 0x9261,	/* ´¢ */
	0xb4a3, 0x9262,	/* ´£ */
	0xb4a4, 0x9265,	/* ´¤ */
	0xb4a5, 0x9269,	/* ´¥ */
	0xb4a6, 0x9273,	/* ´¦ */
	0xb4a7, 0x9275,	/* ´§ */
	0xb4a8, 0x9277,	/* ´¨ */
	0xb4a9, 0x9281,	/* ´© */
	0xb4aa, 0x9282,	/* ´ª */
	0xb4ab, 0x9285,	/* ´« */
	0xb4ac, 0x9288,	/* ´¬ */
	0xb4ad, 0x9289,	/* ´­ */
	0xb4ae, 0x9291,	/* ´® */
	0xb4af, 0x9293,	/* ´¯ */
	0xb4b0, 0x9295,	/* ´° */
	0xb4b1, 0x9297,	/* ´± */
	0xb4b2, 0x92a1,	/* ´² */
	0xb4b3, 0x92b6,	/* ´³ */
	0xb4b4, 0x92c1,	/* ´´ */
	0xb4b5, 0x92e1,	/* ´µ */
	0xb4b6, 0x92e5,	/* ´¶ */
	0xb4b7, 0x92e9,	/* ´· */
	0xb4b8, 0x92f1,	/* ´¸ */
	0xb4b9, 0x92f3,	/* ´¹ */
	0xb4ba, 0x9341,	/* ´º */
	0xb4bb, 0x9342,	/* ´» */
	0xb4bc, 0x9349,	/* ´¼ */
	0xb4bd, 0x9351,	/* ´½ */
	0xb4be, 0x9353,	/* ´¾ */
	0xb4bf, 0x9357,	/* ´¿ */
	0xb4c0, 0x9361,	/* ´À */
	0xb4c1, 0x9362,	/* ´Á */
	0xb4c2, 0x9365,	/* ´Â */
	0xb4c3, 0x9369,	/* ´Ã */
	0xb4c4, 0x936a,	/* ´Ä */
	0xb4c5, 0x936b,	/* ´Å */
	0xb4c6, 0x9371,	/* ´Æ */
	0xb4c7, 0x9373,	/* ´Ç */
	0xb4c8, 0x9375,	/* ´È */
	0xb4c9, 0x9377,	/* ´É */
	0xb4ca, 0x9378,	/* ´Ê */
	0xb4cb, 0x937c,	/* ´Ë */
	0xb4cc, 0x9381,	/* ´Ì */
	0xb4cd, 0x9385,	/* ´Í */
	0xb4ce, 0x9389,	/* ´Î */
	0xb4cf, 0x93a1,	/* ´Ï */
	0xb4d0, 0x93a2,	/* ´Ð */
	0xb4d1, 0x93a5,	/* ´Ñ */
	0xb4d2, 0x93a9,	/* ´Ò */
	0xb4d3, 0x93ab,	/* ´Ó */
	0xb4d4, 0x93b1,	/* ´Ô */
	0xb4d5, 0x93b3,	/* ´Õ */
	0xb4d6, 0x93b5,	/* ´Ö */
	0xb4d7, 0x93b7,	/* ´× */
	0xb4d8, 0x93bc,	/* ´Ø */
	0xb4d9, 0x9461,	/* ´Ù */
	0xb4da, 0x9462,	/* ´Ú */
	0xb4db, 0x9463,	/* ´Û */
	0xb4dc, 0x9465,	/* ´Ü */
	0xb4dd, 0x9468,	/* ´Ý */
	0xb4de, 0x9469,	/* ´Þ */
	0xb4df, 0x946a,	/* ´ß */
	0xb4e0, 0x946b,	/* ´à */
	0xb4e1, 0x946c,	/* ´á */
	0xb4e2, 0x9470,	/* ´â */
	0xb4e3, 0x9471,	/* ´ã */
	0xb4e4, 0x9473,	/* ´ä */
	0xb4e5, 0x9475,	/* ´å */
	0xb4e6, 0x9476,	/* ´æ */
	0xb4e7, 0x9477,	/* ´ç */
	0xb4e8, 0x9478,	/* ´è */
	0xb4e9, 0x9479,	/* ´é */
	0xb4ea, 0x947d,	/* ´ê */
	0xb4eb, 0x9481,	/* ´ë */
	0xb4ec, 0x9482,	/* ´ì */
	0xb4ed, 0x9485,	/* ´í */
	0xb4ee, 0x9489,	/* ´î */
	0xb4ef, 0x9491,	/* ´ï */
	0xb4f0, 0x9493,	/* ´ð */
	0xb4f1, 0x9495,	/* ´ñ */
	0xb4f2, 0x9496,	/* ´ò */
	0xb4f3, 0x9497,	/* ´ó */
	0xb4f4, 0x94a1,	/* ´ô */
	0xb4f5, 0x94e1,	/* ´õ */
	0xb4f6, 0x94e2,	/* ´ö */
	0xb4f7, 0x94e3,	/* ´÷ */
	0xb4f8, 0x94e5,	/* ´ø */
	0xb4f9, 0x94e8,	/* ´ù */
	0xb4fa, 0x94e9,	/* ´ú */
	0xb4fb, 0x94eb,	/* ´û */
	0xb4fc, 0x94ec,	/* ´ü */
	0xb4fd, 0x94f1,	/* ´ý */
	0xb4fe, 0x94f3,	/* ´þ */
	0xb5a1, 0x94f5,	/* µ¡ */
	0xb5a2, 0x94f7,	/* µ¢ */
	0xb5a3, 0x94f9,	/* µ£ */
	0xb5a4, 0x94fc,	/* µ¤ */
	0xb5a5, 0x9541,	/* µ¥ */
	0xb5a6, 0x9542,	/* µ¦ */
	0xb5a7, 0x9545,	/* µ§ */
	0xb5a8, 0x9549,	/* µ¨ */
	0xb5a9, 0x9551,	/* µ© */
	0xb5aa, 0x9553,	/* µª */
	0xb5ab, 0x9555,	/* µ« */
	0xb5ac, 0x9556,	/* µ¬ */
	0xb5ad, 0x9557,	/* µ­ */
	0xb5ae, 0x9561,	/* µ® */
	0xb5af, 0x9565,	/* µ¯ */
	0xb5b0, 0x9569,	/* µ° */
	0xb5b1, 0x9576,	/* µ± */
	0xb5b2, 0x9577,	/* µ² */
	0xb5b3, 0x9581,	/* µ³ */
	0xb5b4, 0x9585,	/* µ´ */
	0xb5b5, 0x95a1,	/* µµ */
	0xb5b6, 0x95a2,	/* µ¶ */
	0xb5b7, 0x95a5,	/* µ· */
	0xb5b8, 0x95a8,	/* µ¸ */
	0xb5b9, 0x95a9,	/* µ¹ */
	0xb5ba, 0x95ab,	/* µº */
	0xb5bb, 0x95ad,	/* µ» */
	0xb5bc, 0x95b1,	/* µ¼ */
	0xb5bd, 0x95b3,	/* µ½ */
	0xb5be, 0x95b5,	/* µ¾ */
	0xb5bf, 0x95b7,	/* µ¿ */
	0xb5c0, 0x95b9,	/* µÀ */
	0xb5c1, 0x95bb,	/* µÁ */
	0xb5c2, 0x95c1,	/* µÂ */
	0xb5c3, 0x95c5,	/* µÃ */
	0xb5c4, 0x95c9,	/* µÄ */
	0xb5c5, 0x95e1,	/* µÅ */
	0xb5c6, 0x95f6,	/* µÆ */
	0xb5c7, 0x9641,	/* µÇ */
	0xb5c8, 0x9645,	/* µÈ */
	0xb5c9, 0x9649,	/* µÉ */
	0xb5ca, 0x9651,	/* µÊ */
	0xb5cb, 0x9653,	/* µË */
	0xb5cc, 0x9655,	/* µÌ */
	0xb5cd, 0x9661,	/* µÍ */
	0xb5ce, 0x9681,	/* µÎ */
	0xb5cf, 0x9682,	/* µÏ */
	0xb5d0, 0x9685,	/* µÐ */
	0xb5d1, 0x9689,	/* µÑ */
	0xb5d2, 0x9691,	/* µÒ */
	0xb5d3, 0x9693,	/* µÓ */
	0xb5d4, 0x9695,	/* µÔ */
	0xb5d5, 0x9697,	/* µÕ */
	0xb5d6, 0x96a1,	/* µÖ */
	0xb5d7, 0x96b6,	/* µ× */
	0xb5d8, 0x96c1,	/* µØ */
	0xb5d9, 0x96d7,	/* µÙ */
	0xb5da, 0x96e1,	/* µÚ */
	0xb5db, 0x96e5,	/* µÛ */
	0xb5dc, 0x96e9,	/* µÜ */
	0xb5dd, 0x96f3,	/* µÝ */
	0xb5de, 0x96f5,	/* µÞ */
	0xb5df, 0x96f7,	/* µß */
	0xb5e0, 0x9741,	/* µà */
	0xb5e1, 0x9745,	/* µá */
	0xb5e2, 0x9749,	/* µâ */
	0xb5e3, 0x9751,	/* µã */
	0xb5e4, 0x9757,	/* µä */
	0xb5e5, 0x9761,	/* µå */
	0xb5e6, 0x9762,	/* µæ */
	0xb5e7, 0x9765,	/* µç */
	0xb5e8, 0x9768,	/* µè */
	0xb5e9, 0x9769,	/* µé */
	0xb5ea, 0x976b,	/* µê */
	0xb5eb, 0x9771,	/* µë */
	0xb5ec, 0x9773,	/* µì */
	0xb5ed, 0x9775,	/* µí */
	0xb5ee, 0x9777,	/* µî */
	0xb5ef, 0x9781,	/* µï */
	0xb5f0, 0x97a1,	/* µð */
	0xb5f1, 0x97a2,	/* µñ */
	0xb5f2, 0x97a5,	/* µò */
	0xb5f3, 0x97a8,	/* µó */
	0xb5f4, 0x97a9,	/* µô */
	0xb5f5, 0x97b1,	/* µõ */
	0xb5f6, 0x97b3,	/* µö */
	0xb5f7, 0x97b5,	/* µ÷ */
	0xb5f8, 0x97b6,	/* µø */
	0xb5f9, 0x97b7,	/* µù */
	0xb5fa, 0x97b8,	/* µú */
	0xb5fb, 0x9861,	/* µû */
	0xb5fc, 0x9862,	/* µü */
	0xb5fd, 0x9865,	/* µý */
	0xb5fe, 0x9869,	/* µþ */
	0xb6a1, 0x9871,	/* ¶¡ */
	0xb6a2, 0x9873,	/* ¶¢ */
	0xb6a3, 0x9875,	/* ¶£ */
	0xb6a4, 0x9876,	/* ¶¤ */
	0xb6a5, 0x9877,	/* ¶¥ */
	0xb6a6, 0x987d,	/* ¶¦ */
	0xb6a7, 0x9881,	/* ¶§ */
	0xb6a8, 0x9882,	/* ¶¨ */
	0xb6a9, 0x9885,	/* ¶© */
	0xb6aa, 0x9889,	/* ¶ª */
	0xb6ab, 0x9891,	/* ¶« */
	0xb6ac, 0x9893,	/* ¶¬ */
	0xb6ad, 0x9895,	/* ¶­ */
	0xb6ae, 0x9896,	/* ¶® */
	0xb6af, 0x9897,	/* ¶¯ */
	0xb6b0, 0x98e1,	/* ¶° */
	0xb6b1, 0x98e2,	/* ¶± */
	0xb6b2, 0x98e5,	/* ¶² */
	0xb6b3, 0x98e9,	/* ¶³ */
	0xb6b4, 0x98eb,	/* ¶´ */
	0xb6b5, 0x98ec,	/* ¶µ */
	0xb6b6, 0x98f1,	/* ¶¶ */
	0xb6b7, 0x98f3,	/* ¶· */
	0xb6b8, 0x98f5,	/* ¶¸ */
	0xb6b9, 0x98f6,	/* ¶¹ */
	0xb6ba, 0x98f7,	/* ¶º */
	0xb6bb, 0x98fd,	/* ¶» */
	0xb6bc, 0x9941,	/* ¶¼ */
	0xb6bd, 0x9942,	/* ¶½ */
	0xb6be, 0x9945,	/* ¶¾ */
	0xb6bf, 0x9949,	/* ¶¿ */
	0xb6c0, 0x9951,	/* ¶À */
	0xb6c1, 0x9953,	/* ¶Á */
	0xb6c2, 0x9955,	/* ¶Â */
	0xb6c3, 0x9956,	/* ¶Ã */
	0xb6c4, 0x9957,	/* ¶Ä */
	0xb6c5, 0x9961,	/* ¶Å */
	0xb6c6, 0x9976,	/* ¶Æ */
	0xb6c7, 0x99a1,	/* ¶Ç */
	0xb6c8, 0x99a2,	/* ¶È */
	0xb6c9, 0x99a5,	/* ¶É */
	0xb6ca, 0x99a9,	/* ¶Ê */
	0xb6cb, 0x99b7,	/* ¶Ë */
	0xb6cc, 0x99c1,	/* ¶Ì */
	0xb6cd, 0x99c9,	/* ¶Í */
	0xb6ce, 0x99e1,	/* ¶Î */
	0xb6cf, 0x9a41,	/* ¶Ï */
	0xb6d0, 0x9a45,	/* ¶Ð */
	0xb6d1, 0x9a81,	/* ¶Ñ */
	0xb6d2, 0x9a82,	/* ¶Ò */
	0xb6d3, 0x9a85,	/* ¶Ó */
	0xb6d4, 0x9a89,	/* ¶Ô */
	0xb6d5, 0x9a90,	/* ¶Õ */
	0xb6d6, 0x9a91,	/* ¶Ö */
	0xb6d7, 0x9a97,	/* ¶× */
	0xb6d8, 0x9ac1,	/* ¶Ø */
	0xb6d9, 0x9ae1,	/* ¶Ù */
	0xb6da, 0x9ae5,	/* ¶Ú */
	0xb6db, 0x9ae9,	/* ¶Û */
	0xb6dc, 0x9af1,	/* ¶Ü */
	0xb6dd, 0x9af3,	/* ¶Ý */
	0xb6de, 0x9af7,	/* ¶Þ */
	0xb6df, 0x9b61,	/* ¶ß */
	0xb6e0, 0x9b62,	/* ¶à */
	0xb6e1, 0x9b65,	/* ¶á */
	0xb6e2, 0x9b68,	/* ¶â */
	0xb6e3, 0x9b69,	/* ¶ã */
	0xb6e4, 0x9b71,	/* ¶ä */
	0xb6e5, 0x9b73,	/* ¶å */
	0xb6e6, 0x9b75,	/* ¶æ */
	0xb6e7, 0x9b81,	/* ¶ç */
	0xb6e8, 0x9b85,	/* ¶è */
	0xb6e9, 0x9b89,	/* ¶é */
	0xb6ea, 0x9b91,	/* ¶ê */
	0xb6eb, 0x9b93,	/* ¶ë */
	0xb6ec, 0x9ba1,	/* ¶ì */
	0xb6ed, 0x9ba5,	/* ¶í */
	0xb6ee, 0x9ba9,	/* ¶î */
	0xb6ef, 0x9bb1,	/* ¶ï */
	0xb6f0, 0x9bb3,	/* ¶ð */
	0xb6f1, 0x9bb5,	/* ¶ñ */
	0xb6f2, 0x9bb7,	/* ¶ò */
	0xb6f3, 0x9c61,	/* ¶ó */
	0xb6f4, 0x9c62,	/* ¶ô */
	0xb6f5, 0x9c65,	/* ¶õ */
	0xb6f6, 0x9c69,	/* ¶ö */
	0xb6f7, 0x9c71,	/* ¶÷ */
	0xb6f8, 0x9c73,	/* ¶ø */
	0xb6f9, 0x9c75,	/* ¶ù */
	0xb6fa, 0x9c76,	/* ¶ú */
	0xb6fb, 0x9c77,	/* ¶û */
	0xb6fc, 0x9c78,	/* ¶ü */
	0xb6fd, 0x9c7c,	/* ¶ý */
	0xb6fe, 0x9c7d,	/* ¶þ */
	0xb7a1, 0x9c81,	/* ·¡ */
	0xb7a2, 0x9c82,	/* ·¢ */
	0xb7a3, 0x9c85,	/* ·£ */
	0xb7a4, 0x9c89,	/* ·¤ */
	0xb7a5, 0x9c91,	/* ·¥ */
	0xb7a6, 0x9c93,	/* ·¦ */
	0xb7a7, 0x9c95,	/* ·§ */
	0xb7a8, 0x9c96,	/* ·¨ */
	0xb7a9, 0x9c97,	/* ·© */
	0xb7aa, 0x9ca1,	/* ·ª */
	0xb7ab, 0x9ca2,	/* ·« */
	0xb7ac, 0x9ca5,	/* ·¬ */
	0xb7ad, 0x9cb5,	/* ·­ */
	0xb7ae, 0x9cb7,	/* ·® */
	0xb7af, 0x9ce1,	/* ·¯ */
	0xb7b0, 0x9ce2,	/* ·° */
	0xb7b1, 0x9ce5,	/* ·± */
	0xb7b2, 0x9ce9,	/* ·² */
	0xb7b3, 0x9cf1,	/* ·³ */
	0xb7b4, 0x9cf3,	/* ·´ */
	0xb7b5, 0x9cf5,	/* ·µ */
	0xb7b6, 0x9cf6,	/* ·¶ */
	0xb7b7, 0x9cf7,	/* ·· */
	0xb7b8, 0x9cfd,	/* ·¸ */
	0xb7b9, 0x9d41,	/* ·¹ */
	0xb7ba, 0x9d42,	/* ·º */
	0xb7bb, 0x9d45,	/* ·» */
	0xb7bc, 0x9d49,	/* ·¼ */
	0xb7bd, 0x9d51,	/* ·½ */
	0xb7be, 0x9d53,	/* ·¾ */
	0xb7bf, 0x9d55,	/* ·¿ */
	0xb7c0, 0x9d57,	/* ·À */
	0xb7c1, 0x9d61,	/* ·Á */
	0xb7c2, 0x9d62,	/* ·Â */
	0xb7c3, 0x9d65,	/* ·Ã */
	0xb7c4, 0x9d69,	/* ·Ä */
	0xb7c5, 0x9d71,	/* ·Å */
	0xb7c6, 0x9d73,	/* ·Æ */
	0xb7c7, 0x9d75,	/* ·Ç */
	0xb7c8, 0x9d76,	/* ·È */
	0xb7c9, 0x9d77,	/* ·É */
	0xb7ca, 0x9d81,	/* ·Ê */
	0xb7cb, 0x9d85,	/* ·Ë */
	0xb7cc, 0x9d93,	/* ·Ì */
	0xb7cd, 0x9d95,	/* ·Í */
	0xb7ce, 0x9da1,	/* ·Î */
	0xb7cf, 0x9da2,	/* ·Ï */
	0xb7d0, 0x9da5,	/* ·Ð */
	0xb7d1, 0x9da9,	/* ·Ñ */
	0xb7d2, 0x9db1,	/* ·Ò */
	0xb7d3, 0x9db3,	/* ·Ó */
	0xb7d4, 0x9db5,	/* ·Ô */
	0xb7d5, 0x9db7,	/* ·Õ */
	0xb7d6, 0x9dc1,	/* ·Ö */
	0xb7d7, 0x9dc5,	/* ·× */
	0xb7d8, 0x9dd7,	/* ·Ø */
	0xb7d9, 0x9df6,	/* ·Ù */
	0xb7da, 0x9e41,	/* ·Ú */
	0xb7db, 0x9e45,	/* ·Û */
	0xb7dc, 0x9e49,	/* ·Ü */
	0xb7dd, 0x9e51,	/* ·Ý */
	0xb7de, 0x9e53,	/* ·Þ */
	0xb7df, 0x9e55,	/* ·ß */
	0xb7e0, 0x9e57,	/* ·à */
	0xb7e1, 0x9e61,	/* ·á */
	0xb7e2, 0x9e65,	/* ·â */
	0xb7e3, 0x9e69,	/* ·ã */
	0xb7e4, 0x9e73,	/* ·ä */
	0xb7e5, 0x9e75,	/* ·å */
	0xb7e6, 0x9e77,	/* ·æ */
	0xb7e7, 0x9e81,	/* ·ç */
	0xb7e8, 0x9e82,	/* ·è */
	0xb7e9, 0x9e85,	/* ·é */
	0xb7ea, 0x9e89,	/* ·ê */
	0xb7eb, 0x9e91,	/* ·ë */
	0xb7ec, 0x9e93,	/* ·ì */
	0xb7ed, 0x9e95,	/* ·í */
	0xb7ee, 0x9e97,	/* ·î */
	0xb7ef, 0x9ea1,	/* ·ï */
	0xb7f0, 0x9eb6,	/* ·ð */
	0xb7f1, 0x9ec1,	/* ·ñ */
	0xb7f2, 0x9ee1,	/* ·ò */
	0xb7f3, 0x9ee2,	/* ·ó */
	0xb7f4, 0x9ee5,	/* ·ô */
	0xb7f5, 0x9ee9,	/* ·õ */
	0xb7f6, 0x9ef1,	/* ·ö */
	0xb7f7, 0x9ef5,	/* ·÷ */
	0xb7f8, 0x9ef7,	/* ·ø */
	0xb7f9, 0x9f41,	/* ·ù */
	0xb7fa, 0x9f42,	/* ·ú */
	0xb7fb, 0x9f45,	/* ·û */
	0xb7fc, 0x9f49,	/* ·ü */
	0xb7fd, 0x9f51,	/* ·ý */
	0xb7fe, 0x9f53,	/* ·þ */
	0xb8a1, 0x9f55,	/* ¸¡ */
	0xb8a2, 0x9f57,	/* ¸¢ */
	0xb8a3, 0x9f61,	/* ¸£ */
	0xb8a4, 0x9f62,	/* ¸¤ */
	0xb8a5, 0x9f65,	/* ¸¥ */
	0xb8a6, 0x9f69,	/* ¸¦ */
	0xb8a7, 0x9f71,	/* ¸§ */
	0xb8a8, 0x9f73,	/* ¸¨ */
	0xb8a9, 0x9f75,	/* ¸© */
	0xb8aa, 0x9f77,	/* ¸ª */
	0xb8ab, 0x9f78,	/* ¸« */
	0xb8ac, 0x9f7b,	/* ¸¬ */
	0xb8ad, 0x9f7c,	/* ¸­ */
	0xb8ae, 0x9fa1,	/* ¸® */
	0xb8af, 0x9fa2,	/* ¸¯ */
	0xb8b0, 0x9fa5,	/* ¸° */
	0xb8b1, 0x9fa9,	/* ¸± */
	0xb8b2, 0x9fb1,	/* ¸² */
	0xb8b3, 0x9fb3,	/* ¸³ */
	0xb8b4, 0x9fb5,	/* ¸´ */
	0xb8b5, 0x9fb7,	/* ¸µ */
	0xb8b6, 0xa061,	/* ¸¶ */
	0xb8b7, 0xa062,	/* ¸· */
	0xb8b8, 0xa065,	/* ¸¸ */
	0xb8b9, 0xa067,	/* ¸¹ */
	0xb8ba, 0xa068,	/* ¸º */
	0xb8bb, 0xa069,	/* ¸» */
	0xb8bc, 0xa06a,	/* ¸¼ */
	0xb8bd, 0xa06b,	/* ¸½ */
	0xb8be, 0xa071,	/* ¸¾ */
	0xb8bf, 0xa073,	/* ¸¿ */
	0xb8c0, 0xa075,	/* ¸À */
	0xb8c1, 0xa077,	/* ¸Á */
	0xb8c2, 0xa078,	/* ¸Â */
	0xb8c3, 0xa07b,	/* ¸Ã */
	0xb8c4, 0xa07d,	/* ¸Ä */
	0xb8c5, 0xa081,	/* ¸Å */
	0xb8c6, 0xa082,	/* ¸Æ */
	0xb8c7, 0xa085,	/* ¸Ç */
	0xb8c8, 0xa089,	/* ¸È */
	0xb8c9, 0xa091,	/* ¸É */
	0xb8ca, 0xa093,	/* ¸Ê */
	0xb8cb, 0xa095,	/* ¸Ë */
	0xb8cc, 0xa096,	/* ¸Ì */
	0xb8cd, 0xa097,	/* ¸Í */
	0xb8ce, 0xa098,	/* ¸Î */
	0xb8cf, 0xa0a1,	/* ¸Ï */
	0xb8d0, 0xa0a2,	/* ¸Ð */
	0xb8d1, 0xa0a9,	/* ¸Ñ */
	0xb8d2, 0xa0b7,	/* ¸Ò */
	0xb8d3, 0xa0e1,	/* ¸Ó */
	0xb8d4, 0xa0e2,	/* ¸Ô */
	0xb8d5, 0xa0e5,	/* ¸Õ */
	0xb8d6, 0xa0e9,	/* ¸Ö */
	0xb8d7, 0xa0eb,	/* ¸× */
	0xb8d8, 0xa0f1,	/* ¸Ø */
	0xb8d9, 0xa0f3,	/* ¸Ù */
	0xb8da, 0xa0f5,	/* ¸Ú */
	0xb8db, 0xa0f7,	/* ¸Û */
	0xb8dc, 0xa0f8,	/* ¸Ü */
	0xb8dd, 0xa0fd,	/* ¸Ý */
	0xb8de, 0xa141,	/* ¸Þ */
	0xb8df, 0xa142,	/* ¸ß */
	0xb8e0, 0xa145,	/* ¸à */
	0xb8e1, 0xa149,	/* ¸á */
	0xb8e2, 0xa151,	/* ¸â */
	0xb8e3, 0xa153,	/* ¸ã */
	0xb8e4, 0xa155,	/* ¸ä */
	0xb8e5, 0xa156,	/* ¸å */
	0xb8e6, 0xa157,	/* ¸æ */
	0xb8e7, 0xa161,	/* ¸ç */
	0xb8e8, 0xa162,	/* ¸è */
	0xb8e9, 0xa165,	/* ¸é */
	0xb8ea, 0xa169,	/* ¸ê */
	0xb8eb, 0xa175,	/* ¸ë */
	0xb8ec, 0xa176,	/* ¸ì */
	0xb8ed, 0xa177,	/* ¸í */
	0xb8ee, 0xa179,	/* ¸î */
	0xb8ef, 0xa181,	/* ¸ï */
	0xb8f0, 0xa1a1,	/* ¸ð */
	0xb8f1, 0xa1a2,	/* ¸ñ */
	0xb8f2, 0xa1a4,	/* ¸ò */
	0xb8f3, 0xa1a5,	/* ¸ó */
	0xb8f4, 0xa1a9,	/* ¸ô */
	0xb8f5, 0xa1ab,	/* ¸õ */
	0xb8f6, 0xa1b1,	/* ¸ö */
	0xb8f7, 0xa1b3,	/* ¸÷ */
	0xb8f8, 0xa1b5,	/* ¸ø */
	0xb8f9, 0xa1b7,	/* ¸ù */
	0xb8fa, 0xa1c1,	/* ¸ú */
	0xb8fb, 0xa1c5,	/* ¸û */
	0xb8fc, 0xa1d6,	/* ¸ü */
	0xb8fd, 0xa1d7,	/* ¸ý */
	0xb8fe, 0xa241,	/* ¸þ */
	0xb9a1, 0xa245,	/* ¹¡ */
	0xb9a2, 0xa249,	/* ¹¢ */
	0xb9a3, 0xa253,	/* ¹£ */
	0xb9a4, 0xa255,	/* ¹¤ */
	0xb9a5, 0xa257,	/* ¹¥ */
	0xb9a6, 0xa261,	/* ¹¦ */
	0xb9a7, 0xa265,	/* ¹§ */
	0xb9a8, 0xa269,	/* ¹¨ */
	0xb9a9, 0xa273,	/* ¹© */
	0xb9aa, 0xa275,	/* ¹ª */
	0xb9ab, 0xa281,	/* ¹« */
	0xb9ac, 0xa282,	/* ¹¬ */
	0xb9ad, 0xa283,	/* ¹­ */
	0xb9ae, 0xa285,	/* ¹® */
	0xb9af, 0xa288,	/* ¹¯ */
	0xb9b0, 0xa289,	/* ¹° */
	0xb9b1, 0xa28a,	/* ¹± */
	0xb9b2, 0xa28b,	/* ¹² */
	0xb9b3, 0xa291,	/* ¹³ */
	0xb9b4, 0xa293,	/* ¹´ */
	0xb9b5, 0xa295,	/* ¹µ */
	0xb9b6, 0xa297,	/* ¹¶ */
	0xb9b7, 0xa29b,	/* ¹· */
	0xb9b8, 0xa29d,	/* ¹¸ */
	0xb9b9, 0xa2a1,	/* ¹¹ */
	0xb9ba, 0xa2a5,	/* ¹º */
	0xb9bb, 0xa2a9,	/* ¹» */
	0xb9bc, 0xa2b3,	/* ¹¼ */
	0xb9bd, 0xa2b5,	/* ¹½ */
	0xb9be, 0xa2c1,	/* ¹¾ */
	0xb9bf, 0xa2e1,	/* ¹¿ */
	0xb9c0, 0xa2e5,	/* ¹À */
	0xb9c1, 0xa2e9,	/* ¹Á */
	0xb9c2, 0xa341,	/* ¹Â */
	0xb9c3, 0xa345,	/* ¹Ã */
	0xb9c4, 0xa349,	/* ¹Ä */
	0xb9c5, 0xa351,	/* ¹Å */
	0xb9c6, 0xa355,	/* ¹Æ */
	0xb9c7, 0xa361,	/* ¹Ç */
	0xb9c8, 0xa365,	/* ¹È */
	0xb9c9, 0xa369,	/* ¹É */
	0xb9ca, 0xa371,	/* ¹Ê */
	0xb9cb, 0xa375,	/* ¹Ë */
	0xb9cc, 0xa3a1,	/* ¹Ì */
	0xb9cd, 0xa3a2,	/* ¹Í */
	0xb9ce, 0xa3a5,	/* ¹Î */
	0xb9cf, 0xa3a8,	/* ¹Ï */
	0xb9d0, 0xa3a9,	/* ¹Ð */
	0xb9d1, 0xa3ab,	/* ¹Ñ */
	0xb9d2, 0xa3b1,	/* ¹Ò */
	0xb9d3, 0xa3b3,	/* ¹Ó */
	0xb9d4, 0xa3b5,	/* ¹Ô */
	0xb9d5, 0xa3b6,	/* ¹Õ */
	0xb9d6, 0xa3b7,	/* ¹Ö */
	0xb9d7, 0xa3b9,	/* ¹× */
	0xb9d8, 0xa3bb,	/* ¹Ø */
	0xb9d9, 0xa461,	/* ¹Ù */
	0xb9da, 0xa462,	/* ¹Ú */
	0xb9db, 0xa463,	/* ¹Û */
	0xb9dc, 0xa464,	/* ¹Ü */
	0xb9dd, 0xa465,	/* ¹Ý */
	0xb9de, 0xa468,	/* ¹Þ */
	0xb9df, 0xa469,	/* ¹ß */
	0xb9e0, 0xa46a,	/* ¹à */
	0xb9e1, 0xa46b,	/* ¹á */
	0xb9e2, 0xa46c,	/* ¹â */
	0xb9e3, 0xa471,	/* ¹ã */
	0xb9e4, 0xa473,	/* ¹ä */
	0xb9e5, 0xa475,	/* ¹å */
	0xb9e6, 0xa477,	/* ¹æ */
	0xb9e7, 0xa47b,	/* ¹ç */
	0xb9e8, 0xa481,	/* ¹è */
	0xb9e9, 0xa482,	/* ¹é */
	0xb9ea, 0xa485,	/* ¹ê */
	0xb9eb, 0xa489,	/* ¹ë */
	0xb9ec, 0xa491,	/* ¹ì */
	0xb9ed, 0xa493,	/* ¹í */
	0xb9ee, 0xa495,	/* ¹î */
	0xb9ef, 0xa496,	/* ¹ï */
	0xb9f0, 0xa497,	/* ¹ð */
	0xb9f1, 0xa49b,	/* ¹ñ */
	0xb9f2, 0xa4a1,	/* ¹ò */
	0xb9f3, 0xa4a2,	/* ¹ó */
	0xb9f4, 0xa4a5,	/* ¹ô */
	0xb9f5, 0xa4b3,	/* ¹õ */
	0xb9f6, 0xa4e1,	/* ¹ö */
	0xb9f7, 0xa4e2,	/* ¹÷ */
	0xb9f8, 0xa4e5,	/* ¹ø */
	0xb9f9, 0xa4e8,	/* ¹ù */
	0xb9fa, 0xa4e9,	/* ¹ú */
	0xb9fb, 0xa4eb,	/* ¹û */
	0xb9fc, 0xa4f1,	/* ¹ü */
	0xb9fd, 0xa4f3,	/* ¹ý */
	0xb9fe, 0xa4f5,	/* ¹þ */
	0xbaa1, 0xa4f7,	/* º¡ */
	0xbaa2, 0xa4f8,	/* º¢ */
	0xbaa3, 0xa541,	/* º£ */
	0xbaa4, 0xa542,	/* º¤ */
	0xbaa5, 0xa545,	/* º¥ */
	0xbaa6, 0xa548,	/* º¦ */
	0xbaa7, 0xa549,	/* º§ */
	0xbaa8, 0xa551,	/* º¨ */
	0xbaa9, 0xa553,	/* º© */
	0xbaaa, 0xa555,	/* ºª */
	0xbaab, 0xa556,	/* º« */
	0xbaac, 0xa557,	/* º¬ */
	0xbaad, 0xa561,	/* º­ */
	0xbaae, 0xa562,	/* º® */
	0xbaaf, 0xa565,	/* º¯ */
	0xbab0, 0xa569,	/* º° */
	0xbab1, 0xa573,	/* º± */
	0xbab2, 0xa575,	/* º² */
	0xbab3, 0xa576,	/* º³ */
	0xbab4, 0xa577,	/* º´ */
	0xbab5, 0xa57b,	/* ºµ */
	0xbab6, 0xa581,	/* º¶ */
	0xbab7, 0xa585,	/* º· */
	0xbab8, 0xa5a1,	/* º¸ */
	0xbab9, 0xa5a2,	/* º¹ */
	0xbaba, 0xa5a3,	/* ºº */
	0xbabb, 0xa5a5,	/* º» */
	0xbabc, 0xa5a9,	/* º¼ */
	0xbabd, 0xa5b1,	/* º½ */
	0xbabe, 0xa5b3,	/* º¾ */
	0xbabf, 0xa5b5,	/* º¿ */
	0xbac0, 0xa5b7,	/* ºÀ */
	0xbac1, 0xa5c1,	/* ºÁ */
	0xbac2, 0xa5c5,	/* ºÂ */
	0xbac3, 0xa5d6,	/* ºÃ */
	0xbac4, 0xa5e1,	/* ºÄ */
	0xbac5, 0xa5f6,	/* ºÅ */
	0xbac6, 0xa641,	/* ºÆ */
	0xbac7, 0xa642,	/* ºÇ */
	0xbac8, 0xa645,	/* ºÈ */
	0xbac9, 0xa649,	/* ºÉ */
	0xbaca, 0xa651,	/* ºÊ */
	0xbacb, 0xa653,	/* ºË */
	0xbacc, 0xa661,	/* ºÌ */
	0xbacd, 0xa665,	/* ºÍ */
	0xbace, 0xa681,	/* ºÎ */
	0xbacf, 0xa682,	/* ºÏ */
	0xbad0, 0xa685,	/* ºÐ */
	0xbad1, 0xa688,	/* ºÑ */
	0xbad2, 0xa689,	/* ºÒ */
	0xbad3, 0xa68a,	/* ºÓ */
	0xbad4, 0xa68b,	/* ºÔ */
	0xbad5, 0xa691,	/* ºÕ */
	0xbad6, 0xa693,	/* ºÖ */
	0xbad7, 0xa695,	/* º× */
	0xbad8, 0xa697,	/* ºØ */
	0xbad9, 0xa69b,	/* ºÙ */
	0xbada, 0xa69c,	/* ºÚ */
	0xbadb, 0xa6a1,	/* ºÛ */
	0xbadc, 0xa6a9,	/* ºÜ */
	0xbadd, 0xa6b6,	/* ºÝ */
	0xbade, 0xa6c1,	/* ºÞ */
	0xbadf, 0xa6e1,	/* ºß */
	0xbae0, 0xa6e2,	/* ºà */
	0xbae1, 0xa6e5,	/* ºá */
	0xbae2, 0xa6e9,	/* ºâ */
	0xbae3, 0xa6f7,	/* ºã */
	0xbae4, 0xa741,	/* ºä */
	0xbae5, 0xa745,	/* ºå */
	0xbae6, 0xa749,	/* ºæ */
	0xbae7, 0xa751,	/* ºç */
	0xbae8, 0xa755,	/* ºè */
	0xbae9, 0xa757,	/* ºé */
	0xbaea, 0xa761,	/* ºê */
	0xbaeb, 0xa762,	/* ºë */
	0xbaec, 0xa765,	/* ºì */
	0xbaed, 0xa769,	/* ºí */
	0xbaee, 0xa771,	/* ºî */
	0xbaef, 0xa773,	/* ºï */
	0xbaf0, 0xa775,	/* ºð */
	0xbaf1, 0xa7a1,	/* ºñ */
	0xbaf2, 0xa7a2,	/* ºò */
	0xbaf3, 0xa7a5,	/* ºó */
	0xbaf4, 0xa7a9,	/* ºô */
	0xbaf5, 0xa7ab,	/* ºõ */
	0xbaf6, 0xa7b1,	/* ºö */
	0xbaf7, 0xa7b3,	/* º÷ */
	0xbaf8, 0xa7b5,	/* ºø */
	0xbaf9, 0xa7b7,	/* ºù */
	0xbafa, 0xa7b8,	/* ºú */
	0xbafb, 0xa7b9,	/* ºû */
	0xbafc, 0xa861,	/* ºü */
	0xbafd, 0xa862,	/* ºý */
	0xbafe, 0xa865,	/* ºþ */
	0xbba1, 0xa869,	/* »¡ */
	0xbba2, 0xa86b,	/* »¢ */
	0xbba3, 0xa871,	/* »£ */
	0xbba4, 0xa873,	/* »¤ */
	0xbba5, 0xa875,	/* »¥ */
	0xbba6, 0xa876,	/* »¦ */
	0xbba7, 0xa877,	/* »§ */
	0xbba8, 0xa87d,	/* »¨ */
	0xbba9, 0xa881,	/* »© */
	0xbbaa, 0xa882,	/* »ª */
	0xbbab, 0xa885,	/* »« */
	0xbbac, 0xa889,	/* »¬ */
	0xbbad, 0xa891,	/* »­ */
	0xbbae, 0xa893,	/* »® */
	0xbbaf, 0xa895,	/* »¯ */
	0xbbb0, 0xa896,	/* »° */
	0xbbb1, 0xa897,	/* »± */
	0xbbb2, 0xa8a1,	/* »² */
	0xbbb3, 0xa8a2,	/* »³ */
	0xbbb4, 0xa8b1,	/* »´ */
	0xbbb5, 0xa8e1,	/* »µ */
	0xbbb6, 0xa8e2,	/* »¶ */
	0xbbb7, 0xa8e5,	/* »· */
	0xbbb8, 0xa8e8,	/* »¸ */
	0xbbb9, 0xa8e9,	/* »¹ */
	0xbbba, 0xa8f1,	/* »º */
	0xbbbb, 0xa8f5,	/* »» */
	0xbbbc, 0xa8f6,	/* »¼ */
	0xbbbd, 0xa8f7,	/* »½ */
	0xbbbe, 0xa941,	/* »¾ */
	0xbbbf, 0xa957,	/* »¿ */
	0xbbc0, 0xa961,	/* »À */
	0xbbc1, 0xa962,	/* »Á */
	0xbbc2, 0xa971,	/* »Â */
	0xbbc3, 0xa973,	/* »Ã */
	0xbbc4, 0xa975,	/* »Ä */
	0xbbc5, 0xa976,	/* »Å */
	0xbbc6, 0xa977,	/* »Æ */
	0xbbc7, 0xa9a1,	/* »Ç */
	0xbbc8, 0xa9a2,	/* »È */
	0xbbc9, 0xa9a5,	/* »É */
	0xbbca, 0xa9a9,	/* »Ê */
	0xbbcb, 0xa9b1,	/* »Ë */
	0xbbcc, 0xa9b3,	/* »Ì */
	0xbbcd, 0xa9b7,	/* »Í */
	0xbbce, 0xaa41,	/* »Î */
	0xbbcf, 0xaa61,	/* »Ï */
	0xbbd0, 0xaa77,	/* »Ð */
	0xbbd1, 0xaa81,	/* »Ñ */
	0xbbd2, 0xaa82,	/* »Ò */
	0xbbd3, 0xaa85,	/* »Ó */
	0xbbd4, 0xaa89,	/* »Ô */
	0xbbd5, 0xaa91,	/* »Õ */
	0xbbd6, 0xaa95,	/* »Ö */
	0xbbd7, 0xaa97,	/* »× */
	0xbbd8, 0xab41,	/* »Ø */
	0xbbd9, 0xab57,	/* »Ù */
	0xbbda, 0xab61,	/* »Ú */
	0xbbdb, 0xab65,	/* »Û */
	0xbbdc, 0xab69,	/* »Ü */
	0xbbdd, 0xab71,	/* »Ý */
	0xbbde, 0xab73,	/* »Þ */
	0xbbdf, 0xaba1,	/* »ß */
	0xbbe0, 0xaba2,	/* »à */
	0xbbe1, 0xaba5,	/* »á */
	0xbbe2, 0xaba9,	/* »â */
	0xbbe3, 0xabb1,	/* »ã */
	0xbbe4, 0xabb3,	/* »ä */
	0xbbe5, 0xabb5,	/* »å */
	0xbbe6, 0xabb7,	/* »æ */
	0xbbe7, 0xac61,	/* »ç */
	0xbbe8, 0xac62,	/* »è */
	0xbbe9, 0xac64,	/* »é */
	0xbbea, 0xac65,	/* »ê */
	0xbbeb, 0xac68,	/* »ë */
	0xbbec, 0xac69,	/* »ì */
	0xbbed, 0xac6a,	/* »í */
	0xbbee, 0xac6b,	/* »î */
	0xbbef, 0xac71,	/* »ï */
	0xbbf0, 0xac73,	/* »ð */
	0xbbf1, 0xac75,	/* »ñ */
	0xbbf2, 0xac76,	/* »ò */
	0xbbf3, 0xac77,	/* »ó */
	0xbbf4, 0xac7b,	/* »ô */
	0xbbf5, 0xac81,	/* »õ */
	0xbbf6, 0xac82,	/* »ö */
	0xbbf7, 0xac85,	/* »÷ */
	0xbbf8, 0xac89,	/* »ø */
	0xbbf9, 0xac91,	/* »ù */
	0xbbfa, 0xac93,	/* »ú */
	0xbbfb, 0xac95,	/* »û */
	0xbbfc, 0xac96,	/* »ü */
	0xbbfd, 0xac97,	/* »ý */
	0xbbfe, 0xaca1,	/* »þ */
	0xbca1, 0xaca2,	/* ¼¡ */
	0xbca2, 0xaca5,	/* ¼¢ */
	0xbca3, 0xaca9,	/* ¼£ */
	0xbca4, 0xacb1,	/* ¼¤ */
	0xbca5, 0xacb3,	/* ¼¥ */
	0xbca6, 0xacb5,	/* ¼¦ */
	0xbca7, 0xacb7,	/* ¼§ */
	0xbca8, 0xacc1,	/* ¼¨ */
	0xbca9, 0xacc5,	/* ¼© */
	0xbcaa, 0xacc9,	/* ¼ª */
	0xbcab, 0xacd1,	/* ¼« */
	0xbcac, 0xacd7,	/* ¼¬ */
	0xbcad, 0xace1,	/* ¼­ */
	0xbcae, 0xace2,	/* ¼® */
	0xbcaf, 0xace3,	/* ¼¯ */
	0xbcb0, 0xace4,	/* ¼° */
	0xbcb1, 0xace5,	/* ¼± */
	0xbcb2, 0xace8,	/* ¼² */
	0xbcb3, 0xace9,	/* ¼³ */
	0xbcb4, 0xaceb,	/* ¼´ */
	0xbcb5, 0xacec,	/* ¼µ */
	0xbcb6, 0xacf1,	/* ¼¶ */
	0xbcb7, 0xacf3,	/* ¼· */
	0xbcb8, 0xacf5,	/* ¼¸ */
	0xbcb9, 0xacf6,	/* ¼¹ */
	0xbcba, 0xacf7,	/* ¼º */
	0xbcbb, 0xacfc,	/* ¼» */
	0xbcbc, 0xad41,	/* ¼¼ */
	0xbcbd, 0xad42,	/* ¼½ */
	0xbcbe, 0xad45,	/* ¼¾ */
	0xbcbf, 0xad49,	/* ¼¿ */
	0xbcc0, 0xad51,	/* ¼À */
	0xbcc1, 0xad53,	/* ¼Á */
	0xbcc2, 0xad55,	/* ¼Â */
	0xbcc3, 0xad56,	/* ¼Ã */
	0xbcc4, 0xad57,	/* ¼Ä */
	0xbcc5, 0xad61,	/* ¼Å */
	0xbcc6, 0xad62,	/* ¼Æ */
	0xbcc7, 0xad65,	/* ¼Ç */
	0xbcc8, 0xad69,	/* ¼È */
	0xbcc9, 0xad71,	/* ¼É */
	0xbcca, 0xad73,	/* ¼Ê */
	0xbccb, 0xad75,	/* ¼Ë */
	0xbccc, 0xad76,	/* ¼Ì */
	0xbccd, 0xad77,	/* ¼Í */
	0xbcce, 0xad81,	/* ¼Î */
	0xbccf, 0xad85,	/* ¼Ï */
	0xbcd0, 0xad89,	/* ¼Ð */
	0xbcd1, 0xad97,	/* ¼Ñ */
	0xbcd2, 0xada1,	/* ¼Ò */
	0xbcd3, 0xada2,	/* ¼Ó */
	0xbcd4, 0xada3,	/* ¼Ô */
	0xbcd5, 0xada5,	/* ¼Õ */
	0xbcd6, 0xada9,	/* ¼Ö */
	0xbcd7, 0xadab,	/* ¼× */
	0xbcd8, 0xadb1,	/* ¼Ø */
	0xbcd9, 0xadb3,	/* ¼Ù */
	0xbcda, 0xadb5,	/* ¼Ú */
	0xbcdb, 0xadb7,	/* ¼Û */
	0xbcdc, 0xadbb,	/* ¼Ü */
	0xbcdd, 0xadc1,	/* ¼Ý */
	0xbcde, 0xadc2,	/* ¼Þ */
	0xbcdf, 0xadc5,	/* ¼ß */
	0xbce0, 0xadc9,	/* ¼à */
	0xbce1, 0xadd7,	/* ¼á */
	0xbce2, 0xade1,	/* ¼â */
	0xbce3, 0xade5,	/* ¼ã */
	0xbce4, 0xade9,	/* ¼ä */
	0xbce5, 0xadf1,	/* ¼å */
	0xbce6, 0xadf5,	/* ¼æ */
	0xbce7, 0xadf6,	/* ¼ç */
	0xbce8, 0xae41,	/* ¼è */
	0xbce9, 0xae45,	/* ¼é */
	0xbcea, 0xae49,	/* ¼ê */
	0xbceb, 0xae51,	/* ¼ë */
	0xbcec, 0xae53,	/* ¼ì */
	0xbced, 0xae55,	/* ¼í */
	0xbcee, 0xae61,	/* ¼î */
	0xbcef, 0xae62,	/* ¼ï */
	0xbcf0, 0xae65,	/* ¼ð */
	0xbcf1, 0xae69,	/* ¼ñ */
	0xbcf2, 0xae71,	/* ¼ò */
	0xbcf3, 0xae73,	/* ¼ó */
	0xbcf4, 0xae75,	/* ¼ô */
	0xbcf5, 0xae77,	/* ¼õ */
	0xbcf6, 0xae81,	/* ¼ö */
	0xbcf7, 0xae82,	/* ¼÷ */
	0xbcf8, 0xae85,	/* ¼ø */
	0xbcf9, 0xae88,	/* ¼ù */
	0xbcfa, 0xae89,	/* ¼ú */
	0xbcfb, 0xae91,	/* ¼û */
	0xbcfc, 0xae93,	/* ¼ü */
	0xbcfd, 0xae95,	/* ¼ý */
	0xbcfe, 0xae97,	/* ¼þ */
	0xbda1, 0xae99,	/* ½¡ */
	0xbda2, 0xae9b,	/* ½¢ */
	0xbda3, 0xae9c,	/* ½£ */
	0xbda4, 0xaea1,	/* ½¤ */
	0xbda5, 0xaeb6,	/* ½¥ */
	0xbda6, 0xaec1,	/* ½¦ */
	0xbda7, 0xaec2,	/* ½§ */
	0xbda8, 0xaec5,	/* ½¨ */
	0xbda9, 0xaec9,	/* ½© */
	0xbdaa, 0xaed1,	/* ½ª */
	0xbdab, 0xaed7,	/* ½« */
	0xbdac, 0xaee1,	/* ½¬ */
	0xbdad, 0xaee2,	/* ½­ */
	0xbdae, 0xaee5,	/* ½® */
	0xbdaf, 0xaee9,	/* ½¯ */
	0xbdb0, 0xaef1,	/* ½° */
	0xbdb1, 0xaef3,	/* ½± */
	0xbdb2, 0xaef5,	/* ½² */
	0xbdb3, 0xaef7,	/* ½³ */
	0xbdb4, 0xaf41,	/* ½´ */
	0xbdb5, 0xaf42,	/* ½µ */
	0xbdb6, 0xaf49,	/* ½¶ */
	0xbdb7, 0xaf51,	/* ½· */
	0xbdb8, 0xaf55,	/* ½¸ */
	0xbdb9, 0xaf57,	/* ½¹ */
	0xbdba, 0xaf61,	/* ½º */
	0xbdbb, 0xaf62,	/* ½» */
	0xbdbc, 0xaf65,	/* ½¼ */
	0xbdbd, 0xaf69,	/* ½½ */
	0xbdbe, 0xaf6a,	/* ½¾ */
	0xbdbf, 0xaf71,	/* ½¿ */
	0xbdc0, 0xaf73,	/* ½À */
	0xbdc1, 0xaf75,	/* ½Á */
	0xbdc2, 0xaf77,	/* ½Â */
	0xbdc3, 0xafa1,	/* ½Ã */
	0xbdc4, 0xafa2,	/* ½Ä */
	0xbdc5, 0xafa5,	/* ½Å */
	0xbdc6, 0xafa8,	/* ½Æ */
	0xbdc7, 0xafa9,	/* ½Ç */
	0xbdc8, 0xafb0,	/* ½È */
	0xbdc9, 0xafb1,	/* ½É */
	0xbdca, 0xafb3,	/* ½Ê */
	0xbdcb, 0xafb5,	/* ½Ë */
	0xbdcc, 0xafb7,	/* ½Ì */
	0xbdcd, 0xafbc,	/* ½Í */
	0xbdce, 0xb061,	/* ½Î */
	0xbdcf, 0xb062,	/* ½Ï */
	0xbdd0, 0xb064,	/* ½Ð */
	0xbdd1, 0xb065,	/* ½Ñ */
	0xbdd2, 0xb069,	/* ½Ò */
	0xbdd3, 0xb071,	/* ½Ó */
	0xbdd4, 0xb073,	/* ½Ô */
	0xbdd5, 0xb076,	/* ½Õ */
	0xbdd6, 0xb077,	/* ½Ö */
	0xbdd7, 0xb07d,	/* ½× */
	0xbdd8, 0xb081,	/* ½Ø */
	0xbdd9, 0xb082,	/* ½Ù */
	0xbdda, 0xb085,	/* ½Ú */
	0xbddb, 0xb089,	/* ½Û */
	0xbddc, 0xb091,	/* ½Ü */
	0xbddd, 0xb093,	/* ½Ý */
	0xbdde, 0xb096,	/* ½Þ */
	0xbddf, 0xb097,	/* ½ß */
	0xbde0, 0xb0b7,	/* ½à */
	0xbde1, 0xb0e1,	/* ½á */
	0xbde2, 0xb0e2,	/* ½â */
	0xbde3, 0xb0e5,	/* ½ã */
	0xbde4, 0xb0e9,	/* ½ä */
	0xbde5, 0xb0eb,	/* ½å */
	0xbde6, 0xb0f1,	/* ½æ */
	0xbde7, 0xb0f3,	/* ½ç */
	0xbde8, 0xb0f6,	/* ½è */
	0xbde9, 0xb0f7,	/* ½é */
	0xbdea, 0xb141,	/* ½ê */
	0xbdeb, 0xb145,	/* ½ë */
	0xbdec, 0xb149,	/* ½ì */
	0xbded, 0xb185,	/* ½í */
	0xbdee, 0xb1a1,	/* ½î */
	0xbdef, 0xb1a2,	/* ½ï */
	0xbdf0, 0xb1a5,	/* ½ð */
	0xbdf1, 0xb1a8,	/* ½ñ */
	0xbdf2, 0xb1a9,	/* ½ò */
	0xbdf3, 0xb1ab,	/* ½ó */
	0xbdf4, 0xb1b1,	/* ½ô */
	0xbdf5, 0xb1b3,	/* ½õ */
	0xbdf6, 0xb1b7,	/* ½ö */
	0xbdf7, 0xb1c1,	/* ½÷ */
	0xbdf8, 0xb1c2,	/* ½ø */
	0xbdf9, 0xb1c5,	/* ½ù */
	0xbdfa, 0xb1d6,	/* ½ú */
	0xbdfb, 0xb1e1,	/* ½û */
	0xbdfc, 0xb1f6,	/* ½ü */
	0xbdfd, 0xb241,	/* ½ý */
	0xbdfe, 0xb245,	/* ½þ */
	0xbea1, 0xb249,	/* ¾¡ */
	0xbea2, 0xb251,	/* ¾¢ */
	0xbea3, 0xb253,	/* ¾£ */
	0xbea4, 0xb261,	/* ¾¤ */
	0xbea5, 0xb281,	/* ¾¥ */
	0xbea6, 0xb282,	/* ¾¦ */
	0xbea7, 0xb285,	/* ¾§ */
	0xbea8, 0xb289,	/* ¾¨ */
	0xbea9, 0xb291,	/* ¾© */
	0xbeaa, 0xb293,	/* ¾ª */
	0xbeab, 0xb297,	/* ¾« */
	0xbeac, 0xb2a1,	/* ¾¬ */
	0xbead, 0xb2b6,	/* ¾­ */
	0xbeae, 0xb2c1,	/* ¾® */
	0xbeaf, 0xb2e1,	/* ¾¯ */
	0xbeb0, 0xb2e5,	/* ¾° */
	0xbeb1, 0xb357,	/* ¾± */
	0xbeb2, 0xb361,	/* ¾² */
	0xbeb3, 0xb362,	/* ¾³ */
	0xbeb4, 0xb365,	/* ¾´ */
	0xbeb5, 0xb369,	/* ¾µ */
	0xbeb6, 0xb36b,	/* ¾¶ */
	0xbeb7, 0xb370,	/* ¾· */
	0xbeb8, 0xb371,	/* ¾¸ */
	0xbeb9, 0xb373,	/* ¾¹ */
	0xbeba, 0xb381,	/* ¾º */
	0xbebb, 0xb385,	/* ¾» */
	0xbebc, 0xb389,	/* ¾¼ */
	0xbebd, 0xb391,	/* ¾½ */
	0xbebe, 0xb3a1,	/* ¾¾ */
	0xbebf, 0xb3a2,	/* ¾¿ */
	0xbec0, 0xb3a5,	/* ¾À */
	0xbec1, 0xb3a9,	/* ¾Á */
	0xbec2, 0xb3b1,	/* ¾Â */
	0xbec3, 0xb3b3,	/* ¾Ã */
	0xbec4, 0xb3b5,	/* ¾Ä */
	0xbec5, 0xb3b7,	/* ¾Å */
	0xbec6, 0xb461,	/* ¾Æ */
	0xbec7, 0xb462,	/* ¾Ç */
	0xbec8, 0xb465,	/* ¾È */
	0xbec9, 0xb466,	/* ¾É */
	0xbeca, 0xb467,	/* ¾Ê */
	0xbecb, 0xb469,	/* ¾Ë */
	0xbecc, 0xb46a,	/* ¾Ì */
	0xbecd, 0xb46b,	/* ¾Í */
	0xbece, 0xb470,	/* ¾Î */
	0xbecf, 0xb471,	/* ¾Ï */
	0xbed0, 0xb473,	/* ¾Ð */
	0xbed1, 0xb475,	/* ¾Ñ */
	0xbed2, 0xb476,	/* ¾Ò */
	0xbed3, 0xb477,	/* ¾Ó */
	0xbed4, 0xb47b,	/* ¾Ô */
	0xbed5, 0xb47c,	/* ¾Õ */
	0xbed6, 0xb481,	/* ¾Ö */
	0xbed7, 0xb482,	/* ¾× */
	0xbed8, 0xb485,	/* ¾Ø */
	0xbed9, 0xb489,	/* ¾Ù */
	0xbeda, 0xb491,	/* ¾Ú */
	0xbedb, 0xb493,	/* ¾Û */
	0xbedc, 0xb495,	/* ¾Ü */
	0xbedd, 0xb496,	/* ¾Ý */
	0xbede, 0xb497,	/* ¾Þ */
	0xbedf, 0xb4a1,	/* ¾ß */
	0xbee0, 0xb4a2,	/* ¾à */
	0xbee1, 0xb4a5,	/* ¾á */
	0xbee2, 0xb4a9,	/* ¾â */
	0xbee3, 0xb4ac,	/* ¾ã */
	0xbee4, 0xb4b1,	/* ¾ä */
	0xbee5, 0xb4b3,	/* ¾å */
	0xbee6, 0xb4b5,	/* ¾æ */
	0xbee7, 0xb4b7,	/* ¾ç */
	0xbee8, 0xb4bb,	/* ¾è */
	0xbee9, 0xb4bd,	/* ¾é */
	0xbeea, 0xb4c1,	/* ¾ê */
	0xbeeb, 0xb4c5,	/* ¾ë */
	0xbeec, 0xb4c9,	/* ¾ì */
	0xbeed, 0xb4d3,	/* ¾í */
	0xbeee, 0xb4e1,	/* ¾î */
	0xbeef, 0xb4e2,	/* ¾ï */
	0xbef0, 0xb4e5,	/* ¾ð */
	0xbef1, 0xb4e6,	/* ¾ñ */
	0xbef2, 0xb4e8,	/* ¾ò */
	0xbef3, 0xb4e9,	/* ¾ó */
	0xbef4, 0xb4ea,	/* ¾ô */
	0xbef5, 0xb4eb,	/* ¾õ */
	0xbef6, 0xb4f1,	/* ¾ö */
	0xbef7, 0xb4f3,	/* ¾÷ */
	0xbef8, 0xb4f4,	/* ¾ø */
	0xbef9, 0xb4f5,	/* ¾ù */
	0xbefa, 0xb4f6,	/* ¾ú */
	0xbefb, 0xb4f7,	/* ¾û */
	0xbefc, 0xb4f8,	/* ¾ü */
	0xbefd, 0xb4fa,	/* ¾ý */
	0xbefe, 0xb4fc,	/* ¾þ */
	0xbfa1, 0xb541,	/* ¿¡ */
	0xbfa2, 0xb542,	/* ¿¢ */
	0xbfa3, 0xb545,	/* ¿£ */
	0xbfa4, 0xb549,	/* ¿¤ */
	0xbfa5, 0xb551,	/* ¿¥ */
	0xbfa6, 0xb553,	/* ¿¦ */
	0xbfa7, 0xb555,	/* ¿§ */
	0xbfa8, 0xb557,	/* ¿¨ */
	0xbfa9, 0xb561,	/* ¿© */
	0xbfaa, 0xb562,	/* ¿ª */
	0xbfab, 0xb563,	/* ¿« */
	0xbfac, 0xb565,	/* ¿¬ */
	0xbfad, 0xb569,	/* ¿­ */
	0xbfae, 0xb56b,	/* ¿® */
	0xbfaf, 0xb56c,	/* ¿¯ */
	0xbfb0, 0xb571,	/* ¿° */
	0xbfb1, 0xb573,	/* ¿± */
	0xbfb2, 0xb574,	/* ¿² */
	0xbfb3, 0xb575,	/* ¿³ */
	0xbfb4, 0xb576,	/* ¿´ */
	0xbfb5, 0xb577,	/* ¿µ */
	0xbfb6, 0xb57b,	/* ¿¶ */
	0xbfb7, 0xb57c,	/* ¿· */
	0xbfb8, 0xb57d,	/* ¿¸ */
	0xbfb9, 0xb581,	/* ¿¹ */
	0xbfba, 0xb585,	/* ¿º */
	0xbfbb, 0xb589,	/* ¿» */
	0xbfbc, 0xb591,	/* ¿¼ */
	0xbfbd, 0xb593,	/* ¿½ */
	0xbfbe, 0xb595,	/* ¿¾ */
	0xbfbf, 0xb596,	/* ¿¿ */
	0xbfc0, 0xb5a1,	/* ¿À */
	0xbfc1, 0xb5a2,	/* ¿Á */
	0xbfc2, 0xb5a5,	/* ¿Â */
	0xbfc3, 0xb5a9,	/* ¿Ã */
	0xbfc4, 0xb5aa,	/* ¿Ä */
	0xbfc5, 0xb5ab,	/* ¿Å */
	0xbfc6, 0xb5ad,	/* ¿Æ */
	0xbfc7, 0xb5b0,	/* ¿Ç */
	0xbfc8, 0xb5b1,	/* ¿È */
	0xbfc9, 0xb5b3,	/* ¿É */
	0xbfca, 0xb5b5,	/* ¿Ê */
	0xbfcb, 0xb5b7,	/* ¿Ë */
	0xbfcc, 0xb5b9,	/* ¿Ì */
	0xbfcd, 0xb5c1,	/* ¿Í */
	0xbfce, 0xb5c2,	/* ¿Î */
	0xbfcf, 0xb5c5,	/* ¿Ï */
	0xbfd0, 0xb5c9,	/* ¿Ð */
	0xbfd1, 0xb5d1,	/* ¿Ñ */
	0xbfd2, 0xb5d3,	/* ¿Ò */
	0xbfd3, 0xb5d5,	/* ¿Ó */
	0xbfd4, 0xb5d6,	/* ¿Ô */
	0xbfd5, 0xb5d7,	/* ¿Õ */
	0xbfd6, 0xb5e1,	/* ¿Ö */
	0xbfd7, 0xb5e2,	/* ¿× */
	0xbfd8, 0xb5e5,	/* ¿Ø */
	0xbfd9, 0xb5f1,	/* ¿Ù */
	0xbfda, 0xb5f5,	/* ¿Ú */
	0xbfdb, 0xb5f7,	/* ¿Û */
	0xbfdc, 0xb641,	/* ¿Ü */
	0xbfdd, 0xb642,	/* ¿Ý */
	0xbfde, 0xb645,	/* ¿Þ */
	0xbfdf, 0xb649,	/* ¿ß */
	0xbfe0, 0xb651,	/* ¿à */
	0xbfe1, 0xb653,	/* ¿á */
	0xbfe2, 0xb655,	/* ¿â */
	0xbfe3, 0xb657,	/* ¿ã */
	0xbfe4, 0xb661,	/* ¿ä */
	0xbfe5, 0xb662,	/* ¿å */
	0xbfe6, 0xb665,	/* ¿æ */
	0xbfe7, 0xb669,	/* ¿ç */
	0xbfe8, 0xb671,	/* ¿è */
	0xbfe9, 0xb673,	/* ¿é */
	0xbfea, 0xb675,	/* ¿ê */
	0xbfeb, 0xb677,	/* ¿ë */
	0xbfec, 0xb681,	/* ¿ì */
	0xbfed, 0xb682,	/* ¿í */
	0xbfee, 0xb685,	/* ¿î */
	0xbfef, 0xb689,	/* ¿ï */
	0xbff0, 0xb68a,	/* ¿ð */
	0xbff1, 0xb68b,	/* ¿ñ */
	0xbff2, 0xb691,	/* ¿ò */
	0xbff3, 0xb693,	/* ¿ó */
	0xbff4, 0xb695,	/* ¿ô */
	0xbff5, 0xb697,	/* ¿õ */
	0xbff6, 0xb6a1,	/* ¿ö */
	0xbff7, 0xb6a2,	/* ¿÷ */
	0xbff8, 0xb6a5,	/* ¿ø */
	0xbff9, 0xb6a9,	/* ¿ù */
	0xbffa, 0xb6b1,	/* ¿ú */
	0xbffb, 0xb6b3,	/* ¿û */
	0xbffc, 0xb6b6,	/* ¿ü */
	0xbffd, 0xb6b7,	/* ¿ý */
	0xbffe, 0xb6c1,	/* ¿þ */
	0xc0a1, 0xb6c2,	/* À¡ */
	0xc0a2, 0xb6c5,	/* À¢ */
	0xc0a3, 0xb6c9,	/* À£ */
	0xc0a4, 0xb6d1,	/* À¤ */
	0xc0a5, 0xb6d3,	/* À¥ */
	0xc0a6, 0xb6d7,	/* À¦ */
	0xc0a7, 0xb6e1,	/* À§ */
	0xc0a8, 0xb6e2,	/* À¨ */
	0xc0a9, 0xb6e5,	/* À© */
	0xc0aa, 0xb6e9,	/* Àª */
	0xc0ab, 0xb6f1,	/* À« */
	0xc0ac, 0xb6f3,	/* À¬ */
	0xc0ad, 0xb6f5,	/* À­ */
	0xc0ae, 0xb6f7,	/* À® */
	0xc0af, 0xb741,	/* À¯ */
	0xc0b0, 0xb742,	/* À° */
	0xc0b1, 0xb745,	/* À± */
	0xc0b2, 0xb749,	/* À² */
	0xc0b3, 0xb751,	/* À³ */
	0xc0b4, 0xb753,	/* À´ */
	0xc0b5, 0xb755,	/* Àµ */
	0xc0b6, 0xb757,	/* À¶ */
	0xc0b7, 0xb759,	/* À· */
	0xc0b8, 0xb761,	/* À¸ */
	0xc0b9, 0xb762,	/* À¹ */
	0xc0ba, 0xb765,	/* Àº */
	0xc0bb, 0xb769,	/* À» */
	0xc0bc, 0xb76f,	/* À¼ */
	0xc0bd, 0xb771,	/* À½ */
	0xc0be, 0xb773,	/* À¾ */
	0xc0bf, 0xb775,	/* À¿ */
	0xc0c0, 0xb777,	/* ÀÀ */
	0xc0c1, 0xb778,	/* ÀÁ */
	0xc0c2, 0xb779,	/* ÀÂ */
	0xc0c3, 0xb77a,	/* ÀÃ */
	0xc0c4, 0xb77b,	/* ÀÄ */
	0xc0c5, 0xb77c,	/* ÀÅ */
	0xc0c6, 0xb77d,	/* ÀÆ */
	0xc0c7, 0xb781,	/* ÀÇ */
	0xc0c8, 0xb785,	/* ÀÈ */
	0xc0c9, 0xb789,	/* ÀÉ */
	0xc0ca, 0xb791,	/* ÀÊ */
	0xc0cb, 0xb795,	/* ÀË */
	0xc0cc, 0xb7a1,	/* ÀÌ */
	0xc0cd, 0xb7a2,	/* ÀÍ */
	0xc0ce, 0xb7a5,	/* ÀÎ */
	0xc0cf, 0xb7a9,	/* ÀÏ */
	0xc0d0, 0xb7aa,	/* ÀÐ */
	0xc0d1, 0xb7ab,	/* ÀÑ */
	0xc0d2, 0xb7b0,	/* ÀÒ */
	0xc0d3, 0xb7b1,	/* ÀÓ */
	0xc0d4, 0xb7b3,	/* ÀÔ */
	0xc0d5, 0xb7b5,	/* ÀÕ */
	0xc0d6, 0xb7b6,	/* ÀÖ */
	0xc0d7, 0xb7b7,	/* À× */
	0xc0d8, 0xb7b8,	/* ÀØ */
	0xc0d9, 0xb7bc,	/* ÀÙ */
	0xc0da, 0xb861,	/* ÀÚ */
	0xc0db, 0xb862,	/* ÀÛ */
	0xc0dc, 0xb865,	/* ÀÜ */
	0xc0dd, 0xb867,	/* ÀÝ */
	0xc0de, 0xb868,	/* ÀÞ */
	0xc0df, 0xb869,	/* Àß */
	0xc0e0, 0xb86b,	/* Àà */
	0xc0e1, 0xb871,	/* Àá */
	0xc0e2, 0xb873,	/* Àâ */
	0xc0e3, 0xb875,	/* Àã */
	0xc0e4, 0xb876,	/* Àä */
	0xc0e5, 0xb877,	/* Àå */
	0xc0e6, 0xb878,	/* Àæ */
	0xc0e7, 0xb881,	/* Àç */
	0xc0e8, 0xb882,	/* Àè */
	0xc0e9, 0xb885,	/* Àé */
	0xc0ea, 0xb889,	/* Àê */
	0xc0eb, 0xb891,	/* Àë */
	0xc0ec, 0xb893,	/* Àì */
	0xc0ed, 0xb895,	/* Àí */
	0xc0ee, 0xb896,	/* Àî */
	0xc0ef, 0xb897,	/* Àï */
	0xc0f0, 0xb8a1,	/* Àð */
	0xc0f1, 0xb8a2,	/* Àñ */
	0xc0f2, 0xb8a5,	/* Àò */
	0xc0f3, 0xb8a7,	/* Àó */
	0xc0f4, 0xb8a9,	/* Àô */
	0xc0f5, 0xb8b1,	/* Àõ */
	0xc0f6, 0xb8b7,	/* Àö */
	0xc0f7, 0xb8c1,	/* À÷ */
	0xc0f8, 0xb8c5,	/* Àø */
	0xc0f9, 0xb8c9,	/* Àù */
	0xc0fa, 0xb8e1,	/* Àú */
	0xc0fb, 0xb8e2,	/* Àû */
	0xc0fc, 0xb8e5,	/* Àü */
	0xc0fd, 0xb8e9,	/* Àý */
	0xc0fe, 0xb8eb,	/* Àþ */
	0xc1a1, 0xb8f1,	/* Á¡ */
	0xc1a2, 0xb8f3,	/* Á¢ */
	0xc1a3, 0xb8f5,	/* Á£ */
	0xc1a4, 0xb8f7,	/* Á¤ */
	0xc1a5, 0xb8f8,	/* Á¥ */
	0xc1a6, 0xb941,	/* Á¦ */
	0xc1a7, 0xb942,	/* Á§ */
	0xc1a8, 0xb945,	/* Á¨ */
	0xc1a9, 0xb949,	/* Á© */
	0xc1aa, 0xb951,	/* Áª */
	0xc1ab, 0xb953,	/* Á« */
	0xc1ac, 0xb955,	/* Á¬ */
	0xc1ad, 0xb957,	/* Á­ */
	0xc1ae, 0xb961,	/* Á® */
	0xc1af, 0xb965,	/* Á¯ */
	0xc1b0, 0xb969,	/* Á° */
	0xc1b1, 0xb971,	/* Á± */
	0xc1b2, 0xb973,	/* Á² */
	0xc1b3, 0xb976,	/* Á³ */
	0xc1b4, 0xb977,	/* Á´ */
	0xc1b5, 0xb981,	/* Áµ */
	0xc1b6, 0xb9a1,	/* Á¶ */
	0xc1b7, 0xb9a2,	/* Á· */
	0xc1b8, 0xb9a5,	/* Á¸ */
	0xc1b9, 0xb9a9,	/* Á¹ */
	0xc1ba, 0xb9ab,	/* Áº */
	0xc1bb, 0xb9b1,	/* Á» */
	0xc1bc, 0xb9b3,	/* Á¼ */
	0xc1bd, 0xb9b5,	/* Á½ */
	0xc1be, 0xb9b7,	/* Á¾ */
	0xc1bf, 0xb9b8,	/* Á¿ */
	0xc1c0, 0xb9b9,	/* ÁÀ */
	0xc1c1, 0xb9bd,	/* ÁÁ */
	0xc1c2, 0xb9c1,	/* ÁÂ */
	0xc1c3, 0xb9c2,	/* ÁÃ */
	0xc1c4, 0xb9c9,	/* ÁÄ */
	0xc1c5, 0xb9d3,	/* ÁÅ */
	0xc1c6, 0xb9d5,	/* ÁÆ */
	0xc1c7, 0xb9d7,	/* ÁÇ */
	0xc1c8, 0xb9e1,	/* ÁÈ */
	0xc1c9, 0xb9f6,	/* ÁÉ */
	0xc1ca, 0xb9f7,	/* ÁÊ */
	0xc1cb, 0xba41,	/* ÁË */
	0xc1cc, 0xba45,	/* ÁÌ */
	0xc1cd, 0xba49,	/* ÁÍ */
	0xc1ce, 0xba51,	/* ÁÎ */
	0xc1cf, 0xba53,	/* ÁÏ */
	0xc1d0, 0xba55,	/* ÁÐ */
	0xc1d1, 0xba57,	/* ÁÑ */
	0xc1d2, 0xba61,	/* ÁÒ */
	0xc1d3, 0xba62,	/* ÁÓ */
	0xc1d4, 0xba65,	/* ÁÔ */
	0xc1d5, 0xba77,	/* ÁÕ */
	0xc1d6, 0xba81,	/* ÁÖ */
	0xc1d7, 0xba82,	/* Á× */
	0xc1d8, 0xba85,	/* ÁØ */
	0xc1d9, 0xba89,	/* ÁÙ */
	0xc1da, 0xba8a,	/* ÁÚ */
	0xc1db, 0xba8b,	/* ÁÛ */
	0xc1dc, 0xba91,	/* ÁÜ */
	0xc1dd, 0xba93,	/* ÁÝ */
	0xc1de, 0xba95,	/* ÁÞ */
	0xc1df, 0xba97,	/* Áß */
	0xc1e0, 0xbaa1,	/* Áà */
	0xc1e1, 0xbab6,	/* Áá */
	0xc1e2, 0xbac1,	/* Áâ */
	0xc1e3, 0xbae1,	/* Áã */
	0xc1e4, 0xbae2,	/* Áä */
	0xc1e5, 0xbae5,	/* Áå */
	0xc1e6, 0xbae9,	/* Áæ */
	0xc1e7, 0xbaf1,	/* Áç */
	0xc1e8, 0xbaf3,	/* Áè */
	0xc1e9, 0xbaf5,	/* Áé */
	0xc1ea, 0xbb41,	/* Áê */
	0xc1eb, 0xbb45,	/* Áë */
	0xc1ec, 0xbb49,	/* Áì */
	0xc1ed, 0xbb51,	/* Áí */
	0xc1ee, 0xbb61,	/* Áî */
	0xc1ef, 0xbb62,	/* Áï */
	0xc1f0, 0xbb65,	/* Áð */
	0xc1f1, 0xbb69,	/* Áñ */
	0xc1f2, 0xbb71,	/* Áò */
	0xc1f3, 0xbb73,	/* Áó */
	0xc1f4, 0xbb75,	/* Áô */
	0xc1f5, 0xbb77,	/* Áõ */
	0xc1f6, 0xbba1,	/* Áö */
	0xc1f7, 0xbba2,	/* Á÷ */
	0xc1f8, 0xbba5,	/* Áø */
	0xc1f9, 0xbba8,	/* Áù */
	0xc1fa, 0xbba9,	/* Áú */
	0xc1fb, 0xbbab,	/* Áû */
	0xc1fc, 0xbbb1,	/* Áü */
	0xc1fd, 0xbbb3,	/* Áý */
	0xc1fe, 0xbbb5,	/* Áþ */
	0xc2a1, 0xbbb7,	/* Â¡ */
	0xc2a2, 0xbbb8,	/* Â¢ */
	0xc2a3, 0xbbbb,	/* Â£ */
	0xc2a4, 0xbbbc,	/* Â¤ */
	0xc2a5, 0xbc61,	/* Â¥ */
	0xc2a6, 0xbc62,	/* Â¦ */
	0xc2a7, 0xbc65,	/* Â§ */
	0xc2a8, 0xbc67,	/* Â¨ */
	0xc2a9, 0xbc69,	/* Â© */
	0xc2aa, 0xbc6c,	/* Âª */
	0xc2ab, 0xbc71,	/* Â« */
	0xc2ac, 0xbc73,	/* Â¬ */
	0xc2ad, 0xbc75,	/* Â­ */
	0xc2ae, 0xbc76,	/* Â® */
	0xc2af, 0xbc77,	/* Â¯ */
	0xc2b0, 0xbc81,	/* Â° */
	0xc2b1, 0xbc82,	/* Â± */
	0xc2b2, 0xbc85,	/* Â² */
	0xc2b3, 0xbc89,	/* Â³ */
	0xc2b4, 0xbc91,	/* Â´ */
	0xc2b5, 0xbc93,	/* Âµ */
	0xc2b6, 0xbc95,	/* Â¶ */
	0xc2b7, 0xbc96,	/* Â· */
	0xc2b8, 0xbc97,	/* Â¸ */
	0xc2b9, 0xbca1,	/* Â¹ */
	0xc2ba, 0xbca5,	/* Âº */
	0xc2bb, 0xbcb7,	/* Â» */
	0xc2bc, 0xbce1,	/* Â¼ */
	0xc2bd, 0xbce2,	/* Â½ */
	0xc2be, 0xbce5,	/* Â¾ */
	0xc2bf, 0xbce9,	/* Â¿ */
	0xc2c0, 0xbcf1,	/* ÂÀ */
	0xc2c1, 0xbcf3,	/* ÂÁ */
	0xc2c2, 0xbcf5,	/* ÂÂ */
	0xc2c3, 0xbcf6,	/* ÂÃ */
	0xc2c4, 0xbcf7,	/* ÂÄ */
	0xc2c5, 0xbd41,	/* ÂÅ */
	0xc2c6, 0xbd57,	/* ÂÆ */
	0xc2c7, 0xbd61,	/* ÂÇ */
	0xc2c8, 0xbd76,	/* ÂÈ */
	0xc2c9, 0xbda1,	/* ÂÉ */
	0xc2ca, 0xbda2,	/* ÂÊ */
	0xc2cb, 0xbda5,	/* ÂË */
	0xc2cc, 0xbda9,	/* ÂÌ */
	0xc2cd, 0xbdb1,	/* ÂÍ */
	0xc2ce, 0xbdb3,	/* ÂÎ */
	0xc2cf, 0xbdb5,	/* ÂÏ */
	0xc2d0, 0xbdb7,	/* ÂÐ */
	0xc2d1, 0xbdb9,	/* ÂÑ */
	0xc2d2, 0xbdc1,	/* ÂÒ */
	0xc2d3, 0xbdc2,	/* ÂÓ */
	0xc2d4, 0xbdc9,	/* ÂÔ */
	0xc2d5, 0xbdd6,	/* ÂÕ */
	0xc2d6, 0xbde1,	/* ÂÖ */
	0xc2d7, 0xbdf6,	/* Â× */
	0xc2d8, 0xbe41,	/* ÂØ */
	0xc2d9, 0xbe45,	/* ÂÙ */
	0xc2da, 0xbe49,	/* ÂÚ */
	0xc2db, 0xbe51,	/* ÂÛ */
	0xc2dc, 0xbe53,	/* ÂÜ */
	0xc2dd, 0xbe77,	/* ÂÝ */
	0xc2de, 0xbe81,	/* ÂÞ */
	0xc2df, 0xbe82,	/* Âß */
	0xc2e0, 0xbe85,	/* Âà */
	0xc2e1, 0xbe89,	/* Âá */
	0xc2e2, 0xbe91,	/* Ââ */
	0xc2e3, 0xbe93,	/* Âã */
	0xc2e4, 0xbe97,	/* Âä */
	0xc2e5, 0xbea1,	/* Âå */
	0xc2e6, 0xbeb6,	/* Âæ */
	0xc2e7, 0xbeb7,	/* Âç */
	0xc2e8, 0xbee1,	/* Âè */
	0xc2e9, 0xbf41,	/* Âé */
	0xc2ea, 0xbf61,	/* Âê */
	0xc2eb, 0xbf71,	/* Âë */
	0xc2ec, 0xbf75,	/* Âì */
	0xc2ed, 0xbf77,	/* Âí */
	0xc2ee, 0xbfa1,	/* Âî */
	0xc2ef, 0xbfa2,	/* Âï */
	0xc2f0, 0xbfa5,	/* Âð */
	0xc2f1, 0xbfa9,	/* Âñ */
	0xc2f2, 0xbfb1,	/* Âò */
	0xc2f3, 0xbfb3,	/* Âó */
	0xc2f4, 0xbfb7,	/* Âô */
	0xc2f5, 0xbfb8,	/* Âõ */
	0xc2f6, 0xbfbd,	/* Âö */
	0xc2f7, 0xc061,	/* Â÷ */
	0xc2f8, 0xc062,	/* Âø */
	0xc2f9, 0xc065,	/* Âù */
	0xc2fa, 0xc067,	/* Âú */
	0xc2fb, 0xc069,	/* Âû */
	0xc2fc, 0xc071,	/* Âü */
	0xc2fd, 0xc073,	/* Âý */
	0xc2fe, 0xc075,	/* Âþ */
	0xc3a1, 0xc076,	/* Ã¡ */
	0xc3a2, 0xc077,	/* Ã¢ */
	0xc3a3, 0xc078,	/* Ã£ */
	0xc3a4, 0xc081,	/* Ã¤ */
	0xc3a5, 0xc082,	/* Ã¥ */
	0xc3a6, 0xc085,	/* Ã¦ */
	0xc3a7, 0xc089,	/* Ã§ */
	0xc3a8, 0xc091,	/* Ã¨ */
	0xc3a9, 0xc093,	/* Ã© */
	0xc3aa, 0xc095,	/* Ãª */
	0xc3ab, 0xc096,	/* Ã« */
	0xc3ac, 0xc097,	/* Ã¬ */
	0xc3ad, 0xc0a1,	/* Ã­ */
	0xc3ae, 0xc0a5,	/* Ã® */
	0xc3af, 0xc0a7,	/* Ã¯ */
	0xc3b0, 0xc0a9,	/* Ã° */
	0xc3b1, 0xc0b1,	/* Ã± */
	0xc3b2, 0xc0b7,	/* Ã² */
	0xc3b3, 0xc0e1,	/* Ã³ */
	0xc3b4, 0xc0e2,	/* Ã´ */
	0xc3b5, 0xc0e5,	/* Ãµ */
	0xc3b6, 0xc0e9,	/* Ã¶ */
	0xc3b7, 0xc0f1,	/* Ã· */
	0xc3b8, 0xc0f3,	/* Ã¸ */
	0xc3b9, 0xc0f5,	/* Ã¹ */
	0xc3ba, 0xc0f6,	/* Ãº */
	0xc3bb, 0xc0f7,	/* Ã» */
	0xc3bc, 0xc141,	/* Ã¼ */
	0xc3bd, 0xc142,	/* Ã½ */
	0xc3be, 0xc145,	/* Ã¾ */
	0xc3bf, 0xc149,	/* Ã¿ */
	0xc3c0, 0xc151,	/* ÃÀ */
	0xc3c1, 0xc153,	/* ÃÁ */
	0xc3c2, 0xc155,	/* ÃÂ */
	0xc3c3, 0xc157,	/* ÃÃ */
	0xc3c4, 0xc161,	/* ÃÄ */
	0xc3c5, 0xc165,	/* ÃÅ */
	0xc3c6, 0xc176,	/* ÃÆ */
	0xc3c7, 0xc181,	/* ÃÇ */
	0xc3c8, 0xc185,	/* ÃÈ */
	0xc3c9, 0xc197,	/* ÃÉ */
	0xc3ca, 0xc1a1,	/* ÃÊ */
	0xc3cb, 0xc1a2,	/* ÃË */
	0xc3cc, 0xc1a5,	/* ÃÌ */
	0xc3cd, 0xc1a9,	/* ÃÍ */
	0xc3ce, 0xc1b1,	/* ÃÎ */
	0xc3cf, 0xc1b3,	/* ÃÏ */
	0xc3d0, 0xc1b5,	/* ÃÐ */
	0xc3d1, 0xc1b7,	/* ÃÑ */
	0xc3d2, 0xc1c1,	/* ÃÒ */
	0xc3d3, 0xc1c5,	/* ÃÓ */
	0xc3d4, 0xc1c9,	/* ÃÔ */
	0xc3d5, 0xc1d7,	/* ÃÕ */
	0xc3d6, 0xc241,	/* ÃÖ */
	0xc3d7, 0xc245,	/* Ã× */
	0xc3d8, 0xc249,	/* ÃØ */
	0xc3d9, 0xc251,	/* ÃÙ */
	0xc3da, 0xc253,	/* ÃÚ */
	0xc3db, 0xc255,	/* ÃÛ */
	0xc3dc, 0xc257,	/* ÃÜ */
	0xc3dd, 0xc261,	/* ÃÝ */
	0xc3de, 0xc271,	/* ÃÞ */
	0xc3df, 0xc281,	/* Ãß */
	0xc3e0, 0xc282,	/* Ãà */
	0xc3e1, 0xc285,	/* Ãá */
	0xc3e2, 0xc289,	/* Ãâ */
	0xc3e3, 0xc291,	/* Ãã */
	0xc3e4, 0xc293,	/* Ãä */
	0xc3e5, 0xc295,	/* Ãå */
	0xc3e6, 0xc297,	/* Ãæ */
	0xc3e7, 0xc2a1,	/* Ãç */
	0xc3e8, 0xc2b6,	/* Ãè */
	0xc3e9, 0xc2c1,	/* Ãé */
	0xc3ea, 0xc2c5,	/* Ãê */
	0xc3eb, 0xc2e1,	/* Ãë */
	0xc3ec, 0xc2e5,	/* Ãì */
	0xc3ed, 0xc2e9,	/* Ãí */
	0xc3ee, 0xc2f1,	/* Ãî */
	0xc3ef, 0xc2f3,	/* Ãï */
	0xc3f0, 0xc2f5,	/* Ãð */
	0xc3f1, 0xc2f7,	/* Ãñ */
	0xc3f2, 0xc341,	/* Ãò */
	0xc3f3, 0xc345,	/* Ãó */
	0xc3f4, 0xc349,	/* Ãô */
	0xc3f5, 0xc351,	/* Ãõ */
	0xc3f6, 0xc357,	/* Ãö */
	0xc3f7, 0xc361,	/* Ã÷ */
	0xc3f8, 0xc362,	/* Ãø */
	0xc3f9, 0xc365,	/* Ãù */
	0xc3fa, 0xc369,	/* Ãú */
	0xc3fb, 0xc371,	/* Ãû */
	0xc3fc, 0xc373,	/* Ãü */
	0xc3fd, 0xc375,	/* Ãý */
	0xc3fe, 0xc377,	/* Ãþ */
	0xc4a1, 0xc3a1,	/* Ä¡ */
	0xc4a2, 0xc3a2,	/* Ä¢ */
	0xc4a3, 0xc3a5,	/* Ä£ */
	0xc4a4, 0xc3a8,	/* Ä¤ */
	0xc4a5, 0xc3a9,	/* Ä¥ */
	0xc4a6, 0xc3aa,	/* Ä¦ */
	0xc4a7, 0xc3b1,	/* Ä§ */
	0xc4a8, 0xc3b3,	/* Ä¨ */
	0xc4a9, 0xc3b5,	/* Ä© */
	0xc4aa, 0xc3b7,	/* Äª */
	0xc4ab, 0xc461,	/* Ä« */
	0xc4ac, 0xc462,	/* Ä¬ */
	0xc4ad, 0xc465,	/* Ä­ */
	0xc4ae, 0xc469,	/* Ä® */
	0xc4af, 0xc471,	/* Ä¯ */
	0xc4b0, 0xc473,	/* Ä° */
	0xc4b1, 0xc475,	/* Ä± */
	0xc4b2, 0xc477,	/* Ä² */
	0xc4b3, 0xc481,	/* Ä³ */
	0xc4b4, 0xc482,	/* Ä´ */
	0xc4b5, 0xc485,	/* Äµ */
	0xc4b6, 0xc489,	/* Ä¶ */
	0xc4b7, 0xc491,	/* Ä· */
	0xc4b8, 0xc493,	/* Ä¸ */
	0xc4b9, 0xc495,	/* Ä¹ */
	0xc4ba, 0xc496,	/* Äº */
	0xc4bb, 0xc497,	/* Ä» */
	0xc4bc, 0xc4a1,	/* Ä¼ */
	0xc4bd, 0xc4a2,	/* Ä½ */
	0xc4be, 0xc4b7,	/* Ä¾ */
	0xc4bf, 0xc4e1,	/* Ä¿ */
	0xc4c0, 0xc4e2,	/* ÄÀ */
	0xc4c1, 0xc4e5,	/* ÄÁ */
	0xc4c2, 0xc4e8,	/* ÄÂ */
	0xc4c3, 0xc4e9,	/* ÄÃ */
	0xc4c4, 0xc4f1,	/* ÄÄ */
	0xc4c5, 0xc4f3,	/* ÄÅ */
	0xc4c6, 0xc4f5,	/* ÄÆ */
	0xc4c7, 0xc4f6,	/* ÄÇ */
	0xc4c8, 0xc4f7,	/* ÄÈ */
	0xc4c9, 0xc541,	/* ÄÉ */
	0xc4ca, 0xc542,	/* ÄÊ */
	0xc4cb, 0xc545,	/* ÄË */
	0xc4cc, 0xc549,	/* ÄÌ */
	0xc4cd, 0xc551,	/* ÄÍ */
	0xc4ce, 0xc553,	/* ÄÎ */
	0xc4cf, 0xc555,	/* ÄÏ */
	0xc4d0, 0xc557,	/* ÄÐ */
	0xc4d1, 0xc561,	/* ÄÑ */
	0xc4d2, 0xc565,	/* ÄÒ */
	0xc4d3, 0xc569,	/* ÄÓ */
	0xc4d4, 0xc571,	/* ÄÔ */
	0xc4d5, 0xc573,	/* ÄÕ */
	0xc4d6, 0xc575,	/* ÄÖ */
	0xc4d7, 0xc576,	/* Ä× */
	0xc4d8, 0xc577,	/* ÄØ */
	0xc4d9, 0xc581,	/* ÄÙ */
	0xc4da, 0xc5a1,	/* ÄÚ */
	0xc4db, 0xc5a2,	/* ÄÛ */
	0xc4dc, 0xc5a5,	/* ÄÜ */
	0xc4dd, 0xc5a9,	/* ÄÝ */
	0xc4de, 0xc5b1,	/* ÄÞ */
	0xc4df, 0xc5b3,	/* Äß */
	0xc4e0, 0xc5b5,	/* Äà */
	0xc4e1, 0xc5b7,	/* Äá */
	0xc4e2, 0xc5c1,	/* Äâ */
	0xc4e3, 0xc5c2,	/* Äã */
	0xc4e4, 0xc5c5,	/* Ää */
	0xc4e5, 0xc5c9,	/* Äå */
	0xc4e6, 0xc5d1,	/* Äæ */
	0xc4e7, 0xc5d7,	/* Äç */
	0xc4e8, 0xc5e1,	/* Äè */
	0xc4e9, 0xc5f7,	/* Äé */
	0xc4ea, 0xc641,	/* Äê */
	0xc4eb, 0xc649,	/* Äë */
	0xc4ec, 0xc661,	/* Äì */
	0xc4ed, 0xc681,	/* Äí */
	0xc4ee, 0xc682,	/* Äî */
	0xc4ef, 0xc685,	/* Äï */
	0xc4f0, 0xc689,	/* Äð */
	0xc4f1, 0xc691,	/* Äñ */
	0xc4f2, 0xc693,	/* Äò */
	0xc4f3, 0xc695,	/* Äó */
	0xc4f4, 0xc697,	/* Äô */
	0xc4f5, 0xc6a1,	/* Äõ */
	0xc4f6, 0xc6a5,	/* Äö */
	0xc4f7, 0xc6a9,	/* Ä÷ */
	0xc4f8, 0xc6b7,	/* Äø */
	0xc4f9, 0xc6c1,	/* Äù */
	0xc4fa, 0xc6d7,	/* Äú */
	0xc4fb, 0xc6e1,	/* Äû */
	0xc4fc, 0xc6e2,	/* Äü */
	0xc4fd, 0xc6e5,	/* Äý */
	0xc4fe, 0xc6e9,	/* Äþ */
	0xc5a1, 0xc6f1,	/* Å¡ */
	0xc5a2, 0xc6f3,	/* Å¢ */
	0xc5a3, 0xc6f5,	/* Å£ */
	0xc5a4, 0xc6f7,	/* Å¤ */
	0xc5a5, 0xc741,	/* Å¥ */
	0xc5a6, 0xc745,	/* Å¦ */
	0xc5a7, 0xc749,	/* Å§ */
	0xc5a8, 0xc751,	/* Å¨ */
	0xc5a9, 0xc761,	/* Å© */
	0xc5aa, 0xc762,	/* Åª */
	0xc5ab, 0xc765,	/* Å« */
	0xc5ac, 0xc769,	/* Å¬ */
	0xc5ad, 0xc771,	/* Å­ */
	0xc5ae, 0xc773,	/* Å® */
	0xc5af, 0xc777,	/* Å¯ */
	0xc5b0, 0xc7a1,	/* Å° */
	0xc5b1, 0xc7a2,	/* Å± */
	0xc5b2, 0xc7a5,	/* Å² */
	0xc5b3, 0xc7a9,	/* Å³ */
	0xc5b4, 0xc7b1,	/* Å´ */
	0xc5b5, 0xc7b3,	/* Åµ */
	0xc5b6, 0xc7b5,	/* Å¶ */
	0xc5b7, 0xc7b7,	/* Å· */
	0xc5b8, 0xc861,	/* Å¸ */
	0xc5b9, 0xc862,	/* Å¹ */
	0xc5ba, 0xc865,	/* Åº */
	0xc5bb, 0xc869,	/* Å» */
	0xc5bc, 0xc86a,	/* Å¼ */
	0xc5bd, 0xc871,	/* Å½ */
	0xc5be, 0xc873,	/* Å¾ */
	0xc5bf, 0xc875,	/* Å¿ */
	0xc5c0, 0xc876,	/* ÅÀ */
	0xc5c1, 0xc877,	/* ÅÁ */
	0xc5c2, 0xc881,	/* ÅÂ */
	0xc5c3, 0xc882,	/* ÅÃ */
	0xc5c4, 0xc885,	/* ÅÄ */
	0xc5c5, 0xc889,	/* ÅÅ */
	0xc5c6, 0xc891,	/* ÅÆ */
	0xc5c7, 0xc893,	/* ÅÇ */
	0xc5c8, 0xc895,	/* ÅÈ */
	0xc5c9, 0xc896,	/* ÅÉ */
	0xc5ca, 0xc897,	/* ÅÊ */
	0xc5cb, 0xc8a1,	/* ÅË */
	0xc5cc, 0xc8b7,	/* ÅÌ */
	0xc5cd, 0xc8e1,	/* ÅÍ */
	0xc5ce, 0xc8e2,	/* ÅÎ */
	0xc5cf, 0xc8e5,	/* ÅÏ */
	0xc5d0, 0xc8e9,	/* ÅÐ */
	0xc5d1, 0xc8eb,	/* ÅÑ */
	0xc5d2, 0xc8f1,	/* ÅÒ */
	0xc5d3, 0xc8f3,	/* ÅÓ */
	0xc5d4, 0xc8f5,	/* ÅÔ */
	0xc5d5, 0xc8f6,	/* ÅÕ */
	0xc5d6, 0xc8f7,	/* ÅÖ */
	0xc5d7, 0xc941,	/* Å× */
	0xc5d8, 0xc942,	/* ÅØ */
	0xc5d9, 0xc945,	/* ÅÙ */
	0xc5da, 0xc949,	/* ÅÚ */
	0xc5db, 0xc951,	/* ÅÛ */
	0xc5dc, 0xc953,	/* ÅÜ */
	0xc5dd, 0xc955,	/* ÅÝ */
	0xc5de, 0xc957,	/* ÅÞ */
	0xc5df, 0xc961,	/* Åß */
	0xc5e0, 0xc965,	/* Åà */
	0xc5e1, 0xc976,	/* Åá */
	0xc5e2, 0xc981,	/* Åâ */
	0xc5e3, 0xc985,	/* Åã */
	0xc5e4, 0xc9a1,	/* Åä */
	0xc5e5, 0xc9a2,	/* Åå */
	0xc5e6, 0xc9a5,	/* Åæ */
	0xc5e7, 0xc9a9,	/* Åç */
	0xc5e8, 0xc9b1,	/* Åè */
	0xc5e9, 0xc9b3,	/* Åé */
	0xc5ea, 0xc9b5,	/* Åê */
	0xc5eb, 0xc9b7,	/* Åë */
	0xc5ec, 0xc9bc,	/* Åì */
	0xc5ed, 0xc9c1,	/* Åí */
	0xc5ee, 0xc9c5,	/* Åî */
	0xc5ef, 0xc9e1,	/* Åï */
	0xc5f0, 0xca41,	/* Åð */
	0xc5f1, 0xca45,	/* Åñ */
	0xc5f2, 0xca55,	/* Åò */
	0xc5f3, 0xca57,	/* Åó */
	0xc5f4, 0xca61,	/* Åô */
	0xc5f5, 0xca81,	/* Åõ */
	0xc5f6, 0xca82,	/* Åö */
	0xc5f7, 0xca85,	/* Å÷ */
	0xc5f8, 0xca89,	/* Åø */
	0xc5f9, 0xca91,	/* Åù */
	0xc5fa, 0xca93,	/* Åú */
	0xc5fb, 0xca95,	/* Åû */
	0xc5fc, 0xca97,	/* Åü */
	0xc5fd, 0xcaa1,	/* Åý */
	0xc5fe, 0xcab6,	/* Åþ */
	0xc6a1, 0xcac1,	/* Æ¡ */
	0xc6a2, 0xcae1,	/* Æ¢ */
	0xc6a3, 0xcae2,	/* Æ£ */
	0xc6a4, 0xcae5,	/* Æ¤ */
	0xc6a5, 0xcae9,	/* Æ¥ */
	0xc6a6, 0xcaf1,	/* Æ¦ */
	0xc6a7, 0xcaf3,	/* Æ§ */
	0xc6a8, 0xcaf7,	/* Æ¨ */
	0xc6a9, 0xcb41,	/* Æ© */
	0xc6aa, 0xcb45,	/* Æª */
	0xc6ab, 0xcb49,	/* Æ« */
	0xc6ac, 0xcb51,	/* Æ¬ */
	0xc6ad, 0xcb57,	/* Æ­ */
	0xc6ae, 0xcb61,	/* Æ® */
	0xc6af, 0xcb62,	/* Æ¯ */
	0xc6b0, 0xcb65,	/* Æ° */
	0xc6b1, 0xcb68,	/* Æ± */
	0xc6b2, 0xcb69,	/* Æ² */
	0xc6b3, 0xcb6b,	/* Æ³ */
	0xc6b4, 0xcb71,	/* Æ´ */
	0xc6b5, 0xcb73,	/* Æµ */
	0xc6b6, 0xcb75,	/* Æ¶ */
	0xc6b7, 0xcb81,	/* Æ· */
	0xc6b8, 0xcb85,	/* Æ¸ */
	0xc6b9, 0xcb89,	/* Æ¹ */
	0xc6ba, 0xcb91,	/* Æº */
	0xc6bb, 0xcb93,	/* Æ» */
	0xc6bc, 0xcba1,	/* Æ¼ */
	0xc6bd, 0xcba2,	/* Æ½ */
	0xc6be, 0xcba5,	/* Æ¾ */
	0xc6bf, 0xcba9,	/* Æ¿ */
	0xc6c0, 0xcbb1,	/* ÆÀ */
	0xc6c1, 0xcbb3,	/* ÆÁ */
	0xc6c2, 0xcbb5,	/* ÆÂ */
	0xc6c3, 0xcbb7,	/* ÆÃ */
	0xc6c4, 0xcc61,	/* ÆÄ */
	0xc6c5, 0xcc62,	/* ÆÅ */
	0xc6c6, 0xcc63,	/* ÆÆ */
	0xc6c7, 0xcc65,	/* ÆÇ */
	0xc6c8, 0xcc69,	/* ÆÈ */
	0xc6c9, 0xcc6b,	/* ÆÉ */
	0xc6ca, 0xcc71,	/* ÆÊ */
	0xc6cb, 0xcc73,	/* ÆË */
	0xc6cc, 0xcc75,	/* ÆÌ */
	0xc6cd, 0xcc76,	/* ÆÍ */
	0xc6ce, 0xcc77,	/* ÆÎ */
	0xc6cf, 0xcc7b,	/* ÆÏ */
	0xc6d0, 0xcc81,	/* ÆÐ */
	0xc6d1, 0xcc82,	/* ÆÑ */
	0xc6d2, 0xcc85,	/* ÆÒ */
	0xc6d3, 0xcc89,	/* ÆÓ */
	0xc6d4, 0xcc91,	/* ÆÔ */
	0xc6d5, 0xcc93,	/* ÆÕ */
	0xc6d6, 0xcc95,	/* ÆÖ */
	0xc6d7, 0xcc96,	/* Æ× */
	0xc6d8, 0xcc97,	/* ÆØ */
	0xc6d9, 0xcca1,	/* ÆÙ */
	0xc6da, 0xcca2,	/* ÆÚ */
	0xc6db, 0xcce1,	/* ÆÛ */
	0xc6dc, 0xcce2,	/* ÆÜ */
	0xc6dd, 0xcce5,	/* ÆÝ */
	0xc6de, 0xcce9,	/* ÆÞ */
	0xc6df, 0xccf1,	/* Æß */
	0xc6e0, 0xccf3,	/* Æà */
	0xc6e1, 0xccf5,	/* Æá */
	0xc6e2, 0xccf6,	/* Æâ */
	0xc6e3, 0xccf7,	/* Æã */
	0xc6e4, 0xcd41,	/* Æä */
	0xc6e5, 0xcd42,	/* Æå */
	0xc6e6, 0xcd45,	/* Ææ */
	0xc6e7, 0xcd49,	/* Æç */
	0xc6e8, 0xcd51,	/* Æè */
	0xc6e9, 0xcd53,	/* Æé */
	0xc6ea, 0xcd55,	/* Æê */
	0xc6eb, 0xcd57,	/* Æë */
	0xc6ec, 0xcd61,	/* Æì */
	0xc6ed, 0xcd65,	/* Æí */
	0xc6ee, 0xcd69,	/* Æî */
	0xc6ef, 0xcd71,	/* Æï */
	0xc6f0, 0xcd73,	/* Æð */
	0xc6f1, 0xcd76,	/* Æñ */
	0xc6f2, 0xcd77,	/* Æò */
	0xc6f3, 0xcd81,	/* Æó */
	0xc6f4, 0xcd89,	/* Æô */
	0xc6f5, 0xcd93,	/* Æõ */
	0xc6f6, 0xcd95,	/* Æö */
	0xc6f7, 0xcda1,	/* Æ÷ */
	0xc6f8, 0xcda2,	/* Æø */
	0xc6f9, 0xcda5,	/* Æù */
	0xc6fa, 0xcda9,	/* Æú */
	0xc6fb, 0xcdb1,	/* Æû */
	0xc6fc, 0xcdb3,	/* Æü */
	0xc6fd, 0xcdb5,	/* Æý */
	0xc6fe, 0xcdb7,	/* Æþ */
	0xc7a1, 0xcdc1,	/* Ç¡ */
	0xc7a2, 0xcdd7,	/* Ç¢ */
	0xc7a3, 0xce41,	/* Ç£ */
	0xc7a4, 0xce45,	/* Ç¤ */
	0xc7a5, 0xce61,	/* Ç¥ */
	0xc7a6, 0xce65,	/* Ç¦ */
	0xc7a7, 0xce69,	/* Ç§ */
	0xc7a8, 0xce73,	/* Ç¨ */
	0xc7a9, 0xce75,	/* Ç© */
	0xc7aa, 0xce81,	/* Çª */
	0xc7ab, 0xce82,	/* Ç« */
	0xc7ac, 0xce85,	/* Ç¬ */
	0xc7ad, 0xce88,	/* Ç­ */
	0xc7ae, 0xce89,	/* Ç® */
	0xc7af, 0xce8b,	/* Ç¯ */
	0xc7b0, 0xce91,	/* Ç° */
	0xc7b1, 0xce93,	/* Ç± */
	0xc7b2, 0xce95,	/* Ç² */
	0xc7b3, 0xce97,	/* Ç³ */
	0xc7b4, 0xcea1,	/* Ç´ */
	0xc7b5, 0xceb7,	/* Çµ */
	0xc7b6, 0xcee1,	/* Ç¶ */
	0xc7b7, 0xcee5,	/* Ç· */
	0xc7b8, 0xcee9,	/* Ç¸ */
	0xc7b9, 0xcef1,	/* Ç¹ */
	0xc7ba, 0xcef5,	/* Çº */
	0xc7bb, 0xcf41,	/* Ç» */
	0xc7bc, 0xcf45,	/* Ç¼ */
	0xc7bd, 0xcf49,	/* Ç½ */
	0xc7be, 0xcf51,	/* Ç¾ */
	0xc7bf, 0xcf55,	/* Ç¿ */
	0xc7c0, 0xcf57,	/* ÇÀ */
	0xc7c1, 0xcf61,	/* ÇÁ */
	0xc7c2, 0xcf65,	/* ÇÂ */
	0xc7c3, 0xcf69,	/* ÇÃ */
	0xc7c4, 0xcf71,	/* ÇÄ */
	0xc7c5, 0xcf73,	/* ÇÅ */
	0xc7c6, 0xcf75,	/* ÇÆ */
	0xc7c7, 0xcfa1,	/* ÇÇ */
	0xc7c8, 0xcfa2,	/* ÇÈ */
	0xc7c9, 0xcfa5,	/* ÇÉ */
	0xc7ca, 0xcfa9,	/* ÇÊ */
	0xc7cb, 0xcfb1,	/* ÇË */
	0xc7cc, 0xcfb3,	/* ÇÌ */
	0xc7cd, 0xcfb5,	/* ÇÍ */
	0xc7ce, 0xcfb7,	/* ÇÎ */
	0xc7cf, 0xd061,	/* ÇÏ */
	0xc7d0, 0xd062,	/* ÇÐ */
	0xc7d1, 0xd065,	/* ÇÑ */
	0xc7d2, 0xd069,	/* ÇÒ */
	0xc7d3, 0xd06e,	/* ÇÓ */
	0xc7d4, 0xd071,	/* ÇÔ */
	0xc7d5, 0xd073,	/* ÇÕ */
	0xc7d6, 0xd075,	/* ÇÖ */
	0xc7d7, 0xd077,	/* Ç× */
	0xc7d8, 0xd081,	/* ÇØ */
	0xc7d9, 0xd082,	/* ÇÙ */
	0xc7da, 0xd085,	/* ÇÚ */
	0xc7db, 0xd089,	/* ÇÛ */
	0xc7dc, 0xd091,	/* ÇÜ */
	0xc7dd, 0xd093,	/* ÇÝ */
	0xc7de, 0xd095,	/* ÇÞ */
	0xc7df, 0xd096,	/* Çß */
	0xc7e0, 0xd097,	/* Çà */
	0xc7e1, 0xd0a1,	/* Çá */
	0xc7e2, 0xd0b7,	/* Çâ */
	0xc7e3, 0xd0e1,	/* Çã */
	0xc7e4, 0xd0e2,	/* Çä */
	0xc7e5, 0xd0e5,	/* Çå */
	0xc7e6, 0xd0e9,	/* Çæ */
	0xc7e7, 0xd0eb,	/* Çç */
	0xc7e8, 0xd0f1,	/* Çè */
	0xc7e9, 0xd0f3,	/* Çé */
	0xc7ea, 0xd0f5,	/* Çê */
	0xc7eb, 0xd0f7,	/* Çë */
	0xc7ec, 0xd141,	/* Çì */
	0xc7ed, 0xd142,	/* Çí */
	0xc7ee, 0xd145,	/* Çî */
	0xc7ef, 0xd149,	/* Çï */
	0xc7f0, 0xd151,	/* Çð */
	0xc7f1, 0xd153,	/* Çñ */
	0xc7f2, 0xd155,	/* Çò */
	0xc7f3, 0xd157,	/* Çó */
	0xc7f4, 0xd161,	/* Çô */
	0xc7f5, 0xd162,	/* Çõ */
	0xc7f6, 0xd165,	/* Çö */
	0xc7f7, 0xd169,	/* Ç÷ */
	0xc7f8, 0xd171,	/* Çø */
	0xc7f9, 0xd173,	/* Çù */
	0xc7fa, 0xd175,	/* Çú */
	0xc7fb, 0xd176,	/* Çû */
	0xc7fc, 0xd177,	/* Çü */
	0xc7fd, 0xd181,	/* Çý */
	0xc7fe, 0xd185,	/* Çþ */
	0xc8a1, 0xd189,	/* È¡ */
	0xc8a2, 0xd193,	/* È¢ */
	0xc8a3, 0xd1a1,	/* È£ */
	0xc8a4, 0xd1a2,	/* È¤ */
	0xc8a5, 0xd1a5,	/* È¥ */
	0xc8a6, 0xd1a9,	/* È¦ */
	0xc8a7, 0xd1ae,	/* È§ */
	0xc8a8, 0xd1b1,	/* È¨ */
	0xc8a9, 0xd1b3,	/* È© */
	0xc8aa, 0xd1b5,	/* Èª */
	0xc8ab, 0xd1b7,	/* È« */
	0xc8ac, 0xd1bb,	/* È¬ */
	0xc8ad, 0xd1c1,	/* È­ */
	0xc8ae, 0xd1c2,	/* È® */
	0xc8af, 0xd1c5,	/* È¯ */
	0xc8b0, 0xd1c9,	/* È° */
	0xc8b1, 0xd1d5,	/* È± */
	0xc8b2, 0xd1d7,	/* È² */
	0xc8b3, 0xd1e1,	/* È³ */
	0xc8b4, 0xd1e2,	/* È´ */
	0xc8b5, 0xd1e5,	/* Èµ */
	0xc8b6, 0xd1f5,	/* È¶ */
	0xc8b7, 0xd1f7,	/* È· */
	0xc8b8, 0xd241,	/* È¸ */
	0xc8b9, 0xd242,	/* È¹ */
	0xc8ba, 0xd245,	/* Èº */
	0xc8bb, 0xd249,	/* È» */
	0xc8bc, 0xd253,	/* È¼ */
	0xc8bd, 0xd255,	/* È½ */
	0xc8be, 0xd257,	/* È¾ */
	0xc8bf, 0xd261,	/* È¿ */
	0xc8c0, 0xd265,	/* ÈÀ */
	0xc8c1, 0xd269,	/* ÈÁ */
	0xc8c2, 0xd273,	/* ÈÂ */
	0xc8c3, 0xd275,	/* ÈÃ */
	0xc8c4, 0xd281,	/* ÈÄ */
	0xc8c5, 0xd282,	/* ÈÅ */
	0xc8c6, 0xd285,	/* ÈÆ */
	0xc8c7, 0xd289,	/* ÈÇ */
	0xc8c8, 0xd28e,	/* ÈÈ */
	0xc8c9, 0xd291,	/* ÈÉ */
	0xc8ca, 0xd295,	/* ÈÊ */
	0xc8cb, 0xd297,	/* ÈË */
	0xc8cc, 0xd2a1,	/* ÈÌ */
	0xc8cd, 0xd2a5,	/* ÈÍ */
	0xc8ce, 0xd2a9,	/* ÈÎ */
	0xc8cf, 0xd2b1,	/* ÈÏ */
	0xc8d0, 0xd2b7,	/* ÈÐ */
	0xc8d1, 0xd2c1,	/* ÈÑ */
	0xc8d2, 0xd2c2,	/* ÈÒ */
	0xc8d3, 0xd2c5,	/* ÈÓ */
	0xc8d4, 0xd2c9,	/* ÈÔ */
	0xc8d5, 0xd2d7,	/* ÈÕ */
	0xc8d6, 0xd2e1,	/* ÈÖ */
	0xc8d7, 0xd2e2,	/* È× */
	0xc8d8, 0xd2e5,	/* ÈØ */
	0xc8d9, 0xd2e9,	/* ÈÙ */
	0xc8da, 0xd2f1,	/* ÈÚ */
	0xc8db, 0xd2f3,	/* ÈÛ */
	0xc8dc, 0xd2f5,	/* ÈÜ */
	0xc8dd, 0xd2f7,	/* ÈÝ */
	0xc8de, 0xd341,	/* ÈÞ */
	0xc8df, 0xd342,	/* Èß */
	0xc8e0, 0xd345,	/* Èà */
	0xc8e1, 0xd349,	/* Èá */
	0xc8e2, 0xd351,	/* Èâ */
	0xc8e3, 0xd355,	/* Èã */
	0xc8e4, 0xd357,	/* Èä */
	0xc8e5, 0xd361,	/* Èå */
	0xc8e6, 0xd362,	/* Èæ */
	0xc8e7, 0xd365,	/* Èç */
	0xc8e8, 0xd367,	/* Èè */
	0xc8e9, 0xd368,	/* Èé */
	0xc8ea, 0xd369,	/* Èê */
	0xc8eb, 0xd36a,	/* Èë */
	0xc8ec, 0xd371,	/* Èì */
	0xc8ed, 0xd373,	/* Èí */
	0xc8ee, 0xd375,	/* Èî */
	0xc8ef, 0xd377,	/* Èï */
	0xc8f0, 0xd37b,	/* Èð */
	0xc8f1, 0xd381,	/* Èñ */
	0xc8f2, 0xd385,	/* Èò */
	0xc8f3, 0xd389,	/* Èó */
	0xc8f4, 0xd391,	/* Èô */
	0xc8f5, 0xd393,	/* Èõ */
	0xc8f6, 0xd397,	/* Èö */
	0xc8f7, 0xd3a1,	/* È÷ */
	0xc8f8, 0xd3a2,	/* Èø */
	0xc8f9, 0xd3a5,	/* Èù */
	0xc8fa, 0xd3a9,	/* Èú */
	0xc8fb, 0xd3b1,	/* Èû */
	0xc8fc, 0xd3b3,	/* Èü */
	0xc8fd, 0xd3b5,	/* Èý */
	0xc8fe, 0xd3b7	/* Èþ */
};

char* ga(char* str){
	char* ret_i="ÀÌ";
	char* ret_ga="°¡";
	int index, len;
	unsigned short code;
	len = strlen(str);
	code = (unsigned char)str[len-2]*0x100+(unsigned char)str[len-1];
	index = ((code >> 8) - 0xb0) * 94 + (code & 0x00ff) - 0xa1;
        if(index<0 || index>=2350) index=0;
	if((hcode[index][1]&0x0000001f)==1){//¹ÞÄ§¾øÀ½
		return ret_ga;
	}
	return ret_i;
}

char* rul(char* str){
	char* ret_ul="À»";
	char* ret_rul="¸¦";
	int index, len;
	unsigned short code;
	len = strlen(str);
	code = (unsigned char)str[len-2]*0x100+(unsigned char)str[len-1];
	index = ((code >> 8) - 0xb0) * 94 + (code & 0x00ff) - 0xa1;
        if(index<0 || index>=2350) index=0;
	if((hcode[index][1]&0x0000001f)==1){//¹ÞÄ§¾øÀ½
		return ret_rul;
	}
	return ret_ul;
}

char* ro(char* str){
	char* ret_uro="À¸·Î";
	char* ret_ro="·Î";
	int index, len;
	unsigned short code;
	len = strlen(str);
	code = (unsigned char)str[len-2]*0x100+(unsigned char)str[len-1];
	index = ((code >> 8) - 0xb0) * 94 + (code & 0x00ff) - 0xa1;
    if(index<0 || index>=2350) index=0;
	if((hcode[index][1]&0x0000001f)==1){//¹ÞÄ§¾øÀ½
		return ret_ro;
	}
	if((hcode[index][1]&0x0000001f)==9){//¹ÞÄ§ '¤©' 
		return ret_ro;
	}
	return ret_uro;
}

char* nun(char* str){
        char* ret_un="Àº";
        char* ret_nun="´Â";
        int index, len;
        unsigned short code;
        len = strlen(str);
        if(len<2) return ret_nun;
        code = (unsigned char)str[len-2]*0x100+(unsigned char)str[len-1];
        index = ((code >> 8) - 0xb0) * 94 + (code & 0x00ff) - 0xa1;
        if(index<0 || index>=2350) index=0;
        if((hcode[index][1]&0x0000001f)==1){//¹ÞÄ§¾øÀ½
                return ret_nun;
        }
        return ret_un;
}

char* da(char* str){
        char* ret_un="ÀÌ´Ù";
        char* ret_nun="´Ù";
        int index, len;
        unsigned short code;
        len = strlen(str);
        if(len<2) return ret_nun;
        code = (unsigned char)str[len-2]*0x100+(unsigned char)str[len-1];
        index = ((code >> 8) - 0xb0) * 94 + (code & 0x00ff) - 0xa1;
        if(index<0 || index>=2350) index=0;
        if((hcode[index][1]&0x0000001f)==1){//¹ÞÄ§¾øÀ½
                return ret_nun;
        }
        return ret_un;
}

char* iro(char* str){
	char* ret_uro="ÀÌ·Î";
	char* ret_ro="·Î";
	int index, len;
	unsigned short code;
	len = strlen(str);
	code = (unsigned char)str[len-2]*0x100+(unsigned char)str[len-1];
	index = ((code >> 8) - 0xb0) * 94 + (code & 0x00ff) - 0xa1;
    if(index<0 || index>=2350) index=0;
	if((hcode[index][1]&0x0000001f)==1){//¹ÞÄ§¾øÀ½
		return ret_ro;
	}
	return ret_uro;
}

int find_bracket(const char* str, int start){
	int i;
	int size = strlen(str);
	for(i=start; i<size; i++){
		if(str[i]=='(')	break;
	}
	if(i==size) i = -1;
	return i;
}

void strparse(char* result, const char* str, int* p_next_index, int b_index, int back, int len, int mode){
	char pre_ch[3];
	pre_ch[2] = 0;
	strncat(result, str+*p_next_index, b_index-*p_next_index-back);
	pre_ch[0] = str[b_index-back-2];
	pre_ch[1] = str[b_index-back-1];
	switch(mode){
	case 0://°¡
		strcat(result, ga(pre_ch));
		break;
	case 1://¸¦
		strcat(result, rul(pre_ch));
		break;
	case 2://´Â
		strcat(result, nun(pre_ch));
		break;
	case 3://·Î
		strcat(result, ro(pre_ch));
		break;
	case 4://´Ù
		strcat(result, da(pre_ch));
		break;
	case 5://(ÀÌ)·Î
		strcat(result, iro(pre_ch));
		break;
	}
	*p_next_index = b_index+len-back;
}

char* josa(const char* str){
	char result[1000];
	int b_index = 0, next_index = 0;
	memset(result, 0, 1000);
	while(b_index >= 0){
		b_index = find_bracket(str, b_index);
		if(b_index<0) break;
		if(b_index >= 0){

			/* ÀÌ(°¡) º¯°æ ·çÆ¾ */
			if(strncmp(str+b_index-3, " ÀÌ(°¡)",7)==0){  //str+b_index-(°ýÈ£°¡³ªÅ¸³ª´ÂÀ§Ä¡), "",(ÀüÃ¼¹®ÀÚ¿­¼ö)
				strparse(result, str, &next_index, b_index, 3, 7, 0); // (°ýÈ£°¡³ªÅ¸³ª´ÂÀ§Ä¡), (ÀüÃ¼¹®ÀÚ¿­¼ö), (Á¶»çÀÎµ¦½º))
			}
			else if(strncmp(str+b_index-3, " °¡(ÀÌ)",7)==0){  
				strparse(result, str, &next_index, b_index, 3, 7, 0);
			}
			else if(strncmp(str+b_index-2, "ÀÌ(°¡)",6)==0){ 
				strparse(result, str, &next_index, b_index, 2, 6, 0);
			}
			else if(strncmp(str+b_index-2, "°¡(ÀÌ)",6)==0){ 
				strparse(result, str, &next_index, b_index, 2, 6, 0);
			}
			else if(strncmp(str+b_index-1, " (°¡)ÀÌ",7)==0){
				strparse(result, str, &next_index, b_index, 1, 7, 0);
			}
			else if(strncmp(str+b_index, "(°¡)ÀÌ",6)==0){
				strparse(result, str, &next_index, b_index, 0, 6, 0);
			}
			else if(strncmp(str+b_index-1, " (ÀÌ)°¡",7)==0){
				strparse(result, str, &next_index, b_index, 1, 7, 0);
			}
			else if(strncmp(str+b_index, "(ÀÌ)°¡",6)==0){
				strparse(result, str, &next_index, b_index, 0, 6, 0);
			}


			/* À»(¸¦) º¯°æ ·çÆ¾ */
			if(strncmp(str+b_index-3, " À»(¸¦)",7)==0){  
				strparse(result, str, &next_index, b_index, 3, 7, 1);
			}
			else if(strncmp(str+b_index-3, " ¸¦(À»)",7)==0){  
				strparse(result, str, &next_index, b_index, 3, 7, 1);
			}
			else if(strncmp(str+b_index-2, "À»(¸¦)",6)==0){ 
				strparse(result, str, &next_index, b_index, 2, 6, 1);
			}
			else if(strncmp(str+b_index-2, "¸¦(À»)",6)==0){ 
				strparse(result, str, &next_index, b_index, 2, 6, 1);
			}
			else if(strncmp(str+b_index-1, " (¸¦)À»",7)==0){
				strparse(result, str, &next_index, b_index, 1, 7, 1);
			}
			else if(strncmp(str+b_index, "(¸¦)À»",6)==0){
				strparse(result, str, &next_index, b_index, 0, 6, 1);
			}
			else if(strncmp(str+b_index-1, " (À»)¸¦",7)==0){
				strparse(result, str, &next_index, b_index, 1, 7, 1);
			}
			else if(strncmp(str+b_index, "(À»)¸¦",6)==0){
				strparse(result, str, &next_index, b_index, 0, 6, 1);
			}

			/* Àº(´Â) º¯°æ ·çÆ¾ */
			if(strncmp(str+b_index-3, " Àº(´Â)",7)==0){  
				strparse(result, str, &next_index, b_index, 3, 7, 2);
			}
			else if(strncmp(str+b_index-3, " ´Â(Àº)",7)==0){  
				strparse(result, str, &next_index, b_index, 3, 7, 2);
			}
			else if(strncmp(str+b_index-2, "Àº(´Â)",6)==0){ 
				strparse(result, str, &next_index, b_index, 2, 6, 2);
			}
			else if(strncmp(str+b_index-2, "´Â(Àº)",6)==0){ 
				strparse(result, str, &next_index, b_index, 2, 6, 2);
			}
			else if(strncmp(str+b_index-1, " (´Â)Àº",7)==0){
				strparse(result, str, &next_index, b_index, 1, 7, 2);
			}
			else if(strncmp(str+b_index, "(´Â)Àº",6)==0){
				strparse(result, str, &next_index, b_index, 0, 6, 2);
			}
			else if(strncmp(str+b_index-1, " (Àº)´Â",7)==0){
				strparse(result, str, &next_index, b_index, 1, 7, 2);
			}
			else if(strncmp(str+b_index, "(Àº)´Â",6)==0){
				strparse(result, str, &next_index, b_index, 0, 6, 2);
			}

			/* (À¸)·Î º¯°æ ·çÆ¾ */
			if(strncmp(str+b_index-1, " (À¸)·Î",7)==0){  
				strparse(result, str, &next_index, b_index, 1, 7, 3);
			}
			else if(strncmp(str+b_index, "(À¸)·Î",6)==0){  
				strparse(result, str, &next_index, b_index, 0, 6, 3);
			}
			else if(strncmp(str+b_index, " (·Î)À¸·Î",9)==0){  
				strparse(result, str, &next_index, b_index, 1, 9, 3);
			}
			else if(strncmp(str+b_index, "(·Î)À¸·Î",8)==0){  
				strparse(result, str, &next_index, b_index, 0, 8, 3);
			}
			else if(strncmp(str+b_index, " (À¸·Î)·Î",9)==0){  
				strparse(result, str, &next_index, b_index, 1, 9, 3);
			}
			else if(strncmp(str+b_index, "(À¸·Î)·Î",8)==0){  
				strparse(result, str, &next_index, b_index, 0, 8, 3);
			}

			/* (ÀÌ)´Ù º¯°æ ·çÆ¾ */
			if(strncmp(str+b_index-1, " (ÀÌ)´Ù",7)==0){  
				strparse(result, str, &next_index, b_index, 1, 7, 4);
			}
			else if(strncmp(str+b_index, "(ÀÌ)´Ù",6)==0){  
				strparse(result, str, &next_index, b_index, 0, 6, 4);
			}
			else if(strncmp(str+b_index, " (´Ù)ÀÌ´Ù",9)==0){  
				strparse(result, str, &next_index, b_index, 1, 9, 4);
			}
			else if(strncmp(str+b_index, "(´Ù)ÀÌ´Ù",8)==0){  
				strparse(result, str, &next_index, b_index, 0, 8, 4);
			}
			else if(strncmp(str+b_index, " (ÀÌ´Ù)´Ù",9)==0){  
				strparse(result, str, &next_index, b_index, 1, 9, 4);
			}
			else if(strncmp(str+b_index, "(ÀÌ´Ù)´Ù",8)==0){  
				strparse(result, str, &next_index, b_index, 0, 8, 4);
			}

			/* (ÀÌ)·Î º¯°æ ·çÆ¾ */
			if(strncmp(str+b_index-1, " (ÀÌ)·Î",7)==0){  
				strparse(result, str, &next_index, b_index, 1, 7, 5);
			}
			else if(strncmp(str+b_index, "(ÀÌ)·Î",6)==0){  
				strparse(result, str, &next_index, b_index, 0, 6, 5);
			}
			else if(strncmp(str+b_index, " (·Î)ÀÌ·Î",9)==0){  
				strparse(result, str, &next_index, b_index, 1, 9, 5);
			}
			else if(strncmp(str+b_index, "(·Î)ÀÌ·Î",8)==0){  
				strparse(result, str, &next_index, b_index, 0, 8, 5);
			}
			else if(strncmp(str+b_index, " (ÀÌ·Î)·Î",9)==0){  
				strparse(result, str, &next_index, b_index, 1, 9, 5);
			}
			else if(strncmp(str+b_index, "(ÀÌ·Î)·Î",8)==0){  
				strparse(result, str, &next_index, b_index, 0, 8, 5);
			}


		}
		b_index++;
	}
	strcat(result, str+next_index);

	return result;
}

// circular buffer for keeping past messages
message_item Store_Message[ NUM_STORED_MESSAGES ];    // buffer of old messages
int Next_Message = 0;                                 // end of messages

char Message_Line = 0;                // line of next (previous?) message

static char god_message_altar_colour( char god )
{
    int  rnd;

    switch (god)
    {
    case GOD_SHINING_ONE:
        return (YELLOW);

    case GOD_ZIN:
        return (WHITE);

    case GOD_ELYVILON:
        return (LIGHTBLUE);     // really, LIGHTGREY but that's plain text

    case GOD_OKAWARU:
        return (CYAN);

    case GOD_YREDELEMNUL:
        return (coinflip() ? DARKGREY : RED);

    case GOD_KIKUBAAQUDGHA:
        return (DARKGREY);

    case GOD_XOM:
        return (random2(15) + 1);

    case GOD_VEHUMET:
        rnd = random2(3);
        return ((rnd == 0) ? LIGHTMAGENTA :
                (rnd == 1) ? LIGHTRED
                           : LIGHTBLUE);

    case GOD_MAKHLEB:
        rnd = random2(3);
        return ((rnd == 0) ? RED :
                (rnd == 1) ? LIGHTRED
                           : YELLOW);

    case GOD_TROG:
        return (RED);

    case GOD_NEMELEX_XOBEH:
        return (LIGHTMAGENTA);

    case GOD_SIF_MUNA:
        return (BLUE);

    case GOD_NO_GOD:
    default:
        return(YELLOW);
    }
}

#ifdef USE_COLOUR_MESSAGES

// returns a colour or MSGCOL_MUTED
static char channel_to_colour( int channel, int param )
{
    char        ret;

    switch (Options.channels[ channel ])
    {
    case MSGCOL_PLAIN:
        // note that if the plain channel is muted, then we're protecting
        // the player from having that spead to other other channels here.
        // The intent of plain is to give non-coloured messages, not to
        // supress them.
        if (Options.channels[ MSGCH_PLAIN ] >= MSGCOL_DEFAULT)
            ret = LIGHTGREY;
        else
            ret = Options.channels[ MSGCH_PLAIN ];
        break;

    case MSGCOL_DEFAULT:
    case MSGCOL_ALTERNATE:
        switch (channel)
        {
        case MSGCH_GOD:
            ret = (Options.channels[ channel ] == MSGCOL_DEFAULT)
                                    ? god_colour( param )
                                    : god_message_altar_colour( param );
            break;

        case MSGCH_DURATION:
            ret = LIGHTBLUE;
            break;

        case MSGCH_DANGER:
            ret = RED;
            break;

        case MSGCH_WARN:
            ret = LIGHTRED;
            break;

        case MSGCH_FOOD:
            ret = YELLOW;
            break;

        case MSGCH_INTRINSIC_GAIN:
            ret = GREEN;
            break;

        case MSGCH_RECOVERY:
            ret = LIGHTGREEN;
            break;

        case MSGCH_TALK:
            ret = WHITE;
            break;

        case MSGCH_MONSTER_SPELL:
        case MSGCH_MONSTER_ENCHANT:
            ret = LIGHTMAGENTA;
            break;

        case MSGCH_MONSTER_DAMAGE:
            ret =  ((param == MDAM_DEAD)               ? RED :
                    (param >= MDAM_HORRIBLY_DAMAGED)   ? LIGHTRED :
                    (param >= MDAM_MODERATELY_DAMAGED) ? YELLOW
                                                       : LIGHTGREY);
            break;

        case MSGCH_PROMPT:
            ret = CYAN;
            break;

        case MSGCH_DIAGNOSTICS:
            ret = DARKGREY;     ///\ makes is easier to ignore at times -- bwr
            break;

        case MSGCH_PLAIN:
        case MSGCH_ROTTEN_MEAT:
        case MSGCH_EQUIPMENT:
        default:
            ret = LIGHTGREY;
            break;
        }
        break;

    case MSGCOL_MUTED:
        ret = MSGCOL_MUTED;
        break;

    default:
        // Setting to a specific colour is handled here, special
        // cases should be handled above.
        if (channel == MSGCH_MONSTER_DAMAGE)
        {
            // a special case right now for monster damage (at least until
            // the init system is improved)... selecting a specific
            // colour here will result in only the death messages coloured
            if (param == MDAM_DEAD)
                ret = Options.channels[ channel ];
            else if (Options.channels[ MSGCH_PLAIN ] >= MSGCOL_DEFAULT)
                ret = LIGHTGREY;
            else
                ret = Options.channels[ MSGCH_PLAIN ];
        }
        else
            ret = Options.channels[ channel ];
        break;
    }

    return (ret);
}

#else // don't use colour messages

static char channel_to_colour( int channel, int param )
{
    return (LIGHTGREY);
}

#endif

void mpr(const char *inf_temp, int channel, int param) // º¯¼ö¸í º¯°æ
{
    char info2[80];
    char inf[1024]; // º¯¼ö Ãß°¡
    int colour = channel_to_colour( channel, param );
    sprintf(inf, josa(inf_temp)); // ÇÔ¼ö Àû¿ë

    if (colour == MSGCOL_MUTED)
        return;

    if (you.running > 0) you.running = 0;

    // If you're travelling, only certain user-specified messages can break 
    // travel
    if (you.running < 0)
    {
        std::string message = inf;
        for (unsigned i = 0; i < Options.stop_travel.size(); ++i)
        {
            if (message.find(Options.stop_travel[i], 0) != std::string::npos)
            {
                you.running = 0;
                break;
            }
        }
    }

    if (Options.sound_mappings.size() > 0) 
    {
        // std::string message = inf;
        for (unsigned i = 0; i < Options.sound_mappings.size(); i++) 
        {
            if (pattern_match(Options.sound_mappings[i][0].c_str(), inf))
            {
                play_sound(Options.sound_mappings[i][1].c_str());
                break;
            }
        }
    }

    flush_input_buffer( FLUSH_ON_MESSAGE );

#ifdef DOS_TERM
    window(1, 1, 80, 25);
#endif

    textcolor(LIGHTGREY);

    const int num_lines = get_number_of_lines();

    if (Message_Line == num_lines - 18) // ( Message_Line == 8 )
    {
        more();
    }
    gotoxy( (Options.delay_message_clear) ? 2 : 1, Message_Line + 18 );

    strncpy(info2, inf, 78);
    info2[78] = 0;

    textcolor( colour );
#ifdef USE_TILE
    mpr_on( MODE_MPR );
#endif
    cprintf(info2);
#ifdef USE_TILE
    mpr_on( MODE_CRT );
#endif
    //
    // reset colour
    textcolor(LIGHTGREY);

    Message_Line++;

    if (Options.delay_message_clear 
            && channel != MSGCH_PROMPT 
            && Message_Line == num_lines - 18)
    {
        more();
    }

    // equipment lists just waste space in the message recall
    if (channel != MSGCH_EQUIPMENT)
    {
        /* Put the message into Store_Message, and move the '---' line forward */
        Store_Message[ Next_Message ].text = inf;
        Store_Message[ Next_Message ].channel = channel;
        Store_Message[ Next_Message ].param = param;
        Next_Message++;

        if (Next_Message >= NUM_STORED_MESSAGES)
            Next_Message = 0;
    }
}                               // end mpr()

bool any_messages(void)
{
    return (Message_Line > 0);
}

void mesclr( bool force )
{
    // if no messages, return.
    if (!any_messages())
        return;

    if (!force && Options.delay_message_clear)
    {
        gotoxy( 1, Message_Line + 18 );
        textcolor( channel_to_colour( MSGCH_PLAIN, 0 ) );
        cprintf( ">" );
        return;
    }

    // turn cursor off -- avoid 'cursor dance'
    _setcursortype(_NOCURSOR);

#ifdef DOS_TERM
    window(1, 18, 78, 25);
    clrscr();
    window(1, 1, 80, 25);
#endif

#ifdef PLAIN_TERM
    int startLine = 18;

    gotoxy(1, startLine);

#if defined(LINUX) || defined(USE_TILE)
#ifdef USE_TILE
    mpr_on( MODE_MPR );
#endif
    clear_to_end_of_screen();
#ifdef USE_TILE
    mpr_on( MODE_CRT );
#endif
#else


    int numLines = get_number_of_lines() - startLine + 1;

    for (int i = 0; i < numLines; i++)
    {
        cprintf( "                                                                               " );
        if (i < numLines - 1)
        {
            cprintf(EOL);
        }
    }

#endif //LINUX
#endif //PLAIN_TERM

    // turn cursor back on
    _setcursortype(_NORMALCURSOR);

    Message_Line = 0;
}                               // end mseclr()

void more(void)
{
    char keypress = 0;

#ifdef PLAIN_TERM
    gotoxy( 2, get_number_of_lines() );
#endif

#ifdef DOS_TERM
    window(1, 18, 80, 25);
    gotoxy(2, 7);
#endif

    textcolor(LIGHTGREY);

#ifdef DOS
    cprintf(EOL);
#endif

#ifdef USE_TILE
        mpr_on( MODE_MPR );
#endif
#ifdef JP
    cprintf("--´ÙÀ½Àå--");
#else
    cprintf("--more--");
#endif

    set_keyin_mode(KEYIN_MODE_MORE);
    do
    {
        keypress = getch();
    }
    while (keypress != ' ' && keypress != '\r' && keypress != '\n');
    set_keyin_mode(KEYIN_MODE_NONE);

    mesclr( (Message_Line >= get_number_of_lines() - 18) );

#ifdef USE_TILE
    mpr_on( MODE_CRT );
#endif
}                               // end more()

std::string get_last_messages(int mcount)
{
    if (mcount <= 0) return std::string();
    if (mcount > NUM_STORED_MESSAGES) mcount = NUM_STORED_MESSAGES;

    bool full_buffer = Store_Message[ NUM_STORED_MESSAGES - 1 ].text.length() == 0;
    int start = Next_Message - mcount;
    if (start < 0)
        start = full_buffer? start + NUM_STORED_MESSAGES : 0;

    std::string text;
    int count = 0;
    for (int i = start; i != Next_Message; ++i)
    {
        if (i >= NUM_STORED_MESSAGES)
            i -= NUM_STORED_MESSAGES;

        if (Store_Message[i].text.length())
        {
            text += "> ";
            text += Store_Message[i].text;
            text += EOL;
            count++;
        }
    }

    // An extra line of clearance.
    if (count) text += EOL;

    return text;
}

void replay_messages(void)
{
    int            win_start_line = 0;
    int            keyin;

    bool           full_buffer = true;
    int            num_msgs = NUM_STORED_MESSAGES;
    int            first_message = Next_Message;

    const int      num_lines = get_number_of_lines();

    if (Store_Message[ NUM_STORED_MESSAGES - 1 ].text.length() == 0)
    {
        full_buffer = false;
        first_message = 0;
        num_msgs = Next_Message;
    }

    int last_message = Next_Message - 1;
    if (last_message < 0)
        last_message += NUM_STORED_MESSAGES;

#ifdef DOS_TERM
    char buffer[4800];

    window(1, 1, 80, 25);
    gettext(1, 1, 80, 25, buffer);
#endif

    // track back a screen's worth of messages from the end
    win_start_line = Next_Message - (num_lines - 2);
    if (win_start_line < 0)
    {
        if (full_buffer)
            win_start_line += NUM_STORED_MESSAGES;
        else
            win_start_line = 0;
    }

    for(;;)
    {
        // turn cursor off
        _setcursortype(_NOCURSOR);

        clrscr();

        gotoxy(1, 1);

        for (int i = 0; i < num_lines - 2; i++)
        {
            // calculate line in circular buffer
            int line = win_start_line + i;
            if (line >= NUM_STORED_MESSAGES)
                line -= NUM_STORED_MESSAGES;

            // avoid wrap-around
            if (line == first_message && i != 0)
                break;

            int colour = channel_to_colour( Store_Message[ line ].channel,
                                            Store_Message[ line ].param );
            if (colour == MSGCOL_MUTED)
                continue;

            textcolor( colour );

#if DEBUG_DIAGNOSTICS
            cprintf( "%d: %s", line, Store_Message[ line ].text.c_str() );
#else
            cprintf( Store_Message[ line ].text.c_str() );
#endif

            cprintf(EOL);
            textcolor(LIGHTGREY);
        }

        // print a footer -- note: relative co-ordinates start at 1
        int rel_start;
        if (!full_buffer)
        {
            if (Next_Message == 0)      // no messages!
                rel_start = 0;
            else
                rel_start = win_start_line + 1;
        }
        else if (win_start_line >= first_message)
            rel_start = win_start_line - first_message + 1;
        else 
            rel_start = (win_start_line + NUM_STORED_MESSAGES) - first_message + 1;

        int rel_end = rel_start + (num_lines - 2) - 1;
        if (rel_end > num_msgs)
            rel_end = num_msgs;

        cprintf( "-------------------------------------------------------------------------------" );
        cprintf(EOL);
#ifdef JP 
        cprintf( "<< %d Çà Áß %d-%d Çà >>", num_msgs, rel_start, rel_end );
#else
        cprintf( "<< Lines %d-%d of %d >>", rel_start, rel_end, num_msgs );
#endif
                 
        // turn cursor back on
        _setcursortype(_NORMALCURSOR);

#ifdef USE_TILE
        set_keyin_mode(KEYIN_MODE_MSG_REPLAY);
        keyin = getch();
        if (keyin == 0)
            keyin = getch();
        set_keyin_mode(KEYIN_MODE_NONE);
#else
        keyin = get_ch();
#endif

        if ((full_buffer && NUM_STORED_MESSAGES > num_lines - 2)
            || (!full_buffer && Next_Message > num_lines - 2))
        {
            int new_line;
            int end_mark;

#ifdef USE_TILE
            if (keyin == 'k' || keyin == '8' || keyin == '-'
                 || keyin == CMD_MOUSE_WHEEL_UP)
#else
            if (keyin == 'k' || keyin == '8' || keyin == '-')
#endif
            {
                new_line = win_start_line - (num_lines - 2);
                
                // end_mark is equivalent to Next_Message, but
                // is always less than win_start_line.
                end_mark = first_message;
                if (end_mark > win_start_line)
                    end_mark -= NUM_STORED_MESSAGES;

                ASSERT( end_mark <= win_start_line );

                if (new_line <= end_mark)
                    new_line = end_mark;   // hit top

                // handle wrap-around
                if (new_line < 0)
                {
                    if (full_buffer)
                        new_line += NUM_STORED_MESSAGES;
                    else
                        new_line = 0;
                }
            }
#ifdef USE_TILE
            else if (keyin == 'j' || keyin == '2' || keyin == '+'
                      || keyin == CMD_MOUSE_WHEEL_DOWN)
#else
            else if (keyin == 'j' || keyin == '2' || keyin == '+')
#endif
            {
                new_line = win_start_line + (num_lines - 2);

                // as above, but since we're adding we want
                // this mark to always be greater.
                end_mark = last_message;
                if (end_mark < win_start_line)
                    end_mark += NUM_STORED_MESSAGES;

                ASSERT( end_mark >= win_start_line );

                // hit bottom
                if (new_line >= end_mark - (num_lines - 2))
                    new_line = end_mark - (num_lines - 2) + 1; 

                if (new_line >= NUM_STORED_MESSAGES)
                    new_line -= NUM_STORED_MESSAGES;
            }
            else
                break;

            win_start_line = new_line;
        }
        else
        {
#ifdef USE_TILE
            if (keyin != 'k' && keyin != '8' && keyin != '-'
                && keyin != 'j' && keyin != '2' && keyin != '+'
                && keyin != CMD_MOUSE_WHEEL_UP
                && keyin != CMD_MOUSE_WHEEL_DOWN)
#else
            if (keyin != 'k' && keyin != '8' && keyin != '-'
                && keyin != 'j' && keyin != '2' && keyin != '+')
#endif
            {
                break;
            }
        }
    }


#ifdef DOS_TERM
    puttext(1, 1, 80, 25, buffer);
#endif

    return;
}                               // end replay_messages()

