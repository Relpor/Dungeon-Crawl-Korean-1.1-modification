...
void in_a_cloud(void)
{
    ...
    {
#ifdef JP
        mpr("´ç½ÅÀÇ ¾óÀ½ ¹æÆÐ°¡ ºÎ½¤Á³´Ù!", MSGCH_DURATION);
#else
...
#ifdef JP
                ouch( hurted, cl, KILLED_BY_CLOUD, "È­¿°" );
#else
    ...
    case CLOUD_COLD_MON:
        ...
        else
        {
            ...
            hurted /= (1 + resist * resist);
#ifdef JP
            ouch( hurted, cl, KILLED_BY_CLOUD, "³Ãµ¿ °¡½º" );
#else
...
#ifdef JP
        ouch( (hurted * you.time_taken) / 10, cl, KILLED_BY_CLOUD, "µ¶°¡½º" );
#else
...
#ifdef JP
        ouch( hurted, cl, KILLED_BY_CLOUD, "Áúº­ÀÇ µ¶±â" );
#else
...
void up_stairs(void)
{
    ...
    {
#ifdef JP
        mpr("µé°í ÀÖ´Â ÁüÀÌ ¹«°Å¿ö¼­ À§ÃþÀ¸·Î ¿Ã¶ó°¥ ¼ö ¾ø´Ù.");
#else
    ...
    {
#ifdef JP
        mpr("Áö¿ÁÀ» ¹æ¹®ÇÑ°É °¨»çÇÏ³×, Á¶¸¸°£ ¶Ç ¹æ¹®ÇØ ÁÖ°Ô³ª.");
#else
...
#ifdef JP
            mpr("À§·Î ³¯¾Æ¿Ã¶ú´Ù... ±×¸®°í ÃµÀå¿¡ ºÎµúÇû´Ù!");
#else
...
#ifdef JP
        mpr( "°­·ÂÇÑ ¸¶·ÂÀÌ °ø°£À» °­Á¦·Î ÀÏ±×·¯¶ß¸®´Â °ÍÀ» °¨ÁöÇß´Ù.", MSGCH_WARN );
#else
...
void down_stairs( bool remove_stairs, int old_level )
{
    ...
    {
#ifdef JP
        mpr("¹Ì¾È, ÀÌ ´øÁ¯ ±¸°£Àº ¾Ë¼ö¾ø´Â Áõ±â·Î ºÀ¼âµÇ¾ú³×.");
        mpr("´ÙÀ½ ¹öÀü¿¡ µµÀüÇØº¸°Ô.");
#else
    ...
    {
#ifdef JP
        mpr("½ÅºñÇÑ ÈûÀÌ ¾Æ·¡ÃþÀ¸·Î ³»·Á°¡´Â °ÍÀ» ¹æÇØÇÑ´Ù.");
#else
    ...
    if (stair_find == DNGN_ENTER_ZOT)
    {
        ...
        if (num_runes < NUMBER_OF_RUNES_NEEDED)
        {
            ...
            case 1:
#ifdef JP
                mpr("ÀÌ°÷¿¡ µé¾î°¡·Á¸é ÇÑ °³ÀÇ ·éÀÌ ÇÊ¿äÇÏ´Ù.");
#else
            ...
            && stair_find < DNGN_RETURN_FROM_ORCISH_MINES))
    {
        ...
        case DNGN_ENTER_DIS:
#ifdef JP
            strcat(info, "Ã¶ÀÇ µµ½Ã µð½º¿¡ ¿Â °ÍÀ» È¯¿µÇÏ³×!");
#else
...
#ifdef JP
            strcpy(info, "»ç¹æ¿¡¼­ ºÐ³ëÀÇ ½ÅÀ½¼Ò¸®°¡ µé·Á¿Â´Ù.");
#else
...
#ifdef JP
        mpr("¹®À» Åë°úÇÏ¿© °è´ÜÀ» ¿À¸£´Â ÀÚ½ÅÀ» ¹ß°ßÇß´Ù.");
#else
    ...
    {
#ifdef JP
        mpr("È¥¶õÇÑ ³ª¸ÓÁö, °è´ÜÀ¸·ÎºÎÅÍ ±¼·¯ ¶³¾îÁ³´Ù.");
#else
...
#ifdef JP
        mpr("´ç½ÅÀº ¾îµÓ°í À½¿ïÇÑ ¹Ì±ÃÀ¸·Î µé¾î°£´Ù.");
#else
    ...
    case LEVEL_ABYSS:
#ifdef JP
        mpr("´ç½ÅÀº ¾îºñ½º·Î µé¾î°£´Ù! ");
        mpr("ºüÁ®³ª¿À±â À§ÇØ¼­´Â Ãâ±¸¸¦ Ã£¾Æ³»¾ß¸¸ ÇÑ´Ù.");
#else
    ...
    case LEVEL_PANDEMONIUM:
...
#ifdef JP
            mpr("´ç½ÅÀº ÆÇµ¥¸ð´Ï¿òÀÇ ´Ù¸¥ ¹æÀ¸·Î µé¾î°¬´Ù.");
#else
        ...
        {
#ifdef JP
            mpr("´ç½ÅÀº ÆÇµ¥¸ð´Ï¿òÀÇ Å« ¹æ¿¡ µé¾î¿Ô´Ù! ");
            mpr("ºüÁ®³ª¿À±â À§ÇØ¼­´Â Ãâ±¸¸¦ Ã£¾Æ³»¾ß¸¸ ÇÑ´Ù.");
#else
    ...
    if (you.skills[SK_TRANSLOCATIONS] > 0 && !allow_control_teleport( true ))
#ifdef JP
        mpr( "´ç½ÅÀº °­·ÂÇÑ ¸¶¹ýÀÇ ÈûÀÌ °ø°£À» ¿Ö°î½ÃÅ°´Â °ÍÀ» °¨ÁöÇß´Ù.", MSGCH_WARN );
#else
        mpr( "You sense a powerful magical force warping space.", MSGCH_WARN );
#endif
    if (collect_travel_data) {
...
#ifdef JP
            cprintf("ÁöÇÏ %dÃþ                  ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("µð½º %dÃþ                ", curr_subdungeon_level);
#else
            cprintf(" of Dis                   ");
#endif
            break;
...
#ifdef JP
            cprintf("°ÔÇì³ª %dÃþ                ", curr_subdungeon_level);
#else
            cprintf(" of Gehenna               ");
#endif
            break;
...
#ifdef JP
            cprintf("ÄÚÅ°Åõ½º %dÃþ              ", curr_subdungeon_level);
#else
            cprintf(" of Cocytus                   ");
#endif
            break;
...
#ifdef JP
            cprintf("Å¸¸£Å¸·ç½º %dÃþ              ", curr_subdungeon_level);
#else
            cprintf(" of Tartarus                ");
#endif
            break;
...
#ifdef JP
            cprintf("ÀÎÆä¸£³ë %dÃþ                  ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("ÇÇÆ® %dÃþ                   ", curr_subdungeon_level);
#else
            cprintf(" of the Pit              ");
#endif
            break;
...
#ifdef JP
            cprintf("¿ÀÅ© ±¤»ê %dÃþ              ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("¹úÁý %dÃþ                    ", curr_subdungeon_level);
#else
            cprintf(" of the Hive                  ");
#endif
            break;
...
#ifdef JP
            cprintf("Áü½Â ¼Ò±¼ %dÃþ                ", curr_subdungeon_level);
#else
            cprintf(" of the Lair                  ");
#endif
            break;
...
#ifdef JP
            cprintf("½½¶óÀÓ ¼Ò±¼ %dÃþ          ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("º¸¹°Ã¢°í %dÃþ                    ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("³³°ñ´ç %dÃþ                  ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("Ä®³¯ÀÇ ¹æ %dÃþ                  ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("Á¶Æ®ÀÇ ¹æ %dÃþ              ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("¹ì ¼Ò±¼ %dÃþ                      ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("¿¤ÇÁÀÇ ¹æ %dÃþ            ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("¹«´ý %dÃþ                      ", curr_subdungeon_level);
#else
            cprintf(" of the Tomb                  ");
#endif
            break;
...
#ifdef JP
            cprintf("´ËÁö %dÃþ                        ", curr_subdungeon_level);
#else
                       ...
                       bool poison )
{
    ...
    {
#ifdef JP
        snprintf( info, INFO_SIZE, "´ç½ÅÀº %sÀÇ ÇÔÁ¤À» ¹«»çÈ÷ Åë°úÇß´Ù.",
#else
...
#ifdef JP
    snprintf( info, INFO_SIZE, "%sÀÌ(°¡) ½î¾ÆÁ® ³ª¿Í ", pbolt.beam_name );
#else
...
void handle_traps(char trt, int i, bool trap_known)
{
    ...
    case TRAP_AMNESIA:
#ifdef JP
        mpr("´ç½ÅÀº ¼ø°£ÀûÀ¸·Î ¹æÇâ°¨°¢À» ÀÒ¾î¹ö·È´Ù.");
#else
    ...
    case TRAP_BLADE:
...
#ifdef JP
            mpr("´ç½ÅÀº ºí·¹ÀÌµå ÇÔÁ¤À» ¹«»çÈ÷ Åë°úÇß´Ù.");
#else
        ...
        {
#ifdef JP
            mpr("°Å´ëÇÑ Ä®³¯ÀÌ ´ç½ÅÀÇ µÚ¸¦ ¾Æ½½¾Æ½½ÇÏ°Ô ½ºÃÄÁö³ª°£´Ù!");
#else
        ...
        {
#ifdef JP
            mpr("°Å´ëÇÑ Ä®³¯ÀÌ ¾îµð¼±°¡ ³¯¾Æ¿Í, ´ç½ÅÀ» º£¾î¹ö·È´Ù!");
#else
            mpr("A huge blade swings out and slices into you!");
#endif
            ouch( (you.your_level * 2) + random2avg(29, 2)
#ifdef JP
                    - random2(1 + player_AC()), 0, KILLED_BY_TRAP, "Ä®³¯" );
#else
    ...
    default:
#ifdef JP
        mpr((trap_known) ? "´ç½ÅÀº Á¶Æ®ÀÇ ÇÔÁ¤¿¡ ¹ßÀ» µðµ±´Ù. "
                         : "ÀÌ·±! ´ç½ÅÀº Á¶Æ®ÀÇ ÇÔÁ¤¿¡ ¹ßÀ» µðµ® ¹ö·È´Ù!");
#else
...
void disarm_trap( struct dist &disa )
{
    ...
    {
#ifdef JP
        mpr("´ç½ÅÀº ÇÔÁ¤À» ÇØÁ¦ÇÏ´Âµ¥ ½ÇÆÐÇß´Ù.");
#else
...
#ifdef JP
    mpr("´ç½ÅÀº ÇÔÁ¤À» ¼º°øÀûÀ¸·Î ÇØÁ¦Çß´Ù.");
#else
...
#ifdef JP // Å×½ºÆ®½Ã È®ÀÎ¹Ù¶÷ (¹Ù±ø)
    strcpy(stringy, (temp_rand == 0) ? "±¸ºÎ·¯Áø" :
                    (temp_rand == 1) ? "¼±¸íÇÑ" :
                    (temp_rand == 2) ? "½ºÄ£µíÇÑ" :
                    (temp_rand == 3) ? "±¸ºÒ±¸ºÒÇÑ" :
                    (temp_rand == 4) ? "¾ó·èÁø" :
                    (temp_rand == 5) ? "¸ð³­" :
                    (temp_rand == 6) ? "Èñ¹ÌÇÏ°Ô ºû³ª´Â" :
                    (temp_rand == 7) ? "°­·ÄÇÏ°Ô ºû³ª´Â" : "");
#else
...
#ifdef JP // Å×½ºÆ®½Ã È®ÀÎ¹Ù¶÷ (¹Ù±ø)
    strcat(stringy, (temp_rand ==  0) ? "³ë¶õ»ö" :
                    (temp_rand ==  1) ? "°¥»ö" :
                    (temp_rand ==  2) ? "°ËÀº»ö" :
                    (temp_rand ==  3) ? "º¸¶ó»ö" :
                    (temp_rand ==  4) ? "¿À·»Áö»ö" :
...
#ifdef JP // Å×½ºÆ®½Ã È®ÀÎ¹Ù¶÷ (¹Ù±ø)
    strcat(stringy, (temp_rand == 0) ? "¸Þ¸ð" :
                    (temp_rand == 1) ? "³«¼­" :
                    (temp_rand == 2) ? "ÀÎÀå" :
                    (temp_rand == 3) ? "·é¹®ÀÚ" :
                    (temp_rand == 4) ? "»óÇü¹®ÀÚ" :
...
void fall_into_a_pool(bool place, unsigned char terrain)
{
...
#ifdef JP
    strcat(info, (terrain == DNGN_LAVA)       ? "¿ë¾Ï" :
                 (terrain == DNGN_DEEP_WATER) ? "¹°"
                                              : "ÇÁ·Î±×·¥ÀÇ Æ´");
#else
    ...
    if (terrain == DNGN_LAVA)
    {
        ...
        {
#ifdef JP
            mpr( "¿ë¾ÏÀÌ ´ç½ÅÀ» ÇÑÁÜÀÇ Àç·Î ¸¸µé¾î¹ö·È´Ù!" );
#else
...
#ifdef JP
            mpr( "´ç½ÅÀº ¿ë¾Ï¿¡ ÀÇÇØ È­»óÀ» ÀÔ¾ú´Ù!" );
#else
        ...
        {
#ifdef JP
            mpr("´ç½ÅÀÇ ¾óÀ½¹æÆÐ°¡ ±úÁ³´Ù!", MSGCH_DURATION);
#else
            mpr("Your icy shield dissipates!", MSGCH_DURATION);
#endif
            you.duration[DUR_CONDENSATION_SHIELD] = 0;
    ...
    else
    {
        ...
        if (!place)
#ifdef JP
            mpr("´ç½ÅÀº ºüÁ®³ª¿À·Á°í ÇßÁö¸¸, Áü¿¡ ²ø·Á ¶³¾îÁ³´Ù!");
#else
...
#ifdef JP // Å×½ºÆ®½Ã È®ÀÎ¹Ù¶÷ (¹Ù±ø)
    strcpy(wc, (coll_div16 == 0 || coll_div16 ==  7) ? "Âù¶õÇÏ°Ô ºû³ª´Â " :
               (coll_div16 == 1 || coll_div16 ==  8) ? "Èñ¹ÌÇÑ " :
               (coll_div16 == 2 || coll_div16 ==  9) ? "¾ó·è´ú·èÇÑ " :
               (coll_div16 == 3 || coll_div16 == 10) ? "ÀºÀºÇÑ " :
               (coll_div16 == 4 || coll_div16 == 11) ? "´«ºÎ½Å " :
               (coll_div16 == 5 || coll_div16 == 12) ? "¾îµÎ¿î " :
               (coll_div16 == 6 || coll_div16 == 13) ? "¹øÂ½ÀÌ´Â "
                                                     : "Èñ¹ÌÇÑ ");
#else
...
bool go_berserk(bool intentional)
{
    ...
    if (you.exhausted)
    {
        if (intentional)
#ifdef JP
            mpr("´ç½ÅÀº ±¤Àü»ç°¡ µÇ±â¿¡´Â ³Ê¹« ÇÇ·ÎÇÏ´Ù.");
#else
    ...
    if (you.is_undead)
    {
        if (intentional)
#ifdef JP
            mpr("´ç½ÅÀÇ »ý¸íÀÌ ¾ø´Â À°Ã¼´Â ºÐ³ë¸¦ ´À³¢Áö ¸øÇÑ´Ù.");
#else
...
#ifdef JP
    //°¡¼Ó ¸Þ¼¼Áö°¡ Áßº¹ÇÏ¹Ç·Î º¯°æ. (<- 70% ÇÑ±ÛÈ­ÀÇ ÁÖ¼®)
    mpr("´ç½ÅÀÇ ½Ã¾ß´Â »õ»¡°²°Ô ¹°µé¾î°£´Ù! ");
    mpr("´ç½ÅÀº ±¤Àü»ç »óÅÂ¿¡ µ¹ÀÔÇß´Ù!");
#else
