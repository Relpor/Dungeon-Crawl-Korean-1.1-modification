...
void in_a_cloud(void)
{
    ...
    {
#ifdef JP
        mpr("당신의 얼음 방패가 부숴졌다!", MSGCH_DURATION);
#else
...
#ifdef JP
                ouch( hurted, cl, KILLED_BY_CLOUD, "화염" );
#else
    ...
    case CLOUD_COLD_MON:
        ...
        else
        {
            ...
            hurted /= (1 + resist * resist);
#ifdef JP
            ouch( hurted, cl, KILLED_BY_CLOUD, "냉동 가스" );
#else
...
#ifdef JP
        ouch( (hurted * you.time_taken) / 10, cl, KILLED_BY_CLOUD, "독가스" );
#else
...
#ifdef JP
        ouch( hurted, cl, KILLED_BY_CLOUD, "질벼의 독기" );
#else
...
void up_stairs(void)
{
    ...
    {
#ifdef JP
        mpr("들고 있는 짐이 무거워서 위층으로 올라갈 수 없다.");
#else
    ...
    {
#ifdef JP
        mpr("지옥을 방문한걸 감사하네, 조만간 또 방문해 주게나.");
#else
...
#ifdef JP
        mpr( "강력한 마력이 공간을 강제로 일그러뜨리는 것을 감지했다.", MSGCH_WARN );
#else
...
void down_stairs( bool remove_stairs, int old_level )
{
    ...
    {
#ifdef JP
        mpr("미안, 이 던젼 구간은 알수없는 증기로 봉쇄되었네.");
        mpr("다음 버전에 도전해보게.");
#else
            ...
            && stair_find < DNGN_RETURN_FROM_ORCISH_MINES))
    {
        ...
        case DNGN_ENTER_DIS:
#ifdef JP
            strcat(info, "철의 도시 디스에 온 것을 환영하네!");
#else
...
#ifdef JP
            strcpy(info, "사방에서 분노의 신음소리가 들려온다.");
#else
    ...
    if (you.skills[SK_TRANSLOCATIONS] > 0 && !allow_control_teleport( true ))
#ifdef JP
        mpr( "당신은 강력한 마법의 힘이 공간을 왜곡시키는 것을 감지했다.", MSGCH_WARN );
#else
        mpr( "You sense a powerful magical force warping space.", MSGCH_WARN );
#endif
    if (collect_travel_data) {
...
#ifdef JP
            cprintf("지하 %d층                  ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("디스 %d층                ", curr_subdungeon_level);
#else
            cprintf(" of Dis                   ");
#endif
            break;
...
#ifdef JP
            cprintf("게헤나 %d층                ", curr_subdungeon_level);
#else
            cprintf(" of Gehenna               ");
#endif
            break;
...
#ifdef JP
            cprintf("코키투스 %d층              ", curr_subdungeon_level);
#else
            cprintf(" of Cocytus                   ");
#endif
            break;
...
#ifdef JP
            cprintf("타르타루스 %d층              ", curr_subdungeon_level);
#else
            cprintf(" of Tartarus                ");
#endif
            break;
...
#ifdef JP
            cprintf("인페르노 %d층                  ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("피트 %d층                   ", curr_subdungeon_level);
#else
            cprintf(" of the Pit              ");
#endif
            break;
...
#ifdef JP
            cprintf("오크 광산 %d층              ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("벌집 %d층                    ", curr_subdungeon_level);
#else
            cprintf(" of the Hive                  ");
#endif
            break;
...
#ifdef JP
            cprintf("짐승 소굴 %d층                ", curr_subdungeon_level);
#else
            cprintf(" of the Lair                  ");
#endif
            break;
...
#ifdef JP
            cprintf("슬라임 소굴 %d층          ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("보물창고 %d층                    ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("납골당 %d층                  ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("칼날의 방 %d층                  ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("조트의 방 %d층              ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("뱀 소굴 %d층                      ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("엘프의 방 %d층            ", curr_subdungeon_level);
#else
...
#ifdef JP
            cprintf("무덤 %d층                      ", curr_subdungeon_level);
#else
            cprintf(" of the Tomb                  ");
#endif
            break;
...
#ifdef JP
            cprintf("늪지 %d층                        ", curr_subdungeon_level);
#else
                       ...
                       bool poison )
{
    ...
    {
#ifdef JP
        snprintf( info, INFO_SIZE, "당신은 %s의 함정을 무사히 통과했다.",
#else
...
#ifdef JP
    snprintf( info, INFO_SIZE, "%s이(가) 쏘아져 나와 ", pbolt.beam_name );
#else
...
void handle_traps(char trt, int i, bool trap_known)
{
    ...
    case TRAP_BLADE:
        ...
        {
#ifdef JP
            mpr("거대한 칼날이 당신의 뒤를 아슬아슬하게 스쳐지나간다!");
#else
        ...
        {
#ifdef JP
            mpr("거대한 칼날이 어디선가 날아와, 당신을 베어버렸다!");
#else
            mpr("A huge blade swings out and slices into you!");
#endif
            ouch( (you.your_level * 2) + random2avg(29, 2)
#ifdef JP
                    - random2(1 + player_AC()), 0, KILLED_BY_TRAP, "칼날" );
#else
    ...
    default:
#ifdef JP
        mpr((trap_known) ? "당신은 조트의 함정에 발을 디뎠다. "
                         : "이런! 당신은 조트의 함정에 발을 디뎌 버렸다!");
#else
...
#ifdef JP // 테스트시 확인바람 (바깡)
    strcpy(stringy, (temp_rand == 0) ? "구부러진" :
                    (temp_rand == 1) ? "선명한" :
                    (temp_rand == 2) ? "스친듯한" :
                    (temp_rand == 3) ? "구불구불한" :
                    (temp_rand == 4) ? "얼룩진" :
                    (temp_rand == 5) ? "모난" :
                    (temp_rand == 6) ? "희미하게 빛나는" :
                    (temp_rand == 7) ? "강렬하게 빛나는" : "");
#else
...
#ifdef JP // 테스트시 확인바람 (바깡)
    strcat(stringy, (temp_rand ==  0) ? "노란색" :
                    (temp_rand ==  1) ? "갈색" :
                    (temp_rand ==  2) ? "검은색" :
                    (temp_rand ==  3) ? "보라색" :
                    (temp_rand ==  4) ? "오렌지색" :
...
void fall_into_a_pool(bool place, unsigned char terrain)
{
...
#ifdef JP
    strcat(info, (terrain == DNGN_LAVA)       ? "용암" :
    ...
    if (terrain == DNGN_LAVA)
    {
        ...
        {
#ifdef JP
            mpr("당신의 얼음방패가 깨졌다!", MSGCH_DURATION);
#else
            mpr("Your icy shield dissipates!", MSGCH_DURATION);
#endif
            you.duration[DUR_CONDENSATION_SHIELD] = 0;
...
#ifdef JP // 테스트시 확인바람 (바깡)
    strcpy(wc, (coll_div16 == 0 || coll_div16 ==  7) ? "찬란하게 빛나는 " :
               (coll_div16 == 1 || coll_div16 ==  8) ? "희미한 " :
               (coll_div16 == 2 || coll_div16 ==  9) ? "얼룩덜룩한 " :
               (coll_div16 == 3 || coll_div16 == 10) ? "은은한 " :
               (coll_div16 == 4 || coll_div16 == 11) ? "눈부신 " :
               (coll_div16 == 5 || coll_div16 == 12) ? "어두운 " :
               (coll_div16 == 6 || coll_div16 == 13) ? "번쩍이는 "
                                                     : "희미한 ");
#else
...
bool go_berserk(bool intentional)
{
    ...
    if (you.is_undead)
    {
        if (intentional)
#ifdef JP
            mpr("당신의 생명이 없는 육체는 분노를 느끼지 못한다.");
#else
...
#ifdef JP
    //가속 메세지가 중복하므로 변경. (<- 70% 한글화의 주석)
    mpr("당신의 시야는 새빨갛게 물들어간다! ");
    mpr("당신은 광전사 상태에 돌입했다!");
#else
