...
//Tile cursor ï\é¶Ç≥ÇÍÇƒÇ¢ÇÈà íuÅAÉtÉâÉO
static int tile_cursor_x;
static int tile_cursor_y;
static int tile_cursor_flag=0;
// à»ëOï\é¶ÇµÇƒÇ¢ÇΩílÅBà⁄ìÆéûÇÃÉoÉbÉNÉAÉbÉv
static int old_cursor_flag = 0;


#if 1  // MOUSE
//É}ÉEÉXÉ|ÉCÉìÉ^ÇÃÇ†ÇÈà íu
static int tile_cursor_x0 = 0;
static int tile_cursor_y0 = 0;

//ì¸óÕÉÇÅ[ÉhÇ…ÇÊÇËÉ}ÉEÉXãììÆïœâª
//ÉÅÉCÉìë§Ç©ÇÁ set_keyin_mode(mode)Ç≈ïœçX
...
/* 32x32 ÇÃèÍçá  É^ÉCÉãÇÃ(sx,sy)Ç©ÇÁwx*wy ÇÃóÃàÊÇ ox,oy Ç∏ÇÁÇµÇƒÉRÉsÅ[ */

#ifdef QV48
//ëÂÇ´Ç¢BMPÇ™48x48 ÇÃèÍçá
...
#define DOLLS_MAX 11         //ï“èWâ¬î\Ç»íÖÇπë÷Ç¶êlå`ÇÃêî
#define PARTS_DISP_MAX 11    //âÊñ Ç…ï\é¶Ç≥ÇÍÇÈïîïiêî
#define PARTS_ITEMS 12       //ï“èWâ¬î\Ç»çÄñ⁄ÇÃêî
#define TILEP_SELECT_DOLL 20 //ÉÅÉjÉÖÅ[Ç…égÇ§É_É~Å[
...
//èâä˙âª
    ...
    //ÉXÉeÅ[É^ÉXâÊñ Ç…ï\é¶Ç∑ÇÈïîïiî‘çÜÇÃï\é¶ÇÃèáî‘
    const int p_lines[PARTS_ITEMS]=
      {
        TILEP_SELECT_DOLL, //É_É~Å[
        TILEP_PART_BASE,
        TILEP_PART_HELM,
        TILEP_PART_HAIR,
        TILEP_PART_BEARD,
...
{                               \
    ...
    int cur_doll = 0,   //Ç«ÇÃÉhÅ[ÉãÇÇ¢Ç∂Ç¡ÇƒÇ¢ÇÈÇ©
        pre_part = 0,   //Ç«ÇÃïîà ÇÇ¢Ç∂Ç¡ÇƒÇ¢ÇΩÇ©
        cur_part = 0,   //ç°Ç«ÇÃïîà ÇÇ¢Ç∂Ç¡ÇƒÇ¢ÇÈÇ©
        mode     = TILEP_M_DEFAULT;  //dolls.txtÇãNìÆéûÇ…ì«Ç›çûÇﬁÇ©î€Ç©
    int  d = 0;
    FILE *fp;
    char fbuf[80], ibuf[8];
    int tile_str, sx, sy;

    //ÉrÉbÉgÉ}ÉbÉvóÃàÊÇÃämï€
    img_type PartsImg = ImgCreateSimple(TILE_X * PARTS_DISP_MAX, TILE_Y * 1);
    //ïîïiÉäÉXÉgï\é¶ópÉrÉbÉgÉ}ÉbÉv

    img_type DollsListImg = ImgCreateSimple( TILE_X * DOLLS_MAX, TILE_Y * 1);
    //íÖÇπë÷Ç¶êlå`ÉXÉgÉbÉNÇÃÉäÉXÉgï\é¶ópÉrÉbÉgÉ}ÉbÉv

    //ÉoÉbÉNÉoÉbÉtÉ@Çì¡íËêFÇ≈ñÑÇﬂÇÈÅB
    //windowsî≈Ç≈ÇÕÉ^ÉCÉãínê}ÉoÉbÉtÉ@ÇÇªÇÃÇ‹Ç‹íÖÇπë÷Ç¶êlå`ï\é¶ëãÇ…Ç∑ÇÈÅB
    ImgClear(ScrBufImg);

    //íÖÇπë÷Ç¶îzóÒÇÃèâä˙âª
...
//âê‡ï∂ï\é¶
#ifdef JP
    mpr("∫Œ¿ß º±≈√               : [j][k]or[¿ß][æ∆∑°]");
    mpr("¿Â∫Ò º±≈√/∆‰¿Ã¡ˆ ¿Ãµø   : [h][l]or[¡¬][øÏ]/[H][L]or[SHIFT]+[¡¬][øÏ]");
    mpr("¡˜æ˜ø° ¡ÿ«œ¥¬ ¿«∫π      : [CTRL]+[D]");
    mpr("¿Â∫Ò ¿«¡∏ «•Ω√ ON/OFF   : [*]");
    mpr("¿Â∫Ò π˛±‚/∏µŒ π˛±‚     : [t]/[CTRL]+[T]");
    mpr("∫πªÁ/∫Ÿø©≥÷±‚/1¥‹∞Ë√Îº“ : [CTRL]+[C]/[CTRL]+[V]/[CTRL]+[Z]");
    mpr("∑£¥˝¿∏∑Œ ∞•æ∆¿‘»˜±‚     : [CTRL]+[R]");

    mpr_on(MODE_MPR);
    gotoxy(1, 25);
    cprintf("Ω√¿€Ω√ º≥¡§             : [m] («ˆ¿Á º≥¡§ :%s)",
        ( (mode == TILEP_M_LOADING) ? "Ω√¿€Ω√ ∫π¿Â¿ª ¿–æÓµÈ¿Ã¥¬":"Ω√¿€Ω√ ¡˜æ˜ø° µ˚∏• ∫π¿Â" ));
#else
...
//ÉäÉXÉgÇï\é¶
    mpr_on(MODE_STAT);
#ifdef JP
    gotoxy(40,  1);cprintf("  π¯»£ :");
    gotoxy(40,  2);cprintf("  º∫∫∞ :");
    gotoxy(40,  3);cprintf("  ∏”∏Æ :");
    gotoxy(40,  4);cprintf("  µŒπﬂ :");
    gotoxy(40,  5);cprintf("  ºˆø∞ :");
    gotoxy(40,  6);cprintf("  π´±‚ :");
    gotoxy(40,  7);cprintf("  πÊ∆– :");
    gotoxy(40,  8);cprintf("  ¿Â∞© :");
    gotoxy(40,  9);cprintf("  ∏ˆ≈Î :");
    gotoxy(40, 10);cprintf("  ¥Ÿ∏Æ :");
    gotoxy(40, 11);cprintf("  Ω≈πﬂ :");
...
//à»â∫ÅAïîïiëIëÉãÅ[Év
    ...
    while (!done)
    {
        ...
        if (p_lines[cur_part] == TILEP_SELECT_DOLL) //ñÓàÛÇÕï“èWÇ∑ÇÈêlå`ÇÃëIëÇÃà íuÇ…Ç†ÇÈ
        {
            gotoxy(40+9 , 1);mpr_on(MODE_STAT);
#ifdef JP
            cprintf("%02dπ¯", cur_doll);
#else
        ...
        else //ñÓàÛÇ™ïîà ÇÃÇ«Ç±Ç©ÇëIëÇµÇƒÇ¢ÇÈèÍçáÅBïîïiàÍóóâÊëúÇçXêV
        {
...
#ifdef JP
        if (cur_part == 0) //çÄñ⁄Ç™ï“èWíÜÇÃêlå`î‘çÜÇ»ÇÁî‘çÜÇÃï\é¶
            cprintf("%02dπ¯", cur_doll);
#else
        if (cur_part == 0) //çÄñ⁄Ç™ï“èWíÜÇÃêlå`î‘çÜÇ»ÇÁî‘çÜÇÃï\é¶
            cprintf("%02d", cur_doll);
#endif
        else if (cur_part == 1) //çÄñ⁄Ç™ê´ï Ç»ÇÁê´ï ÇÃï\é¶
        {
            cprintf("%s", gender_name[ dolls[cur_doll].parts[p_lines[cur_part] ] % 2 ]);
        }
        else               //çÄñ⁄Ç™ê´ï à»äOÇ»ÇÁêîílÇÃï\é¶
        {
        ...
        switch (kin)
        {
            ...
            case 'h': //ïîïiïœçX:ç∂
            case 'H': //ïîïiïœçX:1ÉyÅ[ÉWç∂
            {
                int *target = &dolls[cur_doll].parts[ p_lines[cur_part] ];

                if (cur_part == 0)      //çÄñ⁄Ç™ï“èWëŒè€ÉhÅ[ÉãëIë
                {
                    if (cur_doll > 0)
                        cur_doll -= 1;
                    else
                ...
                else if (cur_part == 1) //çÄñ⁄Ç™ê´ï Ç»ÇÁê´ï ÇÃÉgÉOÉã
                {
                    if (*target % 2)
                        (*target)++;
                    else
            ...
            case 'm': //êlå`ÉtÉ@ÉCÉããNìÆéûì«Ç›çûÇ›Ç…Ç¬Ç¢ÇƒÇÃê›íËÉgÉOÉã
            {
            ...
            case 't': //ëIëíÜÇÃïîà ÇÃëïîıÇæÇØíEÇÆ
            {
                if (cur_part >= 2)
                {
            ...
            case CONTROL('D'): //ëïîıÇëSÇƒåªç›ÇÃêEã∆ÇÃÉfÉtÉHÉãÉgÇ…ñﬂÇ∑
            {
        ...
        mpr_on(MODE_STAT);
            ...
            case CONTROL('R'): //ÉâÉìÉ_ÉÄÇ»íÖÇπë÷Ç¶
            {
                for(i = 2; i < PARTS_ITEMS; i++)
                {
                    undo_dolls[cur_doll].parts[ p_lines[i] ] = dolls[cur_doll].parts[ p_lines[i] ];
...
//ÉrÉbÉgÉ}ÉbÉvóÒÇ…égópÇµÇΩÉÅÉÇÉäÇÃâï˙
    ImgDestroy(PartsImg);
    ImgDestroy(DollsListImg);

//ÉLÉÉÉbÉVÉÖîpä¸Ç»Ç«ÉQÅ[ÉÄÇ÷ÇÃïúãAèàóù
    ImgClear(ScrBufImg);
    redraw_spx_tcache(TILE_PLAYER);
...
{
    int i, j;
    int cur_doll = 0,          //Ç«ÇÃÉhÅ[ÉãÇ™ëIëÇ≥ÇÍÇƒÇ¢ÇΩÇ©
        mode     = TILEP_M_DEFAULT;  //dolls.txtÇãNìÆéûÇ…ì«Ç›çûÇﬁÇ©î€Ç©
    dolls_data doll;
    FILE *fp;
    char fbuf[80], ibuf[8];
...
void tcache_compose_qv(int kind, int ix, int *fg_dat, int *bg_dat){
  ...
  */
    ...
    int cflag[4];
    int c=1;
    // ìßâﬂópÉ}ÉXÉNñ¢égópÇÃÉtÉâÉO
    int bgflag = 1;
    static char mask[TILE_UX_QV*TILE_UY_QV];
    char *maskptr = NULL;

    char *fg_maskptr = NULL;
    // ÉJÅ[É\ÉãìßâﬂópÉ}ÉXÉNñ¢égópÇÃÉtÉâÉO
    ...
    // priority ÇÃçÇÇ¢êÖñ Ç»Ç«Çå„Ç≈ï`âÊÇ∑ÇÈÇΩÇﬂÉ\Å[Ég
    // êÖñ Ç™è„â∫ç∂âEÇÃè∞Ç…ÇµÇ›ÇæÇ∑ÉGÉtÉFÉNÉgÇÃÇΩÇﬂÇÃèàóù
    sort_bbg(bbg, bbg_region);

    // è∞Ç‚êÖñ ÇÕï«Ç‚ÉÇÉìÉXÉ^Å[Ç»Ç«ÇÊÇËêÊÇ…ëSÇƒÇÃÉåÉCÉÑÇï`âÊÇµÇƒÇ®Ç≠ÅB
    // Ç±Ç§ÇµÇ»Ç¢Ç∆å„ÇÎÇÃÉÇÉìÉXÉ^Å[Ç™è∞Ç≈è„èëÇ´Ç≥ÇÍÇÈ
    for (layer=0; layer<4; layer++)
    {
        if(bbg[layer])
    {
    ...
    //éãäEäOÉÅÉbÉVÉÖÅïÉJÅ[É\ÉãÇÃï`âÊ
    for (layer=0; layer<4; layer++)
    {
        int region = region_of_layer[kind][layer];
    ...
    //if ((bbg0[layer]&TILE_FLAG_MASK)==simple_qv_tile(TILE_DNGN_FLOOR))
        //can_see[layer] = false;
        //Unseen mesh  éãäEäOÇÕè∞ñ Ç…çïÉÅÉbÉVÉÖÇè„èëÇ´
        ...
        //Draw Tile cursor
        // ÉJÅ[É\ÉãÇÃà íuÇ≈ÇÕï«ÇÃìßâﬂèàóùÇçsÇ§ÇΩÇﬂè∞âÊëúÇ mask ìoò^
        if(cflag[layer])
        {
    ...
    if(bg)
    {
        // É}ÉXÉNÇ™Ç†ÇÈèÍçáÇÕï«Çìßâﬂèàóù
        if (is_wall)
            tcache_overlay(tc_img, ix, bg, region, &c, maskptr);
        else
            tcache_overlay(tc_img, ix, bg, region, &c, NULL);

        // â∫ÇËäKíiÇ»Ç«Ç™ìßâﬂÇ∑ÇÈÇÊÇ§É}ÉXÉNìoò^  ï«ÇÃèÍçá(prio==10)ÇÕ
        // çsÇÌÇ»Ç¢
            if ((layer != 3) && !is_wall)
            {
    ...
    if(fg_dat[layer])
    {
        ...
        // âΩÇ©ï®Ç™Ç†ÇÈÇ∆Ç´ÇÕìßâﬂèàóùÇÃÇΩÇﬂè∞Ç∆ï®ÇÉ}ÉXÉNìoò^
            if (layer != 3)
        {
...
const tip_info Tips[] ={
  ...
  {0,12,0,0, "¿Ã∏ß∞˙ ƒ™»£", NULL},
  {0,12,1,1, "¡æ¡∑", NULL},
  {0,15,2,2, "ª˝∏Ì∑¬ ¡°ºˆ", NULL},
  {20,39,2,2,
    "HPπŸ : ø¿∏•¬  ≥°¿Ã √÷¥Îƒ°"
    "∫”¿∫ ∫Œ∫–¿∫ ¡˜¿¸ø° πﬁ¿∫ µ•πÃ¡ˆ", NULL},
  {0,12,3,3, "∏∂∑¬ ¡°ºˆ", NULL},
  {0,12,4,4, "πÊæÓ±∏ ≈¨∑°Ω∫(øÏ≈¨∏Ø¿∏∑Œ ªÛºº∫∏±‚)", "stat_acev"},
  {0,12,5,5, "»∏««∑¬(øÏ≈¨∏Ø¿∏∑Œ ªÛºº∫∏±‚)", "stat_acev"},
  {0,12,6,6,
    "»˚ : ¿¸≈ıΩ√ø° ¡÷¥¬ µ•πÃ¡ˆ∑Æø° øµ«‚¿ª ¡÷∏Á,"
    "¡¸¿ª øÓπ›«œ¥¬ »˚ø°µµ øµ«‚¿ª ¡ÿ¥Ÿ."
    , NULL},
  {0,12,7,7,
    "¡ˆ¥… : ∏∂π˝ æ∆¿Ã≈€¿ª ªÁøÎ«œ¥¬ ¥…∑¬,"
    "æÓ¥¿¡§µµ ¡÷πÆ¿ª ¿ﬂ Ω√¿¸«“¡ˆø° øµ«‚¿ª ¡ÿ¥Ÿ."
    , NULL},
  {0,12,8,8,
    "πŒ√∏ : ¿¸≈ı, ≈ı√¥, ªÁ∞›, ∞¯∞›»∏««,"
    "µµ¿˚∞Ëø≠ ±‚º˙¿« ªÁøÎø° ¿˚øÎ"
    , NULL},
  {0,12,9,9,
    "º“¡ˆ±› :  º“¡ˆ±›¿∫ Ω∫ƒ⁄æÓ∑Œ ∞ËªÍµ«¥¬ ∞Õ∞˙ µøΩ√ø° ∞°∞‘ø°º≠ ºÓ«Œ«“∂ß ªÁøÎ"
    , NULL},
  {0,16,10,10, "∑π∫ß, ∞Ê«Ëƒ°, πÃªÁøÎ ∞Ê«Ëƒ°(øÏ≈¨∏Ø¿∏∑Œ ªÛºº∫∏±‚)",
    "stat_exp"},
  {0,12,11,11, "¥¯¡Ø ≥ªø°º≠¿« ¿ßƒ°", NULL},
  {0,12,12,12, "¿Â∫Ò«œ∞Ì ¿÷¥¬ π´±‚(øÏ≈¨∏Ø¿∏∑Œ ªÛºº∫∏±‚)", "stat_wep"},
  {-1, -1, -1, -1, NULL, NULL}
#else
...
int handle_mouse_button(int mx, int my, int button, bool shift, bool ctrl)
{
    ...
    if (mode == MOUSE_IN_STAT && keyin_mode == KEYIN_MODE_COMMAND)
    {
        ...
        if (Tips[i].tipfilename)
        {
            char fname[256];
#ifdef JP
            snprintf(fname, 250, "tips_k/%s.txt", Tips[i].tipfilename);
#else
...
#define DELIMITER_MORE "...(more)...                                                                   "
    ...
    while(1)
    {
        ...
        gotoxy(14, max);
#ifdef JP
        cprintf("[j/k/+/-/SPACE/b: ≥—±‚±‚  q/ESC/RETURN: ¡æ∑·]");
#else
