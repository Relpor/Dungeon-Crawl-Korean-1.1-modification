...
#ifdef JP // 테스트시 확인바람 (바깡) - 요걸 뭘로 번역해야할 지 애매모호 하군요. 
                   ...
                   bool quiet )
{
    ...
    if (ench == ENCH_FEAR)
    {
        if (!quiet)
#ifdef JP 
            simple_monster_message(mon, "은(는) 용기를 되찾은 것 같다.");
#else
    ...
    if (ench == ENCH_CONFUSION)
    {
        if (!quiet)
#ifdef JP 
            simple_monster_message(mon, "은(는) 덜 혼란스러운 것 같다.");
#else
    ...
    if (ench == ENCH_CHARM)
    {
        if (!quiet)
#ifdef JP 
            simple_monster_message(mon, "은(는) 매혹에서 빠져나왔다.");
#else
    ...
    if (ench == ENCH_BACKLIGHT_I)
    {
        if (!quiet)
#ifdef JP 
            simple_monster_message(mon, "의 찬란함이 사그러든다.");
#else
    ...
    if (ench == ENCH_STICKY_FLAME_I || ench == ENCH_YOUR_STICKY_FLAME_I)
    {
        if (!quiet)
#ifdef JP 
            simple_monster_message(mon, "을(를) 태우고 있던 불길이 사라졌다.");
#else
    ...
    if (ench == ENCH_POISON_I || ench == ENCH_YOUR_POISON_I)
    {
        if (!quiet)
#ifdef JP 
            simple_monster_message(mon, "은(는) 상태가 회복된 것 같다.");
#else
    ...
    if (ench == ENCH_YOUR_ROT_I)
    {
        if (!quiet)
#ifdef JP 
            simple_monster_message(mon, "의 부패는 멈추었다.");
#else
