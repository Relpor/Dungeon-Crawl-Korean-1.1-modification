...
#ifdef JP
    simple_monster_message(monster, "은(는) 경련을 일으켰다!");
#else
...
void direct_effect(struct bolt &pbolt)
{
    ...
    case DMNBM_HELLFIRE:
#ifdef JP
        mpr( "당신은 헬파이어의 폭발에 휘말렸다!" );
        strcpy( pbolt.beam_name, "헬파이어" );
#else
    ...
    case DMNBM_SMITING:
        ...
        strcpy( pbolt.beam_name, "일격" );    // for ouch
        pbolt.aux_source = "천벌의 일격";
#else
...
void mons_direct_effect(struct bolt &pbolt, int i)
{
    ...
    case DMNBM_HELLFIRE:
#ifdef JP
        simple_monster_message(monster, "은(는) 헬파이어 휘말렸다.");
        strcpy(pbolt.beam_name, "헬파이어");
#else
    ...
    case DMNBM_SMITING:
#ifdef JP
        simple_monster_message(monster, "은(는) 일격을 당했다.");
        strcpy(pbolt.beam_name, "일격");
#else
...
#ifdef JP
        strcat(info, "(이)가 분노로 울부짖는 소리를 들었다!");
#else
        strcat(info, "!");
#endif
        mpr(info);
...
#ifdef JP
            strcat(info, (temp_rand == 0) ? "커피 냄새를 맡았다."      :
                         (temp_rand == 1) ? "짠 소금 냄새를 맡았다."          :
                         (temp_rand == 2) ? "머리카락이 불타는 냄새를 맡았다."      :
                         (temp_rand == 3) ? "갓 구운 빵의 냄새를 맡았다.":
                         (temp_rand == 4) ? "무엇인가 기묘한 냄새를 맡았다."      :
                         (temp_rand == 5) ? "젖은 양모의 냄새를 맡았다."    :
                         (temp_rand == 6) ? "유황의 냄새를 맡았다."
                                          : "불과 유황의 냄새를 맡았다!");
#else
...
#ifdef JP
        strcat(info, (temp_rand == 0) ? "귀가 가려워졌다."   :
                     (temp_rand == 1) ? "머리가 아파왔다!"
                                      : "코가 갑자기 씰룩씰룩거렸다!");
#else
...
#ifdef JP
        strcat(info, (temp_rand == 0) ? "노래의 한 소절을 들었다"                       :
                     (temp_rand == 1) ? "누군가가 다른 사람을 부르는 소리를 들었다"             :
                     (temp_rand == 2) ? "매우 기묘한 소리를 들었다"               :
                     (temp_rand == 3) ? "불길이 이글거리는 소리를 들었다"                     :
                     (temp_rand == 4) ? "전에 들어본적 없는 기묘한 소음을 들었다"       :
                     (temp_rand == 5) ? "먼 곳에서 종이 울리는 소리를 들었다 "               :
                     (temp_rand == 6) ? "야크가 큰 소리로 우는 것을 들었다"         :
                     (temp_rand == 7) ? "뭔가 깨부숴지는 소리를 들었다"
                                      : "거대한 종이 울리는 소리를 들었다");
        strcat(info, ".");
#else
...
bool acquirement(unsigned char force_class)
{
    ...
    if (force_class == OBJ_RANDOM)
    {
      ...
      query:
#ifdef JP
        mpr( "[a|A] 무기    [b|B] 방어구    [c|C] 장신구         [d|D] 책  " );
        mpr( "[e|E] 지팡이      [f|F] 음식    [g|G] 보물           [h|H] 금화" );
#else
...
#ifdef JP
        mpr("어떤 종류의 아이템을 원합니까?", MSGCH_PROMPT);
#else
...
*/
    ...
    {
#ifdef JP
        mpr("당신은 첨벙하는 소리를 들었다.");      // how sad (and stupid)
#else
    ...
    else
    {
        ...
        {
#ifdef JP
            mpr("무한한 허공의 악마가 당신을 향해 미소짓고 있다.");
#else
...
void yell(void)
{
...
#ifdef JP
    mpr("뭐라고 말합니까?", MSGCH_PROMPT);
    mpr(" ! - 고함친다.");
    mpr(" a - 아군에게 몬스터를 공격하도록 명령한다.");
#else
    ...
    if (!(you.prev_targ == MHITNOT || you.prev_targ == MHITYOU))
    {
        ...
        {
#ifdef JP
            mpr(" p - 아군에게 당신과 같은 목표를 공격하도록 명령한다.");
#else
