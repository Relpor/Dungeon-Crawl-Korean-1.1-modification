...
char in_a_shop( char shoppy, char id[4][50] )
{
...
#ifdef JP
    snprintf( info, INFO_SIZE, "%s¿¡ ¿À½Å °ÍÀ» È¯¿µÇÕ´Ï´Ù!",
#else
  ...
  print_stock:
    ...
    {
#ifdef JP
        shop_print("¹Ì¾ÈÇÏ±º. ³» °¡°Ô¿¡´Â ÀÌÁ¦ Àç°í°¡ ¾ø¾î.", 20);
#else
...
#   endif
...
#ifdef JP
    shop_print("[a~]:¾ÆÀÌÅÛ ±¸ÀÔ, [x/Esc]:³ª°¡±â, [?/*]:¼ÒÁöÇ° È®ÀÎ, [v]:»óÇ°ÀÇ »ó¼¼ ¼³¸í", 23);
#else
  ...
  purchase:
#ifdef JP
    snprintf( info, INFO_SIZE, "´ç½ÅÀº %d°³ÀÇ ±ÝÈ­¸¦ °¡Áö°í ÀÖ´Ù.", you.gold );
#else
    ...
    textcolor(CYAN);
#ifdef JP
    shop_print("¾î¶² ¹°°ÇÀ» ±¸ÀÔÇÏ½Ã°Ú½À´Ï±î?", 20);
#else
    ...
    if (ft == 'v')
    {
        textcolor(CYAN);
#ifdef JP
        shop_print("¾î¶² ¾ÆÀÌÅÛÀ» Á¶»çÇÕ´Ï±î?", 20);
#else
        ...
        {
#ifdef JP
            shop_print("À¯°¨ÀÌÁö¸¸, ±×·± »óÇ°Àº ³õ¾ÆµÎ°í ÀÖÁö ¾Ê¾Æ.", 20);
#else
    ...
    {
#ifdef JP
        shop_print("¹Ì¾ÈÇÏÁö¸¸, µ·ÀÌ ºÎÁ·ÇØ.", 20);
#else
...
#ifdef JP
    strcat(st_pass, "À»(¸¦) ±¸ÀÔÇÏ½Ã°Ú½À´Ï±î? [y/n]");
#else
    strcat(st_pass, "? [y/n]");
#endif
...
static void purchase( int shop, int item_got, int cost )
{
    ...
    {
#ifdef JP
        snprintf( info, INFO_SIZE, "´õ µé°í ´Ù´Ò ¼ö ¾øÀ» °Í °°À¸´Ï±î, °¡°Ô ¹Ù±ù¿¡ ¿Å°Ü ³õÀ»°Ô." );
#else
...
#ifdef JP
    strcpy(sh_name, "¡º");
    //env.shop[shoppy].keeper_name[0]‚Í1`200
...
#ifdef JP
                     || shop_type == SHOP_WEAPON_ANTIQUE) ? "¹«±â" :
                    (shop_type == SHOP_ARMOUR
                     || shop_type == SHOP_ARMOUR_ANTIQUE) ? "¹æ¾î±¸" :

                    (shop_type == SHOP_JEWELLERY)         ? "º¸¼®" :
                    (shop_type == SHOP_WAND)              ? "¸¶¹ýºÀ" :
                    (shop_type == SHOP_BOOK)              ? "Ã¥" :
                    (shop_type == SHOP_FOOD)              ? "½Ä·áÇ°" :
                    (shop_type == SHOP_SCROLL)            ? "¸¶¹ý µÎ·ç¸¶¸®" :
                    (shop_type == SHOP_GENERAL_ANTIQUE)   ? "°ñµ¿Ç° °¡°Ô" :
                    (shop_type == SHOP_DISTILLERY)        ? "¾çÁ¶Àå" :
                    (shop_type == SHOP_GENERAL)           ? "ÀâÈ­Á¡"
                                                          : "¹ö±× °¡°Ô");
#else
