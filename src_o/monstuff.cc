...
void monster_die(struct monsters *monster, char killer, int i)
{
...
#ifdef JP
            mpr( "당신은 분노의 끓어오름과 함께 몸안에 차오르는 트로그의 힘을 느꼈다.",
#else
    ...
    {
#ifdef JP
        simple_monster_message( monster, "은(는) 산산조각 났다!", MSGCH_MONSTER_DAMAGE,
#else
    ...
    {
#ifdef JP
        simple_monster_message( monster, "은(는) 증발했다!", MSGCH_MONSTER_DAMAGE,
#else
    ...
    {
#ifdef JP
        simple_monster_message(monster, "은(는) 하늘에서 떨어졌다.",
#else
    ...
    else
    {
        ...
        case KILL_YOU_MISSILE:  /* You kill by missile or beam. */
            ...
            strcat(info, (wounded_damaged(monster->type)) ? "을(를) 파괴했다" : "을(를) 죽였다");
            strcat(info, "!");
#else
            strcpy(info, "You ");
...
#ifdef JP
            simple_monster_message(monster, "은(는) 죽었다!", MSGCH_MONSTER_DAMAGE,
#else
...
#ifdef JP
                mpr("당신은 갑자기 불안함을 느꼈다...", MSGCH_MONSTER_SPELL);
#else
...
#ifdef JP
                            random2avg(88, 3), 100, "미라의 죽음의 저주" );
#else
...
#ifdef JP
            simple_monster_message( monster,
                    (tmp == 0) ? "은(는) 말했다. \"이것이 나의 최후는 아니야!\"" :
                    (tmp == 1) ? "은(는) 말했다. \"다음 기회에 너를 잡아주지!\"" :
                    (tmp == 2) ? "은(는) 말했다. \"이것을 마지막이라고 생각하지마!\"" :
                    (tmp == 3) ? "은(는) 말했다. \"다시 돌아오겠다!\"" :
                    (tmp == 4) ? "은(는) 말했다. \"이것이 마지막은 아니다. 이것은 시작에 지나지 않는다!\"" :
                    (tmp == 5) ? "은(는) 말했다. \"나를 죽였다고? 나는 그렇게 생각치 않네!\""
                               : "은(는) 말했다. \"그렇게 간단하게 나를 패배시킨 것이라고 생각치 마라!\"",
                                    MSGCH_TALK );
#else
            simple_monster_message( monster,
    ...
    if (killer != KILL_RESET)
    {
        ...
        if (mons_has_ench(monster, ENCH_ABJ_I, ENCH_ABJ_VI))
        {
            if (mons_weight(mons_charclass(monster->type)))
            {
                ...
                {
#ifdef JP
                    simple_monster_message( monster, "은(는) 증발했다!" );
#else
...
#ifdef JP
    if (!simple_monster_message(parent, "은(는) 둘로 나뉘어졌다!"))
#else
...
bool monster_polymorph( struct monsters *monster, int targetc, int power )
{
    ...
    if (targetc == RANDOM_MONSTER)
    {
        do
        {
            ...
            if (relax > 50)
#ifdef JP
                return (simple_monster_message( monster, "은(는) 격렬하게 몸부림 쳤다." ));
#else
    ...
    bool invis = mons_flag( targetc, M_INVIS )
        ...
        if (invis && !player_see_invis())
            strcat( str_polymon, "은(는) 무엇인가 눈으로 볼 수 없는 존재" );
        else{strcat( str_polymon, "은(는) " );
        ...
        if (mons_has_ench( monster, ENCH_GLOWING_SHAPESHIFTER, ENCH_SHAPESHIFTER ))
            strcat( str_polymon, "(으로) 변신했다!" );
        else if (targetc == MONS_PULSATING_LUMP)
            strcat( str_polymon, "(으)로 퇴화했다!" );
        else
            strcat( str_polymon, "(으)로 변화했다!" );
#else
    if (mons_has_ench( monster, ENCH_GLOWING_SHAPESHIFTER, ENCH_SHAPESHIFTER ))
...
#ifdef JP
        simple_monster_message( monster, "은(는) 저항했다." );
#else
...
void print_wounds(struct monsters *monster)
{
    ...
    {
#ifdef JP
        strcat(str_wound, "이제 곧 ");
        strcat(str_wound, wounded_damaged(monster->type) ? "파괴될 것 같다"
                                                         : "죽을 것 같다");
#else
...
#ifdef JP
        strcat(str_wound, wounded_damaged(monster->type) ? "손상을 입고 있다"
                                                         : "상처를 입고 있다");
#else
...
#ifdef JP
            simple_monster_message(mon, "은(는) 싸우기로 결심했다!");
#else
    ...
    for (int p = 0; p < NUM_MON_ENCHANTS && !died; p++)
    {
...
#ifdef JP
                simple_monster_message( monster, "은(는) 독 데미지를 받았다.",
#else
                simple_monster_message( monster, " takes poison damage.",
#endif
                                        MSGCH_DIAGNOSTICS );
#ifdef JP
                snprintf( info, INFO_SIZE, "독 데미지: %d", dam );
#else
        ...
        case ENCH_YOUR_STICKY_FLAME_IV:
            ...
            if (dam > 0)
            {
                hurt_monster( monster, dam );
#ifdef JP
                simple_monster_message(monster, "은(는) 화상을 입었다!");
#else
...
#if DEBUG_DIAGNOSTICS
#ifdef JP
                snprintf( info, INFO_SIZE, "끈적이는 화염 데미지: %d", dam );
#else
...
static void handle_nearby_ability(struct monsters *monster)
{
    ...
    case MONS_GIANT_EYEBALL:
        ...
        {
#ifdef JP
            simple_monster_message(monster, "은(는) 당신을 응시했다.");
#else
...
//    losight(show, grd, you.x_pos, you.y_pos);
    ...
    case MONS_LAVA_SNAKE:
        ...
        {
#ifdef JP
            simple_monster_message(monster, "은(는) 용암을 내뿜었다!");
#else
    ...
    case MONS_ELECTRICAL_EEL:
        ...
        {
#ifdef JP
            simple_monster_message(monster, "은(는) 전기볼트를 쏘았다!");
#else
    ...
    case MONS_FIEND:
        ...
        if (one_chance_in(4))
        {
            ...
            case 3:
                ...
                {
#ifdef JP
                    simple_monster_message( monster, "은(는) 손짓하기 시작했다!",
#else
    ...
    case MONS_KILLER_KLOWN:
        ...
        {
#ifdef JP
            simple_monster_message(monster, "이(가) 사라졌다.");
#else
    ...
    case MONS_MANTICORE:
        ...
        // set up the beam
#ifdef JP
        strcpy(beem.beam_name, "뾰족한 못의 연사");
#else
        ...
        {
#ifdef JP
            simple_monster_message(monster, "은(는) 꼬리를 채찍처럼 휘둘렀다!");
#else
    ...
    case MONS_XTAHUA:
            ...
            || one_chance_in(10))
        {
            ...
            {
#ifdef JP
                simple_monster_message(monster, "은(는) 브레스를 뿜었다.");
#else
...
static bool handle_potion(struct monsters *monster, bolt & beem)
{
    ...
    else
    {
        ...
        case POT_HEAL_WOUNDS:
            ...
            {
#ifdef JP
                simple_monster_message(monster, "은(는) 물약을 마셨다.");
#else
...
#ifdef JP
                    simple_monster_message(monster, "은(는) 치료되었다!");
#else
    ...
    else
    {
        ...
        case SCR_TELEPORTATION:
            if (!mons_has_ench(monster, ENCH_TP_I))
            {
                ...
                {
#ifdef JP
                    simple_monster_message(monster, "은(는) 마법 두루마리를 읽었다.");
#else
...
#ifdef JP
                    simple_monster_message(monster, "은(는) 마법 두루마리를 읽었다.");
                    simple_monster_message(monster, "은(는) 사라졌다!");
#else
                    simple_monster_message(monster, " reads a scroll.");
                    simple_monster_message(monster, " blinks!");
#endif
...
static bool handle_wand(struct monsters *monster, bolt &beem)
{
    ...
    else if (coinflip())
    {
        ...
        {
#ifdef JP
            if (!simple_monster_message(monster, "은(는) 마법봉을 사용했다."))
#else
...
static bool handle_spell( struct monsters *monster, bolt & beem )
{
    ...
    else
    {
        ...
        if (monsterNearby)      // handle monsters within range of player
        {
...
#ifdef JP
                simple_monster_message( monster, "은(는) 훌륭한 은 나팔을 연주했다.",
#else
            ...
            {
#ifdef JP
                simple_monster_message( monster, "은(는) 손짓했다.",
#else
            ...
            else
            {
                ...
                case MONS_IRON_DRAGON:
#ifdef JP
                    if (!simple_monster_message(monster, "은(는) 브레스를 뿜었다.",
#else
                                                ...
                                                MSGCH_MONSTER_SPELL))
                    {
                        ...
                        {
#ifdef JP
                            mpr("울림 소리를 들었다.", MSGCH_MONSTER_SPELL);
#else
...
#ifdef JP
                    simple_monster_message(monster, "은(는) 응시했다.", MSGCH_MONSTER_SPELL);
#else
...
#ifdef JP
                    simple_monster_message(monster, "의 심장이 고동쳤다.",
#else
...
#ifdef JP
                    simple_monster_message(monster, "은(는) 독을 벹었다.",
#else
...
#if DEBUG_DIAGNOSTICS
#ifdef JP
            snprintf( info, INFO_SIZE, "%s은(는) 다음 행동을 위해 움직임을 멈추었다.",
...
static void mons_in_cloud(struct monsters *monster)
{
...
#ifdef JP
        cprintf("치명적 오류: 몬스터가 존재하지않는 구름속에 있다.");
#else
...
#ifdef JP
        simple_monster_message(monster, "은(는) 화염에 휩싸였다!");
#else
    ...
    case CLOUD_STINK_MON:
#ifdef JP
        simple_monster_message(monster, "은(는) 독가스에 말려들었다!");
#else
    ...
    case CLOUD_COLD_MON:
#ifdef JP
        simple_monster_message(monster, "은(는) 냉각 증기에 말려들었다!");
#else
    ...
    case CLOUD_POISON_MON:
#ifdef JP
        simple_monster_message(monster, "은(는) 독안개에 말려들었다!");
#else
...
#ifdef JP
        simple_monster_message(monster, "은(는) 스팀 증기에 말려들었다!");
#else
...
#ifdef JP
        simple_monster_message(monster, "은(는) 암흑의 독기에 말려들었다!");
#else
