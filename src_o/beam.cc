...
#if DEBUG_DIAGNOSTICS
#ifdef JP
    snprintf( info, INFO_SIZE, "zapping:  power=%d", power ); // -_-?..
#else
...
static void zappy( char z_type, int power, struct bolt &pbolt )
{
...
#ifdef JP
        strcpy(pbolt.beam_name, "¿¸∞›"); // ∏∂π˝∫¿ πﬂµø«œ¥¬ ∏ª¿Œ ∞Õ ∞∞¿∫µ•... 70%ø°¥¬ '¿¸∞›' ¿Ã∂Û≥◊ø‰;πª∑Á π¯ø™«“¡ˆ ¿ﬂ ∏∏£∞⁄Ω¿¥œ¥Ÿ.
#else
...
#ifdef JP
        strcpy(pbolt.beam_name, (temp_rand == 0) ? "∏∑°∏’¡ˆ" :
                                (temp_rand == 1) ? "»Î∏’¡ˆ" :
                                (temp_rand == 2) ? "¿⁄∞•" : "∏∑°");
        strcat(pbolt.beam_name, "¿« ∆¯«≥");
#else
...
#ifdef JP
        strcpy(pbolt.beam_name, coinflip() ? "æœºÆ ∆¯«≥" : "æœºÆ¿« ∆¯«≥"); // ±◊≥… ¥Ÿ∏£∞‘ «œ∑¡∞Ì.... ∞∞∞‘ «œ¥¬∞‘ ≥™¿∏∑¡≥™ø‰?
#else
...
#ifdef JP
        strcpy(pbolt.beam_name, "»≠ø∞ µ¢æÓ∏Æ");
#else
...
#ifdef JP
        strcpy(pbolt.beam_name, "æœºÆ¿« »≠ªÏ");
#else
...
#ifdef JP
        strcpy(pbolt.beam_name, "≤ˆ¿˚¿Ã¥¬ »≠ø∞");        // extra damage
#else
...
#ifdef JP
        strcpy(pbolt.beam_name, "æÛ¿Ω¿« »≠ªÏ");
#else
...
#ifdef JP
        strcpy(pbolt.beam_name, "¿Ωø°≥ ¡ˆ¿« »≠ªÏ"); // "∫Œ¡§¿˚¿Œ" ¿∫ ¡ª ¿ÃªÛ«œ≥◊ø‰,.
#else
...
#ifdef JP
        strcpy(pbolt.beam_name, "«Ô∆ƒ¿ÃæÓ");
#else
...
#ifdef JP
        strcpy(pbolt.beam_name, "µ∂æ◊¿« π∞∫∏∂Û");
#else
    ...
    case ZAP_BREATHE_FROST:     // cap 50
        // max power = lev = 27
#ifdef JP
        strcpy(pbolt.beam_name, "µø∞·¿« ∫Í∑πΩ∫");
#else
...
#ifdef JP
        strcpy( pbolt.beam_name, "µπˆ±Î ±§º±" );
#else
...
void fire_beam( struct bolt &pbolt, item_def *item )
{
    ...
    snprintf( info, INFO_SIZE, "%s%s (%d,%d) to (%d,%d): ty=%d col=%d flav=%d hit=%d dam=%dd%d",
             (pbolt.isBeam) ? "beam" : "missile", // dio. π¯ø©±‚ æ»µ«æÓ¿÷¥¬µ•.. µπˆ±Î«“∂ß æ≤¥¬ ∆ƒ∆Æ¿ŒµÌ.. ¿ßø°√≥∑≥;
                           ...
                           int hurted, bool doFlavouredEffects )
{
    ...
    case BEAM_FIRE:
...
#ifdef JP
                simple_monster_message(monster, "ø°∞‘ æ∆π´∑± º’ªÛ¿ª ¿‘»˜¡ˆ ∏¯«ﬂ¥Ÿ.");
#else
        ...
        else if (resist == 1)
        {
            if (doFlavouredEffects)
#ifdef JP
                simple_monster_message(monster, "¿∫(¥¬) ¿˙«◊«ﬂ¥Ÿ.");
#else
        ...
        else if (resist < 0)
        {
                ...
                || monster->type == MONS_SIMULACRUM_LARGE)
            {
                if (doFlavouredEffects)
#ifdef JP
                    simple_monster_message(monster, "¿∫(¥¬) ≥Ïæ∆≥ª∏Æ±‚ Ω√¿€«ﬂ¥Ÿ!");
#else
            ...
            else
            {
                if (doFlavouredEffects)
#ifdef JP
                    simple_monster_message(monster, "¿∫(¥¬) ∫“≈∏ø√∂Û ƒø¥Ÿ∂ı º’ªÛ¿ª ¿‘æ˙¥Ÿ!"); //dio. æ‡∞£ πŸ≤ÂΩ¿¥œ¥Ÿ..
#else
    ...
    case BEAM_COLD:
        ...
        else if (resist < 0)
        {
            if (doFlavouredEffects)
#ifdef JP
                simple_monster_message(monster, "¿∫(¥¬) æÛæÓ∫ŸæÓ ƒø¥Ÿ∂ı º’ªÛ¿ª ¿‘æ˙¥Ÿ!");
#else
    ...
    case BEAM_POISON_ARROW:
        if (mons_res_poison(monster) > 0)
        {
            ...
            {
#ifdef JP
                simple_monster_message( monster, "¿∫(¥¬) æ‡∞£ ¿˙«◊«ﬂ¥Ÿ." );
#else
...
#ifdef JP
            simple_monster_message(monster, "¿« ª˝∏Ì∑¬¿Ã »Ìºˆ¥Á«ﬂ¥Ÿ.");
#else
    ...
    if (pbolt.flavour == BEAM_LAVA)    //jmf: lava != hellfire
    {
        ...
        else if (resist < 0)
        {
            ...
            else
            {
                if (doFlavouredEffects)
#ifdef JP
                    simple_monster_message(monster, "¿∫(¥¬) ∫“≈∏ø√∂Û ƒø¥Ÿ∂ı º’ªÛ¿ª ¿‘æ˙¥Ÿ!");
#else
                    simple_monster_message(monster, " is burned terribly!");
#endif
            }
...
#ifdef JP
    else if (stricmp(pbolt.beam_name, "«Ô∆ƒ¿ÃæÓ") == 0)
#else
...
bool mass_enchantment( int wh_enchant, int pow, int origin )
{
    ...
    for (i = 0; i < MAX_MONSTERS; i++)
    {
        ...
        {
#ifdef JP
            simple_monster_message(monster, "¿∫(¥¬) æ∆π´∑± øµ«‚µµ πﬁ¡ˆ æ æ“¥Ÿ.");
#else
...
int mons_ench_f2(struct monsters *monster, struct bolt &pbolt)
{
    ...
    case BEAM_SLOW:         /* 0 = slow monster */
        ...
        {
#ifdef JP
            if (simple_monster_message(monster, "¿« ∫¸∏• øÚ¡˜¿”¿Ã ø¯∑°¥Î∑Œ µπæ∆ø‘¥Ÿ."))
#else
        ...
        if (mons_add_ench(monster, ENCH_SLOW))
        {
            ...
            // notice slow down.
#ifdef JP
            if (simple_monster_message(monster, "¿« øÚ¡˜¿”¿Ã ¥¿∑¡¡≥¥Ÿ."))
#else
    ...
    case BEAM_HASTE:                  // 1 = haste
        ...
        if (mons_add_ench(monster, ENCH_HASTE))
        {
            ...
            // notice speed up.
#ifdef JP
            if (simple_monster_message(monster, "¿« øÚ¡˜¿”¿Ã ª°∂Û¡≥¥Ÿ."))
#else
    ...
    case BEAM_HEALING:         /* 2 = healing */
        if (heal_monster( monster, 5 + roll_dice( pbolt.damage ), false ))
        {
            ...
            {
#ifdef JP
                if (simple_monster_message(monster, "¿« ªÛ√≥∞° æ‡∞£ »∏∫πµ«æ˙¥Ÿ."))
#else
...
#ifdef JP
        if (simple_monster_message(monster, "¿« øÚ¡˜¿”¿Ã ∞©¿⁄±‚ ∏ÿ√·¥Ÿ!"))
#else
...
#ifdef JP
            if (simple_monster_message(monster, "¿∫(¥¬) ¡§Ω≈¿ª ∏¯ ¬˜∏Æ∞Ì ¿÷¥Ÿ."))
#else
    ...
    case BEAM_INVISIBILITY:               /* 5 = invisibility */
        ...
        if (mons_add_ench(monster, ENCH_INVIS))
        {
            ...
            {
#ifdef JP
                snprintf( info, INFO_SIZE, "%s¿Ã(∞°) %s",
                          buff, player_see_invis() ? "¿·Ω√ ±Ù∫˝∞≈∏∞¥Ÿ."
                                                   : "±Ù∫˝∞≈∏Æ¥ı¥œ, ¿⁄√Î∏¶ ∞®√ﬂæ˙¥Ÿ!" );
#else
...
#ifdef JP
            if (simple_monster_message(monster, "¿∫(¥¬) ∏≈∑·µ«æ˙¥Ÿ."))
#else
...
#ifdef JP
        simple_monster_message(monster, "¿« ∏ˆ¿∫ ≤˙æÓø¿∏£¥¬ æ◊ªÛ »≠ø∞¿∏∑Œ µ⁄µ§ø© ¿÷¥Ÿ!");
#else
...
#ifdef JP
    if (strcmp(beam.beam_name, "µ∂∞°Ω∫ ∆¯«≥") == 0)
#else
...
#ifdef JP
    if (strcmp(beam.beam_name, "ø¿ø∞µ» ¡ı±‚") == 0)
#else
...
#ifdef JP
    if (strcmp(beam.beam_name, "¿¸±‚ ±∏√º") == 0
        || strcmp(beam.beam_name, "±›º” ±∏√º") == 0
        || strcmp(beam.beam_name, "∞≈¥Î«— ≥√±‚ ∆¯«≥") == 0)
#else
...
static int affect_place_clouds(struct bolt &beam, int x, int y)
{
    ...
    // ORB OF ENERGY
#ifdef JP
    if (strcmp(beam.beam_name, "ø°≥ ¡ˆ ±∏√º") == 0)
#else
    ...
    // BALL OF STEAM
#ifdef JP
    if (strcmp(beam.beam_name, "∂ﬂ∞≈øÓ ¡ı±‚¿« ±∏") == 0)
#else
    ...
    // STICKY FLAME
#ifdef JP
    if (strcmp(beam.beam_name, "≤ˆ¿˚¿Ã¥¬ »≠ø∞") == 0)
#else
...
#ifdef JP
    if (stricmp(beam.beam_name, "æÛ¿Ω ∆¯«≥øÏ") == 0)
#else
...
#ifdef JP
    if (stricmp(beam.beam_name, "æ«√Î¿« ±∏∏ß") == 0)
#else
...
#ifdef JP
    if (strcmp(beam.beam_name, "∞≈¥Î«— »≠ø∞ ∆¯«≥") == 0)
#else
...
#ifdef JP
    if (stricmp(beam.beam_name, "«Ô∆ƒ¿ÃæÓ") == 0)
#else
...
static int affect_player( struct bolt &beam )
{
    ...
    if (beam.beam_name[0] != '0')
    {
        if (!beam.isExplosion && !beam.aimedAtFeet)
        {
            ...
            if (beam.isBeam)
            {
                ...
                {
#ifdef JP
                    strcpy(info, "¥ÁΩ≈¿∫ "); // dio ¡ª πŸ≤ÂΩ¿¥œ¥Ÿ..
            ...
            else
            {
                        ...
                        && player_shield_class() > 0)
                {
                    ...
                    if (hit < block)
                    {
                        you.shield_blocks++;
#ifdef JP
                        snprintf( info, INFO_SIZE, "¥ÁΩ≈¿∫ %s¿ª(∏¶) ∏∑æ∆≥ªæ˙¥Ÿ.",
#else
    ...
    else
    {
...
#ifdef JP
            mpr("æÓ∫ÒΩ∫(Abyss)∑Œ ∫¸¡ÆµÈæ˙¥Ÿ!"); // dio Abyss ∂Û∞Ì æ»æ≤∞Ì æÓ∫ÒΩ∫∂Û∞Ì∏∏ «œ∏È ∫∞∑Œ ∆˜Ω∫∞° æ»¥¿≤∏¡Æº≠ø‰§–§–
#else
        ...
        case BEAM_PAIN:      // pain
...
#ifdef JP
            mpr("æˆ√ª≥≠ ∞Ì≈Î¿Ã ªƒº”±Ó¡ˆ Ω∫∏ÁµÁ¥Ÿ!"); // dio ±◊≥…...-_ -;;
#else
            ...
            if (!beam.aux_source)
#ifdef JP
                beam.aux_source = "Ω≈∞Ê±Ó¡ˆ ∆ƒ±´«œ¥¬ ±ÿΩ…«— ∞Ì≈Îø° ¿««ÿ"; // dio ∫–∏Ì æÓº¯¿Ã πŸ≤ÓæÓæﬂ «“ ∞Õ ∞∞¿∫µ•..;; æ’µ⁄ø° ππ∞° ¿÷¥¬¡ˆ ∏∏£∞⁄≥◊ø‰.
#else
        ...
        case BEAM_DISPEL_UNDEAD:
            ...
            if (!beam.aux_source)
#ifdef JP
                beam.aux_source = "æµ•µÂ ≈ƒ°ø° ¿««ÿ"; // dio. µ¸»˜ ∏¬¥¬ «—±π∏ª¿Ã æ»∂∞ø√∂Ûº≠.
#else
...
#ifdef JP
            mpr("º“«¡∆Æø˛æÓ πˆ±◊∞° ¥ÁΩ≈¿« πﬂ∞°∂Ù¿ª ∞¶æ∆∏‘¥¬¥Ÿ!"); // dio -_-.......
#else
...
#ifdef JP
    snprintf( info, INFO_SIZE, "%s¿Ã(∞°) ¥ÁΩ≈%s¥Ÿ!!", // dio æ‡∞£ ¿⁄ø¨Ω∫∑¥∞‘ πŸ≤ÂΩ¿¥œ¥Ÿ..
                beam.beam_name, (beam.isExplosion ? "¿ª µ§√∆" : "ø°∞‘ ∏Ì¡ﬂ«œø¥") );
#else
...
#if DEBUG_DIAGNOSTICS
#ifdef JP
    snprintf( info, INFO_SIZE, "Player damage: rolled=%d; after AC=%d", // dio -_-?
#else
    ...
    // poisoning
#ifdef JP
    if (strstr(beam.beam_name, "µ∂") != NULL // dio π∫¡ˆ ¿ﬂ ∏Ù∞⁄≥◊ø‰.
       && (strstr(beam.beam_name, "µ∂ƒß") == NULL)
#else
    ...
    {
#ifdef JP
        if (hurted || (strstr( beam.beam_name, "πŸ¥√" ) != NULL
#else
...
#ifdef JP
    if (beam.flavour == BEAM_LAVA || stricmp(beam.beam_name, "«Ô∆ƒ¿ÃæÓ") == 0)
#else
    ...
    // more complex (geez..)
#ifdef JP
    if (beam.flavour == BEAM_FIRE && strcmp(beam.beam_name, "∂ﬂ∞≈øÓ ¡ı±‚¿« ±∏") != 0)
#else
...
#if DEBUG_DIAGNOSTICS
#ifdef JP
    snprintf( info, INFO_SIZE, "Damage: %d", hurted ); // dio ????
#else
...
static int  affect_monster(struct bolt &beam, struct monsters *mon)
{
    ...
    if (beam.beam_name[0] == '0')
    {
        ...
        switch(ench_result)
        {
            case MON_RESIST:
#ifdef JP
                if (simple_monster_message(mon, "¿∫(¥¬) ¿˙«◊«ﬂ¥Ÿ."))
#else
            ...
            case MON_UNAFFECTED:
#ifdef JP
                if (simple_monster_message(mon, "¿∫(¥¬) æ∆π´∑± øµ«‚µµ πﬁ¡ˆ æ æ“¥Ÿ."))
#else
...
#ifdef JP
              "Monster: %s; Damage: pre-AC: %d; post-AC: %d; post-resist: %d", // dio ??
#else
    ...
    if (mons_near(mon))
    {
        ...
        if (player_monster_visible( &menv[tid] ))
            ...
            strcat(info, "π´æ˘¿Œ∞°");
        strcat(info, beam.isExplosion?"¿ª(∏¶) µ§√∆¥Ÿ":"ø°∞‘ ∏Ì¡ﬂ«ﬂ¥Ÿ");
#else
        strcpy(info, "The ");
        strcat(info, beam.beam_name);
    ...
    else
    {
        ...
        /* looks for missiles which aren't poison but
           are poison*ed* */
#ifdef JP
        if (strstr(beam.beam_name, "µ∂") != NULL
           && (strstr(beam.beam_name, "µ∂ƒß") == NULL)
#else
        if (strstr(beam.beam_name, "poison") != NULL
#endif
        ...
        {
#ifdef JP
            if (strstr(beam.beam_name, "πŸ¥√") != NULL
#else
...
static int affect_monster_enchantment(struct bolt &beam, struct monsters *mon)
{
...
#ifdef JP
        if (simple_monster_message(mon, "¿« ∏Ω¿¿Ã ¥Ÿº“ ∫“æ»¡§«ÿ ∫∏¿Œ¥Ÿ."))
#else
    ...
    if (beam.flavour == BEAM_BANISH)
    {
        ...
        {
#ifdef JP
            simple_monster_message(mon, "¿∫(¥¬) ¿·Ω√ ≤ﬁ∆≤∞≈∏∞¥Ÿ.");
#else
...
#ifdef JP
        if (simple_monster_message(mon, "¿∫(¥¬) ∞›∑ƒ«œ∞‘ ∞Ê∑√«ﬂ¥Ÿ!"))
#else
...
#if DEBUG_DIAGNOSTICS
#ifdef JP
        snprintf( info, INFO_SIZE, "HD: %d; pow: %d", // dio ??
#else
...
#ifdef JP
        simple_monster_message(mon, "¿∫(¥¬) ≥Îøπ»≠µ«æ˙¥Ÿ.");
#else
...
#ifdef JP
        if (simple_monster_message(mon, "¿∫(¥¬) ∞Ì≈Î¿∏∑Œ ∞›∑ƒ«œ∞‘ ∞Ê∑√«—¥Ÿ!"))
#else
...
#ifdef JP
        if (strstr( beam.beam_name, "∞Ì≈Î" ) != NULL)
#else
    ...
    {
#ifdef JP
        if (simple_monster_message(mon, "¿« ∏ˆ¿Ã ∆¯∆ƒ¥Á«ﬂ¥Ÿ."))
#else
...
#ifdef JP
        if (simple_monster_message(mon, "¿∫(¥¬) ¡π∑¡ ∫∏¿Œ¥Ÿ..."))
#else
...
#ifdef JP
    if (strcmp( beam.beam_name, "«Ô∆ƒ¿ÃæÓ" ) == 0)
#else
...
static void explosion1(struct bolt &pbolt)
{
...
#ifdef JP
    if (stricmp(pbolt.beam_name, "«Ô∆ƒ¿ÃæÓ") == 0)
#else
    if (stricmp(pbolt.beam_name, "hellfire") == 0)
#endif
    {
#ifdef JP
        seeMsg = "«Ô∆ƒ¿ÃæÓ∞° ∆¯πﬂ«ﬂ¥Ÿ!";
        hearMsg = "øÿ¡ˆ ∫“ƒË«— ∆¯πﬂ¿Ω¿Ã µÈ∏∞¥Ÿ.";
#else
...
#ifdef JP
    if (stricmp(pbolt.beam_name, "»≤±›ªˆ ∫“±Ê") == 0)
#else
...
#ifdef JP
    if (stricmp(pbolt.beam_name, "»≠ø∞±∏") == 0)
#else
...
#ifdef JP
    if (stricmp(pbolt.beam_name, "¿¸±‚ ±∏√º") == 0)
#else
...
#ifdef JP
    if (stricmp(pbolt.beam_name, "ø°≥ ¡ˆ ±∏√º") == 0)
#else
...
#ifdef JP
    if (stricmp(pbolt.beam_name, "±›º” ±∏√º") == 0)
#else
    if (stricmp(pbolt.beam_name, "metal orb") == 0)
#endif
    {
#ifdef JP
        seeMsg = "±›º” ±∏√º∞° ∆¯πﬂ«œø© ≥Øƒ´∑ŒøÓ ªÍ≈∫¿Ã ∏∂±∏ ≥Ø∏∞¥Ÿ!";
        hearMsg = "∆¯πﬂ¿Ω¿Ã µÈ∏∞¥Ÿ!";
#else
...
#ifdef JP
    if (stricmp(pbolt.beam_name, "∞≈¥Î«— ≥√±‚ ∆¯«≥") == 0)
#else
    if (stricmp(pbolt.beam_name, "great blast of cold") == 0)
#endif
    {
#ifdef JP
        seeMsg = "æÛ¿Ω ∆¯«≥¿Ã ∆¯πﬂ«œø© ∞≈¥Î«— æÛ¿Ω¿« ∆¯«≥øÏ∞° ¿œæÓ≥µ¥Ÿ!";
        hearMsg = "∆¯«≥øÏ∞° ∏Ùæ∆ƒ°¥¬ º“∏Æ∞° µÈ∏∞¥Ÿ!";
#else
...
#ifdef JP
        strcpy(pbolt.beam_name, "æÛ¿Ω ∆¯«≥øÏ");
#else
...
#ifdef JP
    if (stricmp(pbolt.beam_name, "¡ı±‚¿« ±∏") == 0)
#else
    if (stricmp(pbolt.beam_name, "ball of vapour") == 0)
#endif
    {
#ifdef JP
        seeMsg = "±∏√º∞° ∆€¡ˆ∏Èº≠ «¯ø¿Ω∫∑ØøÓ ±∏∏ß¿Ã µ«æ˙¥Ÿ!";
        hearMsg = "°∏«ªøÏøÌ°π «œ¥¬ º“∏Æ∞° µÈ∏∞¥Ÿ."; // Dio
        strcpy(pbolt.beam_name, "æ«√Î¿« ±∏∏ß");
#else
...
#ifdef JP
    if (stricmp(pbolt.beam_name, "π∞æ‡") == 0)
#else
    ...
    {
#ifdef JP
        seeMsg = "±§º±¿Ã ∆¯πﬂ«œø© º“«¡∆Æø˛æÓ πˆ±◊¿« ±∏∏ß¿Ã µ«æ˙¥Ÿ!";
        hearMsg = "«—º’¿∏∑Œ π⁄ºˆƒ°¥¬ º“∏Æ∞° µÈ∏∞¥Ÿ!";
#else
