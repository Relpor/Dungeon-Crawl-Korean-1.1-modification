...
void special_wielded(void)
{
    ...
    case SPWLD_SING:
        if (makes_noise)
        {
            ...
            strcat(info,
#ifdef JP 
                     (temp_rand ==  0) ? "짧은 곡을 허밍했다." :
                     (temp_rand ==  1) ? "기나긴 노래를 부르기 시작했다." :
                     (temp_rand ==  2) ? "노래했다." :
                     (temp_rand ==  3) ? "까다롭게 노래했다." :
                     (temp_rand ==  4) ? "멜로디를 울렸다." :
                     (temp_rand ==  5) ? "무서운 소음을 냈다." :
                     (temp_rand ==  6) ? "가락이 맞지 않게 노래했다." :
                     (temp_rand ==  7) ? "\"트랄랄라\" 라고 노래했다." :
                     (temp_rand ==  8) ? "쾌활하게 지~잉하고 울었다." :
                     (temp_rand ==  9) ? "지~잉하고 울었다." :
                     (temp_rand == 10) ? "돌연 째지는 소리를 냈다!" :
                     (temp_rand == 11) ? "높은 웃음소리를 냈다." :
                     (temp_rand == 12) ? "재잘거리듯이 노래했다." :
                     (temp_rand == 13) ? "아름다운 선율을 연주했다." :
                     (temp_rand == 14) ? "아름다운 음악을 연주했다." :
                     (temp_rand == 15) ? "까다로운 오케스트라의 화음을 냈다." :
                     (temp_rand == 16) ? "슬픈 듯 하게 푸념을 늘어놓았다." :
                     (temp_rand == 17) ? "챙겅챙겅 소리를 냈다." :
                     (temp_rand == 18) ? "종 같은 소리를 냈다." :
                     (temp_rand == 19) ? "슬픈 듯 울부짖었다." :
                     (temp_rand == 20) ? "음계의 연습을 시작했다." :
                     (temp_rand == 21) ? "선율이 풍부하고 경쾌하게 노래했다." :
                     (temp_rand == 22) ? "가락이 맞지 않는 허밍했다." :
                     (temp_rand == 23) ? "한숨을 쉬었다." :
                     (temp_rand == 24) ? "깊은 신음소리 같은 음악을 연주했다." :
                     (temp_rand == 25) ? "튀는 것 같은 음악을 연주했다." :
                     (temp_rand == 26) ? "돌연히 짧고 연속적인 음표로 노래했다." :
                     (temp_rand == 27) ? "\"야! 나는 「노래하는 검」이다!!\" 라고 말했다." :
                     (temp_rand == 28) ? "무엇인가를 속삭였다." :
                     (temp_rand == 29) ? "실없는 소리를 했다." :
                     (temp_rand == 30) ? "미친 사람처럼 헛소리를 했다."
                                       : "무엇인가 알수없는 말로 외쳤다.");
#else
...
bool evoke_wielded( void )
{
    ...
    case OBJ_MISCELLANY:
        ...
        case MISC_PORTABLE_ALTAR_OF_NEMELEX:
            ...
            {
#ifdef JP 
                mpr( "이미 이 층에는 충분한 수의 제단이 있다고 생각하지 않는가?" );
#else
...
#ifdef JP 
                mpr("이 아이템을 설치하기 위해서는 깨끗한 장소가 필요하다.");
#else
...
static bool efreet_flask(void)
{
    ...
    {
#ifdef JP 
        mpr( "... 그러자 거대한 이프리트가 튀어 나왔다." );
#else
        mpr( "...and a huge efreet comes out." );
#endif

#ifdef JP 
        mpr( (behaviour == BEH_FRIENDLY) ? "\"나를 해방시켜 주어서 고맙소!\""
                                         : "그것은 미친듯이 발광했다!" );
#else
...
#ifdef JP 
        mpr("원반이 잠시 빛났지만 곧 어두워졌다.");
#else
        mpr("The disc glows for a moment, then fades.");
#endif
    else if (random2(100) < fail_rate)
#ifdef JP 
        mpr("작은 번개 불꽃이 원반 주위에서 파지직 거렸다.");
#else
...
void tome_of_power(char sc_read_2)
{
    ...
    {
#ifdef JP 
        mpr("이 페이지는 글씨가 너무 흐려져서 읽을 수 없다.");
#else
...
#ifdef JP 
    mpr("자신도 모르게 마법 주문을 외우고 있는걸 알았다!");
#else
