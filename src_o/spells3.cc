...
void cast_selective_amnesia(bool force)
{
    ...
    if (you.spell_no == 0)
#ifdef JP
        mpr("¥ÁΩ≈¿∫ æÓ∂≤ ¡÷πÆµµ æÀ¡ˆ ∏¯«—¥Ÿ.");      // re: sif muna {dlb}
#else
    ...
    else
    {
        ...
        {
#ifdef JP
            mpr( "æÓ∂≤ ¡÷πÆ¿ª ¿ÿΩ¿¥œ±Ó? ([*][?] ¿œ∂˜ [ESC] √Îº“)", MSGCH_PROMPT );
#else
...
#ifdef JP
                    mpr( "¡÷πÆ¿Ã «ÿ¡¶µ«æÓ, ¡°¿Ø¥Á«œ∞Ì ¿÷æ˙¥¯ ¥ÁΩ≈¿« ø°≥ ¡ˆ∞° "
                         "µπæ∆ø‘¥Ÿ." );
#else
...
bool remove_curse(bool suppress_msg)
{
    ...
    if (!suppress_msg)
    {
        if (success)
#ifdef JP
            mpr("¥ÁΩ≈¿∫ π´æ∞° ¿ß¥Î«— ¡∏¿Áø°∞‘ µµøÚ¿ª πﬁ¿∫ ∞Õ ∞∞¿∫ ±‚∫–¿Ã µÈæ˙¥Ÿ.");
#else
...
#ifdef JP
        mpr("«ÿ∞Ò¿Ã ∆¯πﬂ«œø© ≥Øƒ´∑ŒøÓ ª¿¿« ∆ƒ∆Ì¿ª »ª—∑»¥Ÿ!");
#else
...
void sublimation(int power)
{
    ...
    {
#ifdef JP
        mpr("¥ÁΩ≈¿Ã º’ø° µÁ ∞Ì±Íµ¢æÓ∏Æ¥¬ ∆º≤¯¿Ã µ«æÓ ∫ÿ±´µ«æ˙¥Ÿ. ");
        mpr("∞›∑˘«œ¥¬ ∏∂π˝ ø°≥ ¡ˆ∞° ¥ÁΩ≈¿« ¡§Ω≈ø° »Í∑ØµÈæ˙¥Ÿ!");
#else
...
void simulacrum(int power)
{
                ...
                && you.inv[ chunk ].sub_type == FOOD_CHUNK)))
    {
        ...
        {
#ifdef JP
            strcpy( info, (summoned == 1) ? "æÛ¿Ω¿« «¸ªÛ¿Ã "
                                          : "∏Ó ∞≥¿« æÛ¿Ω¿« «¸ªÛ¿Ã " );
            strcat( info, "¥ÁΩ≈ æ’ø°º≠ ∏æÁ¿ª ∞Æ√ﬂæ˙¥Ÿ." );
#else
    ...
    {
#ifdef JP
        mpr( "¿Ã ¡÷πÆ¿« »ø∞˙∏¶ ¿ÃøÎ«œ∑¡∏È, ¥ÁΩ≈¿∫ «— µ¢æÓ∏Æ¿« ∞Ì±‚∏¶ "
             "º’ø° ¡„∞Ì ¿÷æÓæﬂ «—¥Ÿ!" );
#else
...
void you_teleport(void)
{
    ...
    else
    {
        ...
        {
#ifdef JP
            mpr("¥ÁΩ≈¿Ã ¿¸¿Ã«œ±‚ ¿ß«ÿº≠¥¬ ¥Ÿº“ Ω√∞£¿Ã « ø‰«“ ∞Õ ∞∞¥Ÿ...");
#else
...
void you_teleport2( bool allow_control, bool new_abyss_area )
{
    ...
    {
#ifdef JP
        mpr("¥ÁΩ≈¿∫ ∏Ò«•«œ¥¬ ¿Âº“∏¶ ¡ˆ¡§«“ ºˆ ¿÷Ω¿¥œ¥Ÿ. ([.] ∂«¥¬ [DEL]∑Œ ∞·¡§)");
        mpr("¥Ÿ∏∏ ¥Ÿº“∞£¿« ¬˜¿Ã∞° øπªÛµÀ¥œ¥Ÿ.");
#else
...
//!!!!
        //äKÉfÅ[É^ÇÃï¥é∏ÇµÇΩÉZÅ[ÉuÉfÅ[É^ÇégópÇµÇƒÇ¢ÇÈèÍçáÅAÇ±Ç±Ç≈ÉtÉäÅ[ÉYÇ∑ÇÈÅB
...
void cast_poison_ammo(void)
{
    ...
    if (set_item_ego_type( you.inv[ammo], OBJ_MISSILES, SPMSL_POISONED ))
    {
        ...
        strcpy(info, str_pass);
#ifdef JP
        strcat(info, (you.inv[ammo].quantity == 1) ? "¿∫(¥¬)" : "¿∫(¥¬)");
        strcat(info, " µ∂¿« ∏∑ø° æ∫øˆ¡≥¥Ÿ.");
#else
...
bool project_noise(void)
{
    ...
    if (!silenced( plox[0], plox[1] ))
    {
        ...
        if (!silenced( you.x_pos, you.y_pos ))
        {
            ...
            {
#ifdef JP
                snprintf( info, INFO_SIZE, "¥ÁΩ≈¿∫ %s¥ÁΩ≈¿« ¿Ã∏ß¿ª ∫Œ∏£¥¬ º“∏Æ∏¶ µÈæ˙¥Ÿ.",
                          (see_grid( plox[0], plox[1] ) ? "∏’ ∞˜ø°º≠ " : "") );
#else
...
#ifdef JP
            if (simple_monster_message(monster, "¿∫(¥¬) ∫“∑ØµÈø©¡≥¥Ÿ."))
#else
...
void portal(void)
{
    ...
    {
#ifdef JP
        mpr("¿Ã ¡÷πÆ¿∫ æ∆π´∞Õµµ æ¯¥¬ ∞¯∞£¿ª ¥ÎªÛ¿∏∑Œ «œ¡ˆ æ ¿∏∏È æ» µ»¥Ÿ.");
#else
    ...
    else
    {
        // the first query {dlb}:
#ifdef JP
        mpr("æÓ¥¿ ¡ˆ¡°ø°? (<:¿ß∑Œ, >:æ∆∑°∑Œ, x:√Îº“)", MSGCH_PROMPT);
#else
        ...
        for (;;)
        {
            ...
            if (keyi == '<')
            {
                if (you.your_level == 0)
#ifdef JP
                    mpr("¥ÁΩ≈¿∫ ¿Ã ¡÷πÆ¿∏∑Œ ¿Ã∞Õ∫∏¥Ÿ ¿ß∑Œ ∞• ºˆ¥¬ æ¯¥Ÿ.");
#else
...
#ifdef JP
        mpr("√˛ ºˆ¥¬? (1 - 9, 'x'∑Œ √Îº“)", MSGCH_PROMPT);
#else
...
#ifdef JP
        mpr( "¥ÁΩ≈¿∫ Ω≈∫Ò¿˚¿Œ ∏∂π˝¿« πÆø° º˚æÓ µÈæÓ∞°º≠, ∞Ë¥‹¿« æ’ø°º≠ "
             "Ω«√º»≠µ«æ˙¥Ÿ." );
#else
...
bool cast_death_channel(int power)
{
    ...
    {
#ifdef JP
        mpr("ªÁæ««— »˚¿Ã ¥ÁΩ≈¿« ¡∏¿Á∏¶ ∞°µÊ √§øÏ∞Ì, «ÿπÊµ«±‚∏¶ ±‚¥Ÿ∏Æ∞Ì ¿÷¥Ÿ.");
#else
