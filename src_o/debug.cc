...
#ifdef JP
    mpr( "(»˘∆Æ: 'ø¿≈© ¡ª∫Ò' øÕ ∞∞¿Ã '∏∏µÈæÓ¡¯' ¿Ã∏ß¿∫ æ»µ )", MSGCH_PROMPT );
    mpr( "æÓ∂≤ ∏ÛΩ∫≈Õ?", MSGCH_PROMPT );
#else
...
#ifdef JP
    mpr( "æÓ∂≤ ¡æ¡∑¿Ã µ«∞Ì ΩÕΩ¿¥œ±Ó? " , MSGCH_PROMPT );
#else
...
{
#ifdef JP
    int spell = debug_prompt_for_int( "æÓ∂≤ ¡÷πÆ¿ª ø‹øÛ¥œ±Ó? (π¯»£∑Œ) ", true );
#else
...
#ifdef JP
    mpr( "æÓ∂≤ ¡÷πÆ¿ª ø‹øÛ¥œ±Ó? (¿Ã∏ß¿∏∑Œ) ", MSGCH_PROMPT );
#else
...
#ifdef JP
    mpr((one_chance_in(20)) ? "¥ÁΩ≈¿∫ ∏∂π˝ «–±≥ø° ¥ŸΩ√ ¥Ÿ≥‡æﬂ «“ ∞Õ ∞∞±∫ø‰."
                            : "±◊∑± ¡÷πÆ¿ª √£¡ˆ ∏¯«ﬂΩ¿¥œ¥Ÿ.");
#else
...
{
#ifdef JP
    int mon = debug_prompt_for_int( "æÓ∂≤ ∏ÛΩ∫≈Õ∏¶ ª˝º∫«’¥œ±Ó? (π¯»£∑Œ) ", true );
#else
...
void level_travel( int delta )
{
    ...
    {
#ifdef JP
        new_level = debug_prompt_for_int( "æÓ∂≤ ∑π∫ß∑Œ ¿Ãµø«’¥œ±Ó? ", true ) - 1;
#else
...
void create_spec_object(void)
{
    ...
    {
#ifdef JP
        int amount = debug_prompt_for_int( "∞ÒµÂ æÛ∏∂? ", true );
#else
...
// SWIN_LIST Ç…àÍóóÇï\é¶
        ...
        case OBJ_BOOKS:
            ...
            {
#ifdef JP
                special_wanted = debug_prompt_for_skill( "æÓ∂≤ ±‚º˙¿« º≥∏Ìº≠∏¶? " );
#else
...
#ifdef JP
    int item = prompt_invent_item( "æÓ∂≤ æ∆¿Ã≈€¿ª ∫Ò∆µ¥œ±Ó? ", -1 );
#else
...
#ifdef JP
            mpr( "a - «√∑ØΩ∫1  b - «√∑ØΩ∫2  c - Ω∫∆‰º»  d - ∞πºˆ  ESC - ¡æ∑·",
#else
            mpr( "a - plus  b - plus2  c - special  d - quantity  ESC - exit",
#endif
                 MSGCH_PROMPT );
#ifdef JP
            mpr( "æÓ∂≤ ¡æ∑˘? ", MSGCH_PROMPT );
#else
...
#ifdef JP
            snprintf( info, INFO_SIZE, "±‚¡∏¿« ∞°ƒ°: %d (0x%04x)", *ptr, *ptr );
#else
...
#ifdef JP
            snprintf( info, INFO_SIZE, "±‚¡∏¿« ∞°ƒ°: %ld (0x%08lx)", *ptr, *ptr );
#else
...
{
#ifdef JP /* ñÛïsópÅH */ // Dio ¿Ã ¿Ã»ƒ¿« µπˆ±Îø° ªÁøÎµ«¥¬µÌ«—(?) ∆ƒ∆Æ¥¬ Ω∫≈µ«’¥œ¥Ÿ-_-;;
    "None",
...
{
#ifdef JP
    int skill = debug_prompt_for_skill( "Ç«ÇÃÉXÉLÉãÇ≈Ç∑Ç©ÅH(ñºëOÇ≈ì¸óÕ) " );
#else
...
#ifdef JP
        int amount = debug_prompt_for_int( "âΩÉåÉxÉãÇ…ÇµÇ‹Ç∑Ç©ÅH ", true );
#else
...
void debug_set_all_skills(void)
{
    int i;
#ifdef JP
    int amount = debug_prompt_for_int( "ëSÉXÉLÉãÇâΩÉåÉxÉãÇ…ïœçXÇµÇ‹Ç∑Ç©ÅH ", true );
#else
...
bool debug_add_mutation(void)
{
...
#ifdef JP
    mpr( "Ç«ÇÃìÀëRïœàŸÇ≈Ç∑Ç©ÅH(ïœàŸälìæéûÇÃÉÅÉbÉZÅ[ÉWÇ≈éwíË) \n", MSGCH_PROMPT );
#else
    ...
    if (mutation == -1)
#ifdef JP
        mpr("I can't warp you that way!"); //∞®¿⁄∞°øµπÆ¿∏∑Œ
#else
    ...
    {
#ifdef JP
        snprintf( info, INFO_SIZE, "Found: %s", mutation_name( mutation, 1 ) ); //∞®¿⁄∞°øµπÆ¿∏∑Œ
#else
...
#ifdef JP
        int levels = debug_prompt_for_int( "How many levels? ", false ); //∞®¿⁄∞°øµπÆ¿∏∑Œ
#else
...
void debug_get_religion(void)
{
...
#ifdef JP
    mpr( "Ç«ÇÃê_ÇêMã¬ÇµÇ‹Ç∑Ç©ÅH(ñºëOÇ≈éwíË) ", MSGCH_PROMPT );
#else
    ...
    if (god == -1)
#ifdef JP
        mpr( "That god doesn't seem to be taking followers today." ); //∞®¿⁄∞°øµπÆ¿∏∑Œ
#else
...
{
#ifdef JP
    mpr("Oh dear. There appears to be a bug in the program."); //∞®¿⁄∞°øµπÆ¿∏∑Œ
    mpr("I suggest you leave this level then save as soon as possible."); //∞®¿⁄∞°øµπÆ¿∏∑Œ
#else
