...
static unsigned char spellbook_contents( item_def &book, int action )
{
    ...
    case RBOOK_MEMORIZE:
#ifdef JP
        cprintf( "기억할 주문을 선택해 주세요. (%d레벨분의 기억이 가능)" EOL,
                 spell_levels );
#else
...
#ifdef JP
        cprintf( "해설을 보고 싶은 주문을 선택해 주세요." EOL );
#else
...
#ifdef JP
    snprintf( info, INFO_SIZE, "당신은 %d레벨분의 주문을 기억할 수 있다.",
             avail_levels );
#else
...
#ifdef JP
    book = prompt_invent_item( "어떤 마법 두루마리에서 기억합니까?", OBJ_BOOKS );
#else
...
unsigned char read_book( item_def &book, int action )
{
    ...
    {
#ifdef JP
        mpr( "이 책의 내용은 현재의 당신의 이해력을 훨씬 뛰어넘고 있다." );
#else
...
bool learn_spell(void)
{
    ...
    {
#ifdef JP
        mpr("당신은 마법서 같은 것을 읽을 수 없다! 현재로서는 두루마리만 읽을 수 있다.");
#else
    ...
    if (player_spell_levels() < levels_needed)
    {
        redraw_screen();
#ifdef JP
        mpr("당신은 아직 그런 고레벨의 주문은 기억할 수 없다!");
#else
    ...
    if (you.experience_level < spell_difficulty(specspell))
    {
        redraw_screen();
#ifdef JP
        mpr("당신은 그 주문을 습득하기에는 경험이 너무 부족하다!");
#else
    ...
    temp_rand = random2(4);
    strcat(info, (temp_rand == 0) ? "배우기 위한 " :
                 (temp_rand == 1) ? "기억하기 위한 " :
                 (temp_rand == 2) ? "몸에 익히기 위한 " :
                 (temp_rand == 3) ? "이해하기 위한 "
                                  : "");
    temp_rand = random2(3);
    strcat(info, (temp_rand == 0) ? "난이도는 " :
                 (temp_rand == 1) ? "귀찮음은 " :
                 (temp_rand == 2) ? "위험성은 "
                                  : "");

    strcat(info, (chance >= 80) ? "터무니없을 정도다" :
                 (chance >= 60) ? "대단히 크다" :
                 (chance >= 45) ? "상당하다" :
                 (chance >= 30) ? "그리 대단하지 않다"
                                : "별 것 아니다");
    ...
    {
#ifdef JP
        mpr("글자가 이해할 수 없을 정도로 종잡을 수 없이 흐려졌다.");
#else
    ...
    if (random2(40) + random2(40) + random2(40) < chance)
    {
        ...
        {
#ifdef JP
            mpr("이 책은 당신의 어리석은 짓에 수고를 끼치는 것을 기뻐하지 않는다!");
