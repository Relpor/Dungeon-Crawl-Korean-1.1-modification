...
#ifdef JP
        snprintf( info, INFO_SIZE, "%s은(는) 벗겨져 떨어졌다.", str_pass );
#else
...
bool transform(int pow, char which_trans)
{
...
#ifdef JP
        mpr("당신은 물속에서는 원래의 형태 이외로 변신할 수 없다.");
#else
    ...
    {
#ifdef JP
        mpr("당신의 죽은 육체는 이 방법으로는 변신할 수 없다.");
#else
    ...
    case TRAN_LICH:
        ...
        {
#ifdef JP
            mpr( "그 변신은 현재 발동하고 있는 마법과 양립되지 않는다."
                 "already in effect." );
#else
        ...
        {
#ifdef JP
            mpr( "당신의 재생은 멈추었다.", MSGCH_DURATION );
#else
...
void untransform(void)
{
    ...
    case TRAN_SPIDER:
#ifdef JP
        mpr("당신은 변신이 풀렸다.", MSGCH_DURATION);
#else
    ...
    case TRAN_BLADE_HANDS:
#ifdef JP
        mpr( "당신의 양손은 보통의 손으로 돌아왔다.", MSGCH_DURATION );
#else
    ...
    case TRAN_STATUE:
#ifdef JP
        mpr( "당신은 본래의 육질의 몸로 돌아왔다.", MSGCH_DURATION );
#else
    ...
    case TRAN_ICE_BEAST:
#ifdef JP
        mpr( "당신은 다시 따뜻한 육체로 돌아왔다.", MSGCH_DURATION );
#else
    ...
    case TRAN_LICH:
#ifdef JP
        mpr( "당신은 살아있는 자로 돌아왔다.", MSGCH_DURATION );
#else
    ...
    case TRAN_AIR:
#ifdef JP
        mpr( "당신의 몸은 고체로 돌아왔다.", MSGCH_DURATION );
#else
...
#ifdef JP
    mpr( "당신은 소지품을 가지고 다닐 수 없게 된 것을 깨달았다!" );
#else
