...
void mons_trap(struct monsters *monster)
{
    ...
    if (mons_flies(monster))
    {
        if (trap_category(env.trap[which_trap].type) == DNGN_TRAP_MECHANICAL)
        {
            if (trapKnown)
#ifdef JP
                simple_monster_message(monster, "은(는) 함정 위를 안전하게 날고있다.");
#else
...
#ifdef JP
                                           "은(는) 좌우로 흔들리는 거대한 칼날을 피했다."))
#else
                                           " avoids a huge, swinging blade."))
#endif
            {
#ifdef JP
                mpr("거대한 칼날이 좌우로 흔들렸다!");
#else
    ...
    case TRAP_ZOT:
                ...
                && !silenced(you.x_pos, you.y_pos))
        {
            if (monsterNearby)
#ifdef JP
                strcpy(info, "\"조트\"의 성난 목소리를 들었다!");
#else
                strcpy(info, "You hear a loud \"Zot\"!");
#endif
            else
#ifdef JP
                strcpy(info, "멀리서 \"조트\"의 음성을 들었다!");
#else
    ...
    if (projectileFired)
    {
        ...
        {
#ifdef JP
            snprintf( info, INFO_SIZE, "%s에게 쏘아진 %s은(는) %s%s!",
                      ptr_monam( monster, DESC_NOCAP_THE ), //beem.beam_name,
                      beem.beam_name, //ptr_monam( monster, DESC_NOCAP_THE ),
                      (damage_taken >= 0) ? "명중했다" : "빗나갔다",
                      (damage_taken == 0) ? ", 그렇지만 피해를 주지 못했다" : "" );
#else
...
void mons_cast(struct monsters *monster, struct bolt &pbolt, int spell_cast)
{
...
#ifdef JP
        simple_monster_message(monster, "은(는) 지옥의 힘을 불러냈다!");
#else
    ...
    case MS_CANTRIP:
        ...
        case 0:
#ifdef JP
            simple_monster_message( monster, "은(는) 잠시 밝게 빛났다.",
#else
        ...
        case 2:
#ifdef JP
            mpr( "사악한 에너지의 파동이 당신 너머로 지나가는 것을 느꼈다." );
#else
        ...
        case 3:
#ifdef JP
            simple_monster_message( monster, "은(는) 강해보인다.",
#else
        ...
        case 4:
#ifdef JP
            simple_monster_message( monster, "은(는) 약간 반투명으로 변했다.",
#else
        ...
        case 5:
#ifdef JP
            simple_monster_message( monster, "의 눈이 빛나기 시작했다.",
#else
...
void monster_teleport(struct monsters *monster, bool instan)
{
    if (!instan)
    {
        ...
        {
#ifdef JP
            simple_monster_message(monster, "은(는) 더욱 안정되어 보인다.");
#else
...
void setup_dragon(struct monsters *monster, struct bolt &pbolt)
{
    ...
    case MONS_XTAHUA:
#ifdef JP
        strcat(pbolt.beam_name, "의 불꽃 폭풍");
#else
...
#ifdef JP
        strcat(pbolt.beam_name, "의 냉기 폭풍");
#else
...
bool mons_throw(struct monsters *monster, struct bolt &pbolt, int hand_used)
{
    ...
    if (launched)
    {
            ...
            && bow_brand != SPWPN_FROST && ammo_brand != SPMSL_ICE)
        {
            ...
            if (poison)
#ifdef JP
                strcat(pbolt.beam_name, "독 볼트");
#else
                strcat(pbolt.beam_name, "poison ");
#endif

#ifdef JP
            strcat(pbolt.beam_name, "화염 볼트");
#else
    ...
    strcat(info, (launched) ? "을(를) 쏘았다" : "을(를) 던졌다");
#else
    ...
    {
#ifdef JP
        snprintf( throw_buff, sizeof(throw_buff), "%s이(가) 쏘아 %s을(를) 맞춘",
                  ptr_monam( monster, DESC_PLAIN ),
                  pbolt.beam_name );
        char aux_buff[80];
        snprintf( aux_buff, sizeof(aux_buff), "%s의 사격",
                  pbolt.beam_name );
    ...
    {
#ifdef JP
        snprintf( throw_buff, sizeof(throw_buff), "%s이(가) 던져 %s을(를) 맞춘",
                  ptr_monam( monster, DESC_PLAIN),
                  pbolt.beam_name );
        char aux_buff[80];
        snprintf( aux_buff, sizeof(aux_buff), "%s의 투척",
                  pbolt.beam_name );
    pbolt.aux_source = aux_buff;
    }
#else
...
#ifdef JP
        beam.name = "마법 다트";       // inv_name [throw_2]);
#else
...
static unsigned char monster_abjuration(int pow, bool test)
{
    ...
    for (int ab = 0; ab < MAX_MONSTERS; ab++)
    {
        ...
        {
#ifdef JP
            simple_monster_message(monster, "은(는) 몸부림 쳤다.");
#else
