...
void level_change(void)
{
            ...
            && you.experience > exp_needed(you.experience_level + 2))
    {
        ...
        {
#ifdef JP
            snprintf( info, INFO_SIZE, "%d레벨로 돌아갔다!",
#else
        ...
        {
#ifdef JP
            snprintf( info, INFO_SIZE, "당신은 레벨 %d의 %s이(가) 되었다!",
#else
...
#ifdef JP
                    mpr("당신은 매력적이게 되었다!", MSGCH_INTRINSIC_GAIN);
#else
            ...
            case SP_MUMMY:
                ...
                {
#ifdef JP
                    mpr( "당신은 죽음의 힘과의 접촉을 보다 강하게 느꼈다.",
#else
                ...
                {
#ifdef JP
                    mpr( "당신은 이제, 부패한 육체를 회복하기 위해 육체에 마력을 충전할 수 있다.", MSGCH_INTRINSIC_GAIN );
#else
            ...
            case SP_NAGA:
                ...
                {
#ifdef JP
                    mpr("당신의 피부는 보다 단단하게 되었다.", MSGCH_INTRINSIC_GAIN);
#else
...
/* Grey is later */
            ...
            case SP_UNK2_DRACONIAN:
                if (you.experience_level == 7)
                {
                    ...
                    case SP_RED_DRACONIAN:
#ifdef JP
                        mpr("당신의 비늘은 불타는 듯한 붉은 색으로 변화하고 있다.",
#else
                    ...
                    case SP_WHITE_DRACONIAN:
#ifdef JP
                        mpr("당신의 비늘은 얼음 같은 하얀 색으로 변화하고 있다.",
#else
                    ...
                    case SP_GREEN_DRACONIAN:
#ifdef JP
                        mpr("당신의 비늘은 녹색으로 변화하고 있다.",
#else
                        mpr("Your scales start taking on a green colour.",
#endif
                            MSGCH_INTRINSIC_GAIN);
#ifdef JP
                        mpr("당신은 독에 대한 내성을 가지게 된 것 같다.", MSGCH_INTRINSIC_GAIN);
#else
                    ...
                    case SP_GOLDEN_DRACONIAN:
#ifdef JP
                        mpr("당신의 비늘은 황금색으로 변화하고 있다.", MSGCH_INTRINSIC_GAIN);
#else
                    ...
                    case SP_BLACK_DRACONIAN:
#ifdef JP
                        mpr("당신의 비늘은 흑색으로 변화하고 있다.", MSGCH_INTRINSIC_GAIN);
#else
                    ...
                    case SP_PURPLE_DRACONIAN:
#ifdef JP
                        mpr("당신의 비늘은 선명한 보라색으로 변화하고 있다.", MSGCH_INTRINSIC_GAIN);
#else
                    ...
                    case SP_MOTTLED_DRACONIAN:
#ifdef JP
                        mpr("당신의 비늘은 기묘한 반점 모양으로 변화하고 있다.", MSGCH_INTRINSIC_GAIN);
#else
                    ...
                    case SP_PALE_DRACONIAN:
#ifdef JP
                        mpr("당신의 비늘은 푸르스름한 회색으로 변화하고 있다.", MSGCH_INTRINSIC_GAIN);
#else
                ...
                if (you.experience_level == 18)
                {
                    ...
                    case SP_RED_DRACONIAN:
#ifdef JP
                        mpr("당신은 불에 대한 내성을 가지게 된 것 같다.", MSGCH_INTRINSIC_GAIN);
#else
                    ...
                    case SP_WHITE_DRACONIAN:
#ifdef JP
                        mpr("당신은 냉기에 대한 내성을 가지게 된 것 같다", MSGCH_INTRINSIC_GAIN);
#else
                ...
                {
#ifdef JP
                    mpr("당신의 비늘은 보다 강고하게 된 것 같다.", MSGCH_INTRINSIC_GAIN);
#else
            ...
            case SP_GREY_DRACONIAN:
                ...
                {
#ifdef JP
                    mpr("당신의 비늘은 회색으로 변화하고 있다.", MSGCH_INTRINSIC_GAIN);
#else
...
#ifdef JP
    mpr("어떤 능력을 올리시겠습니까? (S)힘 (I)지능 (D)민첩 ", MSGCH_PROMPT);
#else
...
void display_char_status(void)
{
    ...
    case TRAN_SERPENT_OF_HELL:
#ifdef JP
        mpr( "당신의 거대한 악마의 독사다." );  //테스트시 확인 바람(ZK2)
#else
...
#ifdef JP
        mpr( "당신은 성스러운 영기에 의해 언데드에게서 지켜지고 있다." );
#else
    ...
    {
#ifdef JP
        snprintf( info, INFO_SIZE, "당신은 %s 독에 시달리고 있다.",
                  (you.poison > 10) ? "지독한" :
                  (you.poison > 5)  ? "심한" :
                  (you.poison > 3)  ? "약간 심한"
                                    : "약한" );
    ...
    {
#ifdef JP
        snprintf( info, INFO_SIZE, "당신은 %s 병에 걸려 있다.",
                  (you.disease > 120) ? "심한" :
                  (you.disease >  40) ? ""
                                      : "약한" );
#else
...
#ifdef JP
        snprintf( info, INFO_SIZE, "당신의 살은 %s",
                  (you.rotting > 15) ? "보고 있는 사이에도 썩어 간다!":
                  (you.rotting > 8)  ? "급속하게 썩어 간다.":
                  (you.rotting > 4)  ? "썩어 간다."
             : ((you.species == SP_GHOUL && you.rotting > 0)
                        ? "평상시보다 빠르게 썩어 간다." : "썩어 간다.") );
#else
    ...
    {
#ifdef JP
        snprintf( info, INFO_SIZE, "당신의 손은 %s 빨간색으로 빛나고 있다.",
                  (you.confusing_touch > 40) ? "눈부신" :
                  (you.confusing_touch > 20) ? "밝은"
                                             : "" );
#else
    ...
    {
#ifdef JP
        snprintf( info, INFO_SIZE, "당신은 검과 %s 결합되어 있다.",
                  (you.sure_blade > 15) ? "강하게" :
                  (you.sure_blade >  5) ? ""
                                        : "약하게" );
#else
...
// defaults:                               false       false     true
{
    ...
    if (player_genus( GENPC_DRACONIAN, speci ))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                case SP_MOTTLED_DRACONIAN:
#ifdef JP
                    strcpy( species_buff, "얼룩무늬 드라코니언" );
#else
...
#ifdef JP
            strcpy( species_buff, "엘프족" );
#else
            strcpy( species_buff, "Elven" );
#endif
        else if (genus)
#ifdef JP
            strcpy( species_buff, "엘프" );
#else
    ...
    else if (player_genus( GENPC_DWARVEN, speci ))
    {
        if (adj)  // doesn't care about species/genus
#ifdef JP
            strcpy( species_buff, "드워프족" );
#else
            strcpy( species_buff, "Dwarven" );
#endif
        else if (genus)
#ifdef JP
            strcpy( species_buff, "드워프" );
#else
    ...
    else
    {
        ...
        case SP_HUMAN:
#ifdef JP
            strcpy( species_buff, (adj) ? "인간족" : "인간" );
#else
        ...
        case SP_HILL_ORC:
#ifdef JP
            strcpy( species_buff, (adj) ? "오크족" : (genus) ? "오크"
                                                             : "힐 오크" );
#else
        ...
        case SP_GNOME:
#ifdef JP
            strcpy( species_buff, (adj) ? "노움족" : "노움" );
#else
        ...
        case SP_OGRE:
#ifdef JP
            strcpy( species_buff, (adj) ? "오우거족" : "오우거" );
#else
        ...
        case SP_TROLL:
#ifdef JP
            strcpy( species_buff, (adj) ? "트롤족" : "트롤" );
#else
        ...
        case SP_DEMIGOD:
#ifdef JP
            strcpy( species_buff, (adj) ? "반신족" : "반신족" );
#else
        ...
        case SP_DEMONSPAWN:
#ifdef JP
            strcpy( species_buff, (adj) ? "데몬족" : "데몬스폰" );
#else
        ...
        case SP_MERFOLK:
#ifdef JP
            strcpy( species_buff, (adj) ? "머포크족" : "머포크" );
#else
        ...
        default:
#ifdef JP
            strcpy( species_buff, (adj) ? "야크족" : "야크" );
#else
...
void modify_stat(unsigned char which_stat, char amount, bool suppress_msg)
{
    ...
    case STAT_STRENGTH:
        ...
        if (!suppress_msg)
#ifdef JP
            strcat(info, (amount > 0) ? "강력해졌다." : "힘이 약해졌다.");
#else
    ...
    case STAT_DEXTERITY:
        ...
        if (!suppress_msg)
#ifdef JP
            strcat(info, (amount > 0) ? "기민해졌다." : "서툴러졌다.");
#else
    ...
    case STAT_INTELLIGENCE:
        ...
        if (!suppress_msg)
#ifdef JP
            strcat(info, (amount > 0) ? "현명해졌다." : "어리석어졌다.");
#else
...
#ifdef JP
    { "전사", "마법사", "사제", "도둑", "검투사", "강령술사",
      "성기사", "암살자", "광전사", "사냥꾼", "요술사", "주술사",
      "화염술사", "냉기술사", "소환술사", "대기술사",
      "대지술사", "성전사", "죽음의 기사", "독술사",
      "혼돈의 기사", "변이술사", "치유사", "쿼터", "약탈자", "추적자",
      "수도승", "전이술사", "방랑자"};
#else
...
void contaminate_player(int change, bool statusOnly)
{
    ...
    if (statusOnly)
    {
        if (new_level > 0)
        {
...
#ifdef JP
                    "당신은 전신에 기분 나쁜 빛을 받았다! " :
                    "당신은 파삭파삭 튀고 있는 마력의 구름에 삼켜졌다!");
#else
            ...
            {
#ifdef JP
                snprintf( info, INFO_SIZE, "당신은 잔류된 마력에 %s%s",
                    (new_level == 3) ? "빛을 발할 정도다" :
                    (new_level == 2) ? "가득 차 있다"
                                     : "오염되어 있다",
                    (new_level == 3) ? "!" : ".");
#else
...
#ifdef JP
    snprintf( info, INFO_SIZE, "당신은 마력의 에너지에 의한 오염이 %s을(를) 느꼈다.",
              (change < 0) ? "줄어들었음" : "더해졌음" );
#else
...
void poison_player( int amount, bool force )
{
    ...
    {
#ifdef JP
        snprintf( info, INFO_SIZE, "당신은 %s 독에 시달렸다.",
                  (old_value > 0) ? "더 심한" : "" );
#else
...
void reduce_poison_player( int amount )
{
    ...
    if (you.poison <= 0)
    {
        you.poison = 0;
#ifdef JP
        mpr( "당신은 기분이 좋아졌다.", MSGCH_RECOVERY );
#else
    ...
    {
#ifdef JP
        mpr( "당신은 조금 기분이 좋아졌다.", MSGCH_RECOVERY );
#else
...
void confuse_player( int amount, bool resistable )
{
    ...
    {
#ifdef JP
        snprintf( info, INFO_SIZE, "당신은 %s 혼란스러워졌다.",
                  (old_value > 0) ? "더 심하게" : "" );
#else
...
void dec_slow_player( void )
{
    ...
    {
#ifdef JP
        mpr("당신의 움직임이 정상이 되었다.", MSGCH_DURATION);
#else
...
void dec_haste_player( void )
{
    if (you.haste > 1)
    {
        ...
        {
#ifdef JP
            mpr( "당신의 가속은 없어지고 있다.", MSGCH_DURATION );
#else
    ...
    {
#ifdef JP
        mpr( "당신의 가속은 없어졌다.", MSGCH_DURATION );
#else
...
void dec_disease_player( void )
{
    if (you.disease > 0)
    {
        ...
        if (!you.disease)
#ifdef JP
            mpr("당신의 건강 상태는 회복되었다.", MSGCH_RECOVERY);
#else
...
#ifdef JP
        snprintf( info, INFO_SIZE, "당신의 고기가 %s!",
                  (you.rotting) ? "썩고 있다" : "썩기 시작했다" );
#else
