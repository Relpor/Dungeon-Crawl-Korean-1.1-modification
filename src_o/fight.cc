 ...
 */
    ...
    if (player_in_water() && !player_is_swimming())
    {
        ...
        {
#ifdef JP
            mpr("제대로 서있지 못할 정도로 기운이 없어 당신은 공격에 실패했다.");
#else
            ...
            && !one_chance_in(10 + you.skills[SK_STABBING])))
    {
        ...
        if (defender->hit_points < 1)
        {
            ...
            {
#ifdef JP
                snprintf( info, INFO_SIZE, "당신은 거대 포자를 %s", damage_noise);
#else
            ...
            {
#ifdef JP
                snprintf( info, INFO_SIZE, "당신은 전기 구체를 %s", damage_noise);
#else
        ...
        if (damage_done < 1 && player_monster_visible( defender ))
        {
...
#ifdef JP
            snprintf( info, INFO_SIZE, "당신은 %s을(를) %s",
            ...
            {
                strcat(info, ". 그러나 손상을 입히지 못했다.");
                mpr(info);
            }
            else
            {
                mpr(info);
                strcpy(info, ". 그러나 손상을 입히지 못했다.");
                mpr(info);
            }
#else
...
#ifdef JP
            strcpy(info, "당신은 갑옷의 무게가 무거워 공격하지 못했다.");
#else
        ...
        || (hit && damage_done < 1 && mons_has_ench(defender,ENCH_INVIS)))
    {
        ...
        strcpy(info, "당신은 ");
        strcat(info, ptr_monam(defender, DESC_PLAIN));
        strcat(info, "에게 ");//         strcat(info, "을(를) ");감자가 수정
    ...
    if (hit)
    {
        ...
        if (defender->type == MONS_HYDRA)
        {
                ...
                && (damage_done >= 4 || wpn_brand == SPWPN_VORPAL || coinflip()))
            {
                ...
                temp_rand = random2(4);
#ifdef JP
                const char *const verb = (temp_rand == 0) ? "베었다." :
                                         (temp_rand == 1) ? "베어냈다." :
                                         (temp_rand == 2) ? "두드려 잘랐다." : "찍어 잘라냈다.";
#else
                ...
                {
#ifdef JP
                    snprintf( info, INFO_SIZE, "당신은 %s의 마지막 목을 %s!",
                ...
                {
#ifdef JP
                    snprintf( info, INFO_SIZE, "당신은 %s의 목중 하나를 %s!",
                              ptr_monam(defender, DESC_PLAIN), verb );
#else
                    ...
                    {
#ifdef JP
                        simple_monster_message( defender, " 의 목이 두개 더 자라났다!" );
#else
                ...
                && random2(20) <= you.skills[SK_EVOCATIONS])
            {
                ...
                case STAFF_AIR:
                    if (damage_done + you.skills[SK_AIR_MAGIC] > random2(30))
                    {
                        ...
                        {
#ifdef JP
                            snprintf( info, INFO_SIZE, "%s는(은) 전기 쇼크를 받았다!",
...
#ifdef JP
                        snprintf( info, INFO_SIZE, "%s 은(는) 얼어붙었다.",
                                  ptr_monam(defender, DESC_PLAIN) );
#else
                ...
                case STAFF_EARTH:
                    ...
                    {
#ifdef JP
                        snprintf( info, INFO_SIZE, "당신은 %s을(를) 으깨버렸다!",
#else
                ...
                case STAFF_FIRE:
                    ...
                    {
#ifdef JP
                        snprintf( info, INFO_SIZE, "당신은 %s을(를) 태웠다!",
#else
                ...
                case STAFF_DEATH:
                    ...
                    if (random2(8) <= you.skills[SK_NECROMANCY])
                    {
                        ...
                        {
#ifdef JP
                            snprintf( info, INFO_SIZE, "%s 은(는) 고통에 몸을 떨었다",
#else
                ...
                default:
#ifdef JP
                    mpr("당신은 내가 들어본 적이 없는 지팡이를 손에 쥐고 있다! (fight.cc)");
#else
...
// END STAFF HACK
        ...
        else
        {
            ...
            case SPWPN_DISRUPTION:
                ...
                {
#ifdef JP
                    simple_monster_message(defender, " 은(는) 격렬하게 진동했다.");
#else
            ...
            case SPWPN_PAIN:
                ...
                {
#ifdef JP
                    simple_monster_message(defender, " 은(는) 고통으로 몸을 떨었다.");
#else
...
#ifdef JP
                    strcpy(info, ptr_monam(defender, DESC_NOCAP_THE));
                    strcat(info, "의 주위의 공간이 격렬하게 비뚤어졌다");
#else
...
#ifdef JP
                strcpy(attack_name, (you.species == SP_KENKU) ? "쪼기"
                                                              : "박치기");
#else
...
void monster_attack(int monster_attacking)
{
...
#ifdef JP
                   "는(은) 당신에게 공격을 시도했지만 신성한 영기에 의해 막혔다.");
#else
            ...
            && (!player_under_penance() && you.piety >= 75)))
    {
        if (mons_has_ench(attacker, ENCH_ABJ_I, ENCH_ABJ_VI))
        {
            ...
            if (coinflip())
            {
                simple_monster_message(attacker,
#ifdef JP
                                   "는(은) 당신에게 공격을 시도했지만 공포로 뒷걸음질쳐 버렸다.");
#else
    ...
    {
#ifdef JP
        simple_monster_message(attacker, "는(은) 주위의 물을 튀기고 있다.");
#else
    ...
    for (runthru = 0; runthru < 4; runthru++)
    {
...
#ifdef JP
            snprintf( info, INFO_SIZE, "당신은 %s의 공격을 막았다.",
        ...
        else if (!blocked)
        {
            hit = false;
#ifdef JP
            simple_monster_message(attacker, "는(은) 당신을 맞추지 못했다.");
#else
        ...
        if (damage_taken < 1 && hit && !blocked)
        {
            simple_monster_message(attacker,
#ifdef JP
                                    "의 공격이 성공했지만 당신에게 아무런 손상을 입히지 못했다.");
#else
        ...
        if (damage_taken > 0)
        {
            ...
            case MONS_BUMBLEBEE:
                ...
                {
#ifdef JP
                    simple_monster_message( attacker, "는(은) 당신을 찔렀다!" );
#else
            ...
            case MONS_SUN_DEMON:
                ...
                {
#ifdef JP
                    mpr("당신의 얼음 방패는 부려져 버렸다!", MSGCH_DURATION);
#else
            ...
            case MONS_YELLOW_WASP:
                        ...
                        // maybe I should flip back the other way? {dlb}
                {
                    if (you.paralysis > 0)
#ifdef JP
                        mpr("당신은 마비된 채 움직일 수 없다!", MSGCH_WARN);
#else
                        mpr("You still can't move!", MSGCH_WARN);
#endif
                    else
#ifdef JP
                        mpr("당신은 갑자기 움직일 수 없게 되었다!", MSGCH_WARN);
#else
            ...
            case MONS_ELECTRIC_GOLEM:
                if (!player_res_electricity())
                {
                    ...
                    strcpy(info, ptr_monam(attacker, DESC_CAP_THE));
#ifdef JP
                    strcat(info, "는(은) 당신에게 전기충격을 주었다!");
#else
...
****************************************************************** */
...
#ifdef JP
                    simple_monster_message(attacker, " 당신의 상처에서 생명력을 빨아 들였다!");
#else
            ...
            case MONS_MIDGE:
                ...
                {
#ifdef JP
                    simple_monster_message(attacker, "는(은) 사라졌다.");
#else
                    ...
                    && ghost.values[ GVAL_BRAND ] != SPWPN_NORMAL)))
        {
            ...
            case SPWPN_FLAMING:
                ...
                {
#ifdef JP
                    simple_monster_message(attacker, "는(은) 당신을 태웠다.");
#else
...
*********************** */
                ...
                {
#ifdef JP
                    mpr("당신의 얼음 방패는 부서져 버렸다!", MSGCH_DURATION);
#else
                    mpr("Your icy shield dissipates!", MSGCH_DURATION);
#endif
                    you.duration[DUR_CONDENSATION_SHIELD] = 0;
            ...
            case SPWPN_FREEZING:
                ...
                {
#ifdef JP
                    simple_monster_message(attacker, "는(은) 당신을 얼어붙게했다.");
#else
        ...
        if (hit)                //(int) damage_taken > 0)
        {
            ...
            if (sees)
            {
                ...
                if (attacker->type != MONS_DANCING_WEAPON
                    ...
                    && !launches_things( mitm[attacker->inv[hand_used]].sub_type ))
                    {
                    it_name(mmov_x, DESC_NOCAP_A, str_pass);       // 무기명
                    strcat(info, str_pass);
                    strcat(info, "(으)로 ");
                    }
                    strcat(info, "공격을 성공시켰다");
#endif
            ...
            case MONS_SUN_DEMON:
                ...
                if (specdam)
#ifdef JP
                    simple_monster_message(defender, "는(은) 불길에 휩싸였다!");
#else
            ...
            case MONS_BORIS:
                ...
                {
#ifdef JP
                    simple_monster_message(defender, "의 생명력이 흡수되었다.");
#else
            ...
            case MONS_VAMPIRE_MAGE:
                ...
                if (heal_monster(attacker, random2(damage_taken), true))
#ifdef JP
                    simple_monster_message(attacker, "은(는) 치료되었다.");
#else
                     ...
                     && ghost.values[ GVAL_BRAND ] != SPWPN_NORMAL)))
        {
                ...
                || attacker->type == MONS_PANDEMONIUM_DEMON)
            {
                ...
                case SPWPN_DISRUPTION:
                    ...
                    {
#ifdef JP
                        simple_monster_message(defender, "은(는) 몹시 떨었다.");
#else
                        simple_monster_message(defender, " shudders.");
#endif
...
#ifdef JP
                            strcat(info, ptr_monam(defender, DESC_NOCAP_THE));
                            strcpy(info, "의 주변 공간이 심각하게 왜곡되었다");
#else
                            strcpy(info, "Space warps horribly around ");
                            strcat(info, ptr_monam(defender, DESC_NOCAP_THE));
#endif
...
static void stab_message( struct monsters *defender, int stab_bonus )
{
    ...
    switch(stab_bonus)
    {
        case 3:     // big melee, monster surrounded/not paying attention
            ...
            {
#ifdef JP
                snprintf( info, INFO_SIZE, "당신은 %s의 맹점을 찾아 기습했다!",
                          ptr_monam(defender, DESC_PLAIN) );
#else
            ...
            {
#ifdef JP
                snprintf( info, INFO_SIZE, "당신은 %s의 순간적인 빈틈을 찾아내었다.",
                          ptr_monam(defender, DESC_PLAIN) );
#else
        ...
        case 2:     // confused/fleeing
            ...
            {
#ifdef JP
                snprintf( info, INFO_SIZE, "당신은 %s의 헛점을 정확하게 찾아내었다",
                           ptr_monam(defender, DESC_PLAIN) );
#else
            ...
            {
#ifdef JP
                snprintf( info, INFO_SIZE, "당신은 %s의 뒤에서 기습했다!",
                           ptr_monam(defender, DESC_PLAIN) );
#else
        ...
        case 1:
#ifdef JP
            snprintf( info, INFO_SIZE, "%s은(는) 방어에 실패했다.",
                           ptr_monam(defender, DESC_PLAIN) );
#else
