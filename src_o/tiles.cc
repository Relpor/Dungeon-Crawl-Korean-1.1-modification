...
//lib*.cc Ç≈ÇÃêÈåæÇ∆ÇÃêÆçáê´Ç…íçà”
...
// gv backup
// åªç›tile_dngn Ç…éùÇ¡ÇƒÇ¢ÇÈÉ^ÉCÉãÇ…ëŒâûÇ∑ÇÈílÅB
// Ç±ÇÍÇ∆î‰ärÇµÇƒà·Ç§èÍçáÇÕ tile_dngn ÇçƒåvéZ
...
int tileidx_weapon(const item_def &item)
{
  ...
  {
    if (strcmp("ÉãÅ[ÉìÇ™çèÇ‹ÇÍÇΩ", randart_name(item))==0)
        etype = 1;
    else
        etype = 3;
  }
...
int tileidx_armour(const item_def &item)
{
  ...
  if (is_random_artefact( item )) 
  {
    if (strcmp("ÉãÅ[ÉìÇ™çèÇ‹ÇÍÇΩ", randart_armour_name(item))==0)
      etype = 2;
    else
    if (strcmp("åıÇÈ", randart_armour_name(item))==0)
      etype = 1;
    else
      etype = 4;
  }
...
void tilep_scan_parts(char *fbuf, int *parts)
{
    ...
    for(i = 0; i < TILEP_PARTS_TOTAL; i++)
    {
        ...
        if (i == 0) //ê´ï Ç™èóê´Ç≈Ç†ÇÈÇ»ÇÁéÌë∞ÇÃëfëÃÇ…+1
            parts[i] += (idx +1) % 2; 
        else if (idx < 0) //ïâÇÃêîÇÕÉ[ÉçÇ…Ç∑ÇÈ
            parts[i] = 0;
        else if (idx == TILEP_SHOW_EQUIP)
            parts[i] = idx;
        else if (idx > tilep_parts_total[i]) //ïsê≥Ç»ïîïiî‘çÜÇä€ÇﬂÇÈ
...
void tilep_print_parts(char *fbuf, int *parts)
{
    ...
    for(i = 0; i < TILEP_PARTS_TOTAL; i++)
    {
        if (i == TILEP_PART_BASE) //ê´ï ÇÕ0èóê´1íjê´Ç≈ä«óù
        {
...
// ÉpÅ[ÉcÇÃñºëO Ç»Ç¢èÍçáÇÕêîéöÇ≈ë„óp
...
void TilePickMenu()
{
    ...
    gotoxy(1,24);
#ifdef JP
    cprintf("A-W, a-w) º±≈√   *) ∏µŒ º±≈√  [SPACE/RET] ¡÷øÚ  ESC) ¡ﬂ¡ˆ");
#else
