...
#ifdef JP 
    snprintf(info, INFO_SIZE, "던젼이 %s!", (sil ? "흔들렸다" : "굉음을 냈다"));
#else
...
static void cast_detect_magic(int pow)
{
  ...
  all_done:
    ...
    {
#ifdef JP 
        mpr("당신은 몬스터 같은 형태가 형성된 필드를 감지했다.");
#else
...
#ifdef JP 
        snprintf( info, INFO_SIZE, "당신은 비밀의 문을 감지했다." );
#else
...
void cast_sticks_to_snakes(int pow)
{
    ...
    {
#ifdef JP 
        snprintf( info, INFO_SIZE, "당신의 %s이(가) 뱀처럼 된 것 같다고 느꼈다!", your_hand(true));
#else
...
#ifdef JP 
        snprintf( info, INFO_SIZE, "당신은 뱀을 만들어 냈다!" );
#else
...
void cast_summon_dragon(int pow)
{
                        ...
                        you.x_pos, you.y_pos, MHITYOU, 250 ) != -1)
    {
        ...
        if (!happy)
#ifdef JP 
            strcat(info, "용은 그리 기분좋아 보이지는 않는다.");
#else
...
#ifdef JP 
    simple_monster_message(monster, "을(를) 길들였다!");
#else
...
void cast_ignite_poison(int pow)
{
    ...
    {
#ifdef JP 
        snprintf( info, INFO_SIZE, "당신이 가지고 있는 물약이 폭발했다!" );
#else
...
void cast_discharge( int pow )
{
    ...
    if (dam == 0) 
    {
        if (coinflip())
#ifdef JP 
            mpr("당신 주변의 대기가 전하 때문에 파직파직 소리를 냈다.");
#else
...
#ifdef JP 
            snprintf( info, INFO_SIZE, "%s푸른 전호가 당신에게는 해를 끼치지 않고 %s지면에 떨어졌다.",
                plural ? "몇 개의 " : "",
                plural ? "주변의 " : (coinflip() ? "옆쪽의 " :
                                     coinflip() ? "뒷쪽의 " : "앞쪽의 ")
                );
#else
...
static int distortion_monsters(int x, int y, int pow, int message)
{
    ...
    else if (coinflip())
    {
        ...
        strcat(info, ptr_monam( defender, DESC_NOCAP_THE ));
#ifdef JP 
        strcat(info, "의 주변 공간이 무섭게 일그러졌다.");
#else
        strcat(info, "!");
#endif
        mpr(info);
...
int disperse_monsters(int x, int y, int pow, int message)
{
    ...
    {
#ifdef JP 
        simple_monster_message(defender, "은(는) 주문에 저항했다.");
#else
    ...
    else if (check_mons_resist_magic(defender, pow))
    {
        ...
        {
#ifdef JP 
            simple_monster_message(defender, "은(는) 몇 개의 주문에 저항했다.");
#else
...
#ifdef JP 
            simple_monster_message(defender, "은(는) 주문에 다소 저항했다.");
#else
            simple_monster_message(defender, " resists.");
#endif
...
bool backlight_monsters(int x, int y, int pow, int garbage)
{
    ...
    if (lvl == ENCH_NONE)
#ifdef JP 
        simple_monster_message( &menv[mon], "은(는) 빛으로 비추어졌다." );
#else
        simple_monster_message( &menv[mon], " is outlined in light." );
#endif
    else if (lvl == ENCH_BACKLIGHT_IV)
#ifdef JP 
        simple_monster_message( &menv[mon], "은(는) 잠깐 보다 밝게 빛났다." );
#else
...
#ifdef JP 
        simple_monster_message( &menv[mon], "은(는) 보다 밝게 빛났다." );
#else
...
void cast_evaporate(int pow)
{
...
#ifdef JP 
    const int potion = prompt_invent_item( "어떤 물약을 던질까요?", OBJ_POTIONS );
#else
    ...
    {
#ifdef JP 
        snprintf( info, INFO_SIZE, "당신의 %s에서 증기가 분출되었다!", 
#else
...
void cast_fulsome_distillation( int powc )
{
...
#ifdef JP 
            snprintf( info, INFO_SIZE, "%s에게서 물약을 정제합니까?", str_pass );
#else
...
#ifdef JP 
    snprintf( info, INFO_SIZE, "당신은 시체로부터 %s을(를) 추출했다.",
#else
    ...
    {
#ifdef JP 
        mpr( "유감이지만, 당신은 이 보다 더 많이 가지고 다닐 수 없다!" );
#else
...
#ifdef JP 
    snprintf( info, INFO_SIZE, "%s은(는) 앞뒤로 휘청거렸다.", ptr_monam( &(menv[mon]), DESC_CAP_THE ));
#else
...
void cast_fragmentation(int pow)        // jmf: ripped idea from airstrike
{
...
#ifdef JP 
    mpr("무엇을 부숩니까? (예:벽 따위)", MSGCH_PROMPT);
#else
    ...
    if (mon != NON_MONSTER)
    {
...
#ifdef JP 
        snprintf( explode_msg, sizeof( explode_msg ), "%s 폭발!",
#else
...
#ifdef JP 
            snprintf( info, INFO_SIZE, "해골이 폭발하여 날카로운 뼈의 파편을 흩뿌렸다!" );
#else
...
#ifdef JP 
            strcpy(blast.beam_name, "뼛조각의 폭풍");
#else
        ...
        case MONS_WOOD_GOLEM:
            explode = false;
#ifdef JP 
            simple_monster_message(&menv[mon], "은(는) 맹렬히 진동했다!");
#else
        ...
        case MONS_METAL_GARGOYLE:
            explode = true;
#ifdef JP 
            strcpy( blast.beam_name, "금속 조각의 폭풍" );
#else
        ...
        case MONS_GARGOYLE:
            ...
            blast.ex_size = 2;
#ifdef JP 
            strcpy(blast.beam_name, "바위 조각의 폭풍");
#else
        ...
        case MONS_CRYSTAL_GOLEM:
            ...
            blast.ex_size = 2;
#ifdef JP 
            strcpy(blast.beam_name, "수정 조각의 폭풍");
#else
  ...
  do_terrain:
...
#ifdef JP 
        what = ((grid == DNGN_UNDISCOVERED_TRAP) ? "바닥" : "함정");
#else
    ...
    case DNGN_FLOOR:
        explode = false;
#ifdef JP 
        snprintf( info, INFO_SIZE, "%s은(는) 부자연스럽게 굳어 있는 것 같다.",
                  (grid == DNGN_PERMAROCK_WALL) ? "그 벽" 
                                                : "그곳의 바닥" );
#else
  ...
  all_done:
    if (explode && blast.damage.num > 0)
    {
        ...
        {
#ifdef JP 
            snprintf( info, INFO_SIZE, "%s은(는) 폭발했다!", what);
#else
...
void cast_twist(int pow)
{
    ...
    {
#ifdef JP 
        simple_monster_message( &menv[ mons ], "은(는) 주문에 저항했다." );
#else
...
void cast_far_strike(int pow)
{
    ...
    {
#ifdef JP 
        simple_monster_message( monster, "은(는) 주문에 저항했다." );
#else
...
void cast_apportation(int pow)
{
    ...
    if (item == NON_ITEM)
    {
        ...
        {
#ifdef JP 
            snprintf( info, INFO_SIZE, "%s은(는) 경련했다.",
#else
    ...
    {
#ifdef JP 
        mpr( "산더미 같은 아이템들은 당신이 끌어당기는 것에 저항하고 있다." );
#else
...
#ifdef JP 
            mpr( "당신은 몇 개의 아이템이 허공에서 없어진 것을 알아차렸다." );
#else
...
#ifdef JP 
            mpr( "훔쳤다!" );
            snprintf( info, INFO_SIZE, "당신은 아이템을 자신이 있는 곳까지 끌어당겼다." );
#else
            mpr( "Yoink!" );
...
#ifdef JP 
    mpr("누구에게 씨앗을 심습니까?", MSGCH_PROMPT);
#else
...
#ifdef JP 
            mpr("죽음과 같은 공포의 존재가 당신의 심장을 경련시켰다.");
#else
...
#ifdef JP 
        simple_monster_message(&menv[i], "은(는) 경련했다.");
#else
...
void cast_condensation_shield(int pow)
{
    ...
    else
    {
        ...
        {
#ifdef JP 
            mpr("파삭파삭 소리를 내는 응집된 공기덩어리가 공중에 형성되었다!");
#else
...
void cast_stoneskin(int pow)
{
    ...
    {
#ifdef JP 
        mpr("이 주문은 당신 같은 언데드의 육체에는 작용하지 않는다.");
#else
    ...
    {
#ifdef JP 
        mpr("이 주문은 당신의 현재 형태에서는 작용하지 않는다.");
#else
