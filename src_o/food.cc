...
#ifdef JP 
        snprintf( info, INFO_SIZE, "%c - %s(으)로 무기를 바꾸었다.", let, buff );
#else
...
bool butchery(void)
{
    ...
    else
    {
        ...
        if (Options.easy_butcher && !can_butcher)
        {
            ...
            {
#ifdef JP 
                mpr ("당신은 고기를 처리할 칼을 찾기에는 너무나 광폭해져서 그럴 수 없다!");
#else
...
#ifdef JP 
                mpr( "어떤 것을 사용할 것인가?", MSGCH_PROMPT );
#else
    ...
    {
#ifdef JP 
        mpr("아마도 날카로운 도구를 가지고 시도해야 할 것이다.");
#else
...
#ifdef JP 
    which_inventory_slot = prompt_invent_item( "어떤걸 먹을 건가요?", OBJ_FOOD );
#else
...
#ifdef JP 
    strcpy(info, (magnitude <= 100) ? "당신은 매우 약간 " :
                 (magnitude <= 350) ? "당신은 어느정도 " :
                 (magnitude <= 800) ? "당신은 많이 "
                                    : "당신은 아주 많이 ");
#else
...
#ifdef JP 
    strcat(info, (you.hunger_state > HS_SATIATED) ? "배가 부른걸 "
                                                  : "허기를 ");
    if ((you.hunger_state > HS_SATIATED) ^ (food_increment < 0))
        strcat(info, "느꼈다.");    
    else
...
static bool eat_from_floor(void)
{
    ...
    for (int o = igrd[you.x_pos][you.y_pos]; o != NON_ITEM; o = mitm[o].link)
    {
        ...
        it_name( o, DESC_NOCAP_A, str_pass );
#ifdef JP 
        snprintf( info, INFO_SIZE, "%s%s 먹습니까?", str_pass, 
                 (mitm[o].quantity > 1) ? "중 1개를" : "을(를)" );
#else
...
#ifdef JP 
            strcat(info, (likes_chunks) ? "좋은 맛이다."
                                        : "그다지 식욕을 돋우지 못한다.");
#else
...
static void eating(unsigned char item_class, int item_type)
{
    ...
    case OBJ_FOOD:
...
#ifdef JP 
                strcpy(info, "이 육포는 ");
                strcat(info, (one_chance_in(4)) ? "질기다"
                                                : "맛있다");
                strcat(info, "!");
#else
        ...
        if (how_carnivorous < 1)
        {
...
#ifdef JP 
                strcpy(info, "음.. 냠.냠.. ");
                strcat(info, (item_type == FOOD_APPLE)   ? "사과다." :
                             (item_type == FOOD_PEAR)    ? "배다." :
                             (item_type == FOOD_APRICOT) ? "살구다."
                                                         : "과일이다.");
#else
            ...
            case FOOD_ORANGE:
#ifdef JP 
                strcpy(info, "이 오렌지는 참 맛있다!");
#else
                strcpy(info, "That orange was delicious!");
#endif
                if (one_chance_in(8))
#ifdef JP 
                    strcat(info, " 껍질마저도 맛있다!");
#else
            ...
            case FOOD_BANANA:
#ifdef JP 
                strcpy(info, "이 바나나는 참 맛있다!");
#else
                strcpy(info, "That banana was delicious!");
#endif
                if (one_chance_in(8))
#ifdef JP 
                    strcat(info, " 껍질까지도 맛있다!");
#else
                    strcat(info, " Even the peel tasted good!");
#endif
                mpr(info);
...
#ifdef JP 
                strcat(info, (temp_rand == 0) ? "햄&파인애플 피자다" :
                             (temp_rand == 1) ? "액스트라 딕 크러스트 피자다." :
                             (temp_rand == 2) ? "베지터블 피자다." :
                             (temp_rand == 3) ? "페퍼로니 피자다." :
                             (temp_rand == 4) ? "음.. 엔쵸비 피자다." :
                             (temp_rand == 5) ? "치즈를 얹은 피자다." :
                             (temp_rand == 6) ? "슈프림 피자다." :
                             (temp_rand == 7) ? "수퍼 슈프림 피자다."
                                              : "치킨 피자다.");
#else
...
#ifdef JP 
            strcat(info, (temp_rand == 0) ? "체다 치즈다" :
                         (temp_rand == 1) ? "에담 치즈다" :
                         (temp_rand == 2) ? "웬즐리데일 치즈다" :
                         (temp_rand == 3) ? "카망베르다" :
                         (temp_rand == 4) ? "엄소젖 치즈다" :
                         (temp_rand == 5) ? "프루츠 치즈다" :
                         (temp_rand == 6) ? "모짜렐라 치즈다" :
                         (temp_rand == 7) ? "양젖 치즈다"
                                          : "야크젖 치즈다");
