...
int check_your_resists(int hurted, int flavour)
{
        ...
        || flavour == BEAM_FRAG)
    {
        ...
        {
#ifdef JP
            mpr( "´ç½ÅÀÇ ¾óÀ½ ¹æÆÐ´Â ºÎ¼­Á³´Ù!", MSGCH_DURATION );
#else
...
void weapon_acid( char acid_strength )
{
    ...
    {
#ifdef JP
        snprintf( info, INFO_SIZE, "´ç½ÅÀÇ %sÀº(´Â) Å¸¹ö·È´Ù!", your_hand(true) );
#else
...
#ifdef JP
        strcat(info, (it_resists) ? "Àº(´Â) ÀúÇ×ÇØ³Â´Ù." : "Àº(´Â) ³ì¾Æ ¾ø¾îÁ³´Ù!");
#else
...
#ifdef JP
    snprintf( info, INFO_SIZE, "ÀÌÁ¦ ·¹º§ %dÀÇ %sÀÌ(°¡) µÇ¾ú´Ù!",
#else
...
void drain_exp(void)
{
    ...
    {
#ifdef JP
        simple_god_message("ÀÌ(°¡) ´ç½ÅÀÇ »ý¸íÀ» ÁöÅ°°íÀÖ´Ù!");
#else
...
void ouch( int dam, int death_source, char death_type, const char *aux )
{
    ...
    if (dam > -9000)            // that is, a "death" caused by hp loss {dlb}
    {
        ...
        case GOD_XOM:
            ...
            {
#ifdef JP
                simple_god_message( "ÀÌ(°¡) ´ç½ÅÀ» ÇÇÇØ·ÎºÎÅÍ ÁöÄ×´Ù!" );
#else
...
#ifdef JP
            mpr( "¾Ç!  ÀÌ°Ç Á¤¸»·Î ¾ÆÇÁ´Ù!", MSGCH_DANGER );
#else
        ...
        {
#ifdef JP
            mpr( "* * * °æ°í! »ý¸í·ÂÀÌ ³·À½ * * *", MSGCH_DANGER );
#else
...
void end_game( struct scorefile_entry &se )
{
    ...
    {
#ifdef JP
        mpr("Á×¾ú´Ù...");      // insert player name here? {dlb}
#else
...
#ifdef JP
        mpr("ÄÉ¸¯ÅÍ ´ýÇÁ¿¡ ¼º°øÇß´Ù! (morgue.txt).");
#else
    ...
    more();
    clrscr();
    cprintf(EOL "* º£Å¸ Å×½ºÆ®¿¡ µµ¿ò ÁÖ½Å ºÐµé (°¡³ª´Ù ¼ø¼­) *" EOL );
    cprintf("http://nethack.byus.net" EOL );
    cprintf(EOL "°¨ÀÚ, °­»ó, ±¸³ª, ±è¼ø¼º, ´ÙÅ©°¡ÀÌ¾Æ, ¶óÀÌÄ­, ·»´Ù¸£Å©," EOL );
    cprintf("¹ÚÁÖÈç, º°¹Ù¶÷, ¼ÒÀÎ¹è, ¼öÆò¼±, ½ÃÃ¼, ÀÛÀº³ª¹«, Á¶ÀçÈñ," EOL );
    cprintf("ÁøÈë, Å«ºÒ, Æú¶û, ÇÇ¾È, ACNDKING, Dio, grane, Hara," EOL );
    cprintf("letranger, mari, nox1999, sfan, Steno, Yugi, ZK2" EOL );
    get_ch();

#if defined(WIN32CONSOLE) || defined(WINDOWS)
//ƒLƒƒƒ‰ƒNƒ^[–¼‚ðƒtƒ@ƒCƒ‹‚É‘‚«ž‚Ý
    FILE *fp;
