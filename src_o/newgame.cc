...
bool new_game(void)
{
    ...
    if (you.your_name[0] != '\0')
    {
        ...
        {
#ifdef JP
            cprintf( EOL " ´Ù½Ã µ¹¾Æ¿Â°ÍÀ» È¯¿µÇÑ´Ù. ;) " );
#else
    ...
    if (you.your_name[0] == '\0')
    {
...
#ifdef JP
        snprintf( info, INFO_SIZE, " ´ç½ÅÀº %sÀÇ %sÀÌ´Ù." EOL,
                  spec_buff,
                  you.class_name );
#else
        ...
        {
#ifdef JP
            cprintf(EOL " Á¤¸» ÀÌÀü¿¡ ÀúÀåÇÑ °ÔÀÓÆÄÀÏ¿¡ µ¤¾î¾²±â¸¦ ¿øÇÏ´Â°¡?");
#else
            ...
            if (!(c == 'Y' || c == 'y'))
            {
                textcolor( BROWN );
#ifdef JP
                cprintf(EOL EOL "µ¹¾Æ¿Â °ÍÀ» È¯¿µÇÑ´Ù.");
#else
...
#ifdef JP
        cprintf(EOL "? - ¹«ÀÛÀ§ ¼±ÅÃ" EOL);
#else
        cprintf(EOL "? - Random" EOL);
#endif
            textcolor( CYAN );
#ifdef JP
            cprintf(EOL " ¾î¶² ¹«±â¸¦ »ç¿ëÇÒ °ÍÀÎ°¡? ");
#else
...
********************************************** */
    textcolor( YELLOW );
#ifdef JP
    cprintf("Dungeon Crawl " VERSION " ÇÑ±ÛÆÇ Ver 1.1.0  2007/11/7");
    textcolor( LIGHTGREY );
    cprintf(EOL "Dungeon Crawl:(c) Copyright 1997-2002 Linley Henzell");
    cprintf(EOL "ÀÏº»¾îÆÇ/Å¸ÀÏÆÇ:(c) Copyright 2003-2004 http://crawlj.sourceforge.jp");
#ifdef USE_TILE
    cprintf(EOL "ÇÑ±ÛÆÇ :(c) Copyright 2006  starwindz, ¹é·ÏÈ­, Dio, ±¸ÀÌ, °¨ÀÚ, ZK2");
#endif //USE_TILE
    cprintf(EOL "ÇÑ±ÛÆÇ ¹èÆ÷ : http://nethack.byus.net"
           EOL);
#else //JP
...
#ifdef JP
                cprintf(EOL "ÀÌ¸§À» ÀÔ·ÂÇÏ°í ¿£ÅÍÅ°¸¦ ´©¸£½Ã¿À. ÀÌÈÄ Á¾Á·°ú Á÷¾÷À» ¼±ÅÃÇÑ´Ù."EOL);
#else
...
bool verifyPlayerName(void)
{
    ...
    {
#ifdef JP
        cprintf(EOL "À¯°¨½º·´Áö¸¸, ±×ÀÌ¸§Àº ´ç½ÅÀÇ OS¿¡°Ô µÎÅëÀ» ¾È°ÜÁØ´Ù." EOL);
#else
    ...
    if (strnicmp(you.your_name, "LPT", 3) == 0)
    {
        switch (william_tanksley_asked_for_this)
        {
            case 2:
#ifdef JP
                cprintf(EOL "ÀÌºÁ Àª¸®¾ö! ¿À¸Þ°¡ ÀÛ¾÷Àº Àß µÇ³ª?" EOL);
#else
            ...
            case 1:
#ifdef JP
                cprintf(EOL "ÀÌºÁ, ±× ÀÌ¸§Àº ºÒ¹ýÀÌ¾ß." EOL);
#else
    ...
    for (unsigned int i = 0; i < len; i++)
    {
        ...
        {
#ifdef JP
            cprintf(EOL "ÄÝ·ÐÀº ¾²Áö¸»°Ô, Á¦¹ß." EOL);
#else
...
//#else
        ...
        {
#ifdef JP
            cprintf( EOL "ÇÑ±Û ÀÌ¸§À» »ç¿ëÇÏ¼Ì³×¿ä. ¿Àµ¿ÀÛ ÇÒ ¼ö ÀÖ½À´Ï´Ù." EOL ); return(true);//cprintf( EOL "¾ËÆÄºª, ¼ýÀÚ ±×¸®°í ¹ØÁÙ(_)¸¸ °¡´ÉÇÏ´Ù." EOL );
#else
...
//#endif
    ...
    {
#ifdef JP
        cprintf( EOL "ÀÌ·±, ÀÌ¸§ÀÇ ³¡ÀÌ ¼ýÀÚ·Î µÇ¾î ÀÖ±º." EOL );
#else
...
spec_query:
    if (!printed)
    {
        ...
        cprintf("ÀÌ°÷ÀÌ Ã³À½ÀÎ°¡ º¸±º!" EOL EOL);
#else
        ...
        textcolor( CYAN );
#ifdef JP
        cprintf("¾Æ·¡ Á¾Á·Áß¿¡¼­ ÇÏ³ª¸¦ ¼±ÅÃÇÒ ¼ö ÀÖ´Ù :" EOL EOL);
#else
...
#ifdef JP
        cprintf("a - ÀÎ°£                    b - ¿¤ÇÁ" EOL);
        cprintf("c - ÇÏÀÌ ¿¤ÇÁ               d - ±×·¹ÀÌ ¿¤ÇÁ " EOL);
        cprintf("e - µö ¿¤ÇÁ                 f - ½½·µÁö ¿¤ÇÁ" EOL);
        cprintf("g - Èú µå¿öÇÁ               h - ¸¶¿îÆ¾ µå¿öÇÁ" EOL);
        cprintf("i - ÇÏÇÃ¸µ                  j - Èú ¿ÀÅ©" EOL);
        cprintf("k - ÄÚº¼Æ®                  l - ¹Ì¶ó" EOL);
        cprintf("m - ³ª°¡                    n - ³ë¿ò" EOL);
        cprintf("o - ¿À¿ì°Å                  p - Æ®·Ñ" EOL);
        cprintf("q - ¿À¿ì°Å ¸¶¼ú»ç           r - µå¶óÄÚ´Ï¾ð" EOL);
        cprintf("s - ¼¾Å¸¿ì¸£                t - ¹Ý½ÅÁ·" EOL);
        cprintf("u - ½ºÇÁ¸®°Ç                v - ¹Ì³ëÅ¸¿ì·ç½º" EOL);
        cprintf("w - µ¥¸ó½ºÆù                x - ±¸¿ï" EOL);
        cprintf("y - ÄËÄí                    z - ¸ÓÆ÷Å©" EOL);
#else
        ...
        textcolor( BROWN );
#ifdef JP
        cprintf(EOL "? - ¹«ÀÛÀ§·Î Á¾Á· ¼±ÅÃ      * - ¹«ÀÛÀ§·Î Ä³¸¯ÅÍ »ý¼º" EOL);
        cprintf(    "X - Á¾·á" EOL);
#else
...
job_query:
    if (!printed)
    {
        ...
        if (strlen(you.your_name) > 0)
        {
            cprintf("ÀÇ \"%s\" " EOL EOL, you.your_name);
        }
        else
        {
            textcolor( LIGHTGREY );
        ...
        textcolor( CYAN );
#ifdef JP
        cprintf("´ç½ÅÀº ¾Æ·¡ÀÇ Á÷¾÷Áß ÇÏ³ª¸¦ ¼±ÅÃÇÒ ¼ö ÀÖ´Ù :" EOL);
#else
        ...
        textcolor( BROWN );
#ifdef JP
        cprintf(EOL "? - ¹«ÀÛÀ§ Á÷¾÷ ¼±ÅÃ ,  x - Á¾Á· ¼±ÅÃ ¸Þ´º·Î µ¹¾Æ°¨ ,  X - °ÔÀÓ Á¾·á" EOL);
#else
...
void give_items_skills()
{
    ...
    case JOB_PRIEST:
        ...
        else
        {
            ...
            textcolor( LIGHTGREY );
#ifdef JP
            cprintf("a - Áø (Á¤ÅëÀÇ »çÁ¦°¡ ¼¶±â´Â ½Å)" EOL);
            cprintf("b - ÀÌ·¹µ¨·½´­ (Á×À½ÀÇ »çÁ¦°¡ ¼¶±â´Â ½Å)" EOL);
#else
    ...
    case JOB_DEATH_KNIGHT:
        ...
        else
        {
            ...
            textcolor( CYAN );
#ifdef JP
            cprintf(EOL " ¾îµð¿¡¼­ ´ç½ÅÀº Á×À½ÀÇ ÈûÀ» ¾òÀ»°ÍÀÎ°¡?" EOL);
#else
    ...
    case JOB_CHAOS_KNIGHT:
        ...
        else
        {
            ...
            textcolor( CYAN );
#ifdef JP
            cprintf(EOL " È¥µ·ÀÇ ½ÅµéÁß ¾î¶²½ÅÀ» ¼¶±æ °ÍÀÎ°¡?" EOL);
#else
            ...
            textcolor( LIGHTGREY );
#ifdef JP
            cprintf("a - È¥µ·ÀÇ Á»" EOL);
            cprintf("b - ÆÄ±«ÀÚ ¸¶Å©·¹ºê" EOL);
#else
