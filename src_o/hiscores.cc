...
void hiscores_new_entry( struct scorefile_entry &ne )
{
    ...
    {
#ifdef JP 
        perror("Ω∫ƒ⁄æÓ ∆ƒ¿œ¿ª ø©¥¬µ• Ω«∆– - ±‚∑œ«“ ºˆ æ¯¿Ω.");
#else
...
static const char *const range_type_verb( const char *const aux )
{
    ...
    if (strncmp( aux, "Shot ", 5 ) == 0)                // launched
        return ("shot");
            ...
            || strstr( aux, "ªœ¡∑«— ∏¯¿« ø¨ªÁ" ) )       // manticore spikes
        return ("ªœ¡∑«— ∏¯¿« ø¨ªÁ");            
    else if (aux[0] == '\0'       
            || strstr( aux, "≈ı√¥" ) )                   // thrown
#else
...
void hiscores_format_single(char *buf, struct scorefile_entry &se)
{
...
#ifdef JP 
            snprintf( scratch, sizeof(scratch), "%sø° ∏ª∑¡ µÈæÓ∞° ¡◊æ˙¥Ÿ.", 
#else
    ...
    case KILLED_BY_BEAM:
        ...
        if (se.death_source_name[0] != '\0')
            ...
            strcat( buf, monam( mon_number, mon_type, true, DESC_PLAIN ) );
        snprintf( scratch, sizeof(scratch), "¿« %sø° ¡◊æ˙¥Ÿ.", 
...
*/
    ...
    case KILLED_BY_LAVA:
        if (se.race == SP_MUMMY)
#ifdef JP 
            strcat( buf, "øÎæœø° ∫¸¡Æ ¿Á∞° µ«æ˙¥Ÿ." );
#else
            strcat( buf, " turned to ash by lava" );
#endif
        else
#ifdef JP 
            strcat( buf, "øÎæœº”¿∏∑Œ ∞°∂Ûæ…æ“¥Ÿ." );
#else
    ...
    case KILLED_BY_TRAP:
#ifdef JP 
        snprintf( scratch, sizeof(scratch), "%s «‘¡§ø° ∞…∑¡ ¡◊æ˙¥Ÿ.", 
#else
...
#ifdef JP 
        strcat( buf, "ø¿∫Í∏¶ ∞°¡ˆ∞Ì ¥¯¡Ø¿ª ≈ª√‚«œ¥¬µ• º∫∞¯«ﬂ¥Ÿ!" );
#else
...
#ifdef JP 
        strcat( buf, "∏µÁ ª˝∏Ì¿Ã »Ìºˆ¥Á«ÿ ¡◊æ˙¥Ÿ." );
#else
    ...
    case KILLED_BY_BURNING:     // only sticky flame ÉJÉâÉäÇ∆ógÇ™Ç¡ÇΩÅAÇ≈ÇÕÇ†ÇÒÇ‹ÇËÇ»ÇÃÇ≈ïœçX
#ifdef JP 
        strcat( buf, "πŸªË«œ∞‘ ≈∏ ¡◊æ˙¥Ÿ." );
#else
    ...
    case KILLED_BY_WILD_MAGIC:
        ...
        {
#ifdef JP 
            const bool need_by = ( strstr( se.auxkilldata, "ø° ¿««ÿ" ) == NULL);
#else
...
#ifdef JP 
        strcat( buf, "Ω¥∞ÌΩ∫ ∫Œ»≠∑Œ ±◊ ¿Ø√Ê¿Ã ≥ª¿Â¿ª ∆ƒ∏‘»˜∞Ì ¡◊æ˙¥Ÿ." );
#else
    ...
    case KILLED_BY_ACID:
#ifdef JP 
        strcat( buf, "ªÍ¿ª µ⁄¡˝æÓæ≤∞Ì ≥Ïæ∆ ¡◊æ˙¥Ÿ." );
#else
                                 ...
                                 bool verbose )
{
...
#ifdef JP 
        snprintf( buf, HIGHSCORE_SIZE, "%8ld %s [%s](∑π∫ß %d",
    ...
    {
#ifdef JP 
        snprintf( buf, HIGHSCORE_SIZE, "%8ld %s¿« %s [%s] (∑π∫ß%d",
...
#ifdef JP 
    strncat( buf, ((se.wiz_mode) ? ") *¿ß¿˙µÂ ∏µÂ*" : ")"), HIGHSCORE_SIZE );
#else
    ...
    if (verbose)
    {
        ...
        {
#ifdef JP   // ≈◊Ω∫∆ÆΩ√ »Æ¿ŒπŸ∂˜ (∞®¿⁄) 
            strncat( buf, "¿« ªÓ¿ª ", HIGHSCORE_SIZE );
#else
...
#ifdef JP 
        strncat( buf, "¿œø° ∏∂∞®«ﬂ¥Ÿ." , HIGHSCORE_SIZE );
#else
        ...
        if (se.race != SP_DEMIGOD && se.god != -1)
        {
            ...
            {
#ifdef JP 
                snprintf( scratch, INFO_SIZE, "±◊¥¬ ¡ª¿« %s ¿Â≥≠∞® ¿Ãæ˙¥Ÿ.",
                          (se.lvl >= 20) ? "∏∂¿Ωø° µÂ¥¬" : "" );
#else
    ...
    else 
    {
        ...
        else 
        {
            ...
            {
                snprintf( scratch, sizeof(scratch), "% d√˛", se.dlvl );
                strcat( buf, scratch );
            }
        }
        strcat( buf, "ø°º≠ " );
    ...
    case KILLED_BY_CLOUD:
        ...
        {
#ifdef JP 
            snprintf( scratch, sizeof(scratch), "%s¿« ±∏∏ßø° ∏ª∑¡µÈæÓ ¡◊æ˙¥Ÿ.", 
#else
...
#ifdef JP 
        //¿œ∫ªæÓ∆«ø°º≠¥¬ ¿Ã æÓº¯¿∫ ªÁøÎ«œ¡ˆ æ ¥¬¥Ÿ. ±◊∑°º≠ «—±€∆«µµ ªÁøÎæ»«“ ∞≈ ∞∞¥Ÿ.
...
#ifdef JP 
            snprintf( scratch, sizeof(scratch), "%sø° ¡◊æ˙¥Ÿ.", se.auxkilldata );
#else
    ...
    case KILLED_BY_WEAKNESS:
#ifdef JP 
        strcat( buf, "¿⁄Ω≈¿Ã ¡ˆ∞Ì¿÷¥¬ π´∞‘∏¶ ∞®¥Á«œ¡ˆ ∏¯«œø© ±Ú∑¡ ¡◊æ˙¥Ÿ." );
#else
    ...
    case KILLED_BY_CLUMSINESS:
#ifdef JP 
        strcat( buf, "πŸ≥™≥™ ≤Æ¡˙ø° πÃ≤Ù∑Ø¡Æ ¡◊æ˙¥Ÿ." );
#else
        strcat( buf, "Slipped on a banana peel" );
#endif
        break;
...
#ifdef JP 
            {
            strcat( buf, "¿”π´∏¶ øœºˆ ∏¯«œ∞Ì ¥¯¡Ø¿ª µµ∏¡√ƒ ≥™ø‘¥Ÿ!" );
            }
#else
            strcat( buf, "Got out of the dungeon alive!" );
#endif
    ...
    case KILLED_BY_DRAINING:
#ifdef JP 
        strcat( buf, "∏µÁ ª˝∏Ì∑¬¿Ã »Ìºˆµ«æÓ ¡◊æ˙¥Ÿ." );
#else
    ...
    case KILLED_BY_SHUGGOTH:
#ifdef JP 
        strcat( buf, "Ω¥∞ÌΩ∫ ∫Œ»≠∑Œ ±◊ ¿Ø√Ê¿Ã ≥ª¿Â¿ª ∞¶æ∆∏‘æÓ ¡◊æ˙¥Ÿ." );
#else
        strcat( buf, "Eviscerated by a hatching shuggoth" );
#endif
        needs_damage = true;
    ...
    default:
#ifdef JP 
        strcat( buf, "º“«¡∆Æø˛æÓ πˆ±◊ø° ¥Á«ÿ ¡◊æ˙¥Ÿ." );
#else
        strcat( buf, "Nibbled to death by software bugs" );
#endif
        break;
    ...
    if (verbose)
    {
        ...
        {
#ifdef JP 
            snprintf( scratch, INFO_SIZE, "(%d µ•πÃ¡ˆ)", se.damage );
#else
                    ...
                    && se.num_runes > 0)
        {
            hiscore_newline( buf, line_count );
#ifdef JP   // ≈◊Ω∫∆ÆΩ√ »Æ¿ŒπŸ∂˜ (∞®¿⁄) 
            snprintf( scratch, INFO_SIZE, "%s ∑È %d∞≥", 
                      (se.death_type == KILLED_BY_WINNING) ? "&" : ",",
                      se.num_runes );
#else
            ...
            {
#ifdef JP 
                snprintf( scratch, INFO_SIZE, "(%d ¡æ∑˘)",
        ...
        else if (se.death_type != KILLED_BY_QUITTING)
        {
            ...
            {
#ifdef JP   // ≈◊Ω∫∆ÆΩ√ »Æ¿ŒπŸ∂˜ (∞®¿⁄) 
                snprintf(scratch, INFO_SIZE, "¿˚¿« %sø° ¿««ÿ ¥ÁΩ≈¿∫ ¡◊æ˙¥Ÿ", se.auxkilldata);
#else
            ...
            {
#ifdef JP   // ≈◊Ω∫∆ÆΩ√ »Æ¿ŒπŸ∂˜ (∞®¿⁄) 
                const bool need_by2 = (strstr( se.auxkilldata, "ø° ¿««ÿ") == 0);
                strcat( buf, "¿˚¿« " );
                strcat( buf, se.auxkilldata );
                strcat( buf, (need_by2) ? "ø° ¿««ÿ ¥ÁΩ≈¿∫ ¡◊æ˙¥Ÿ."
                                       : "ø° ¿««ÿ ¥ÁΩ≈¿∫ ¡◊æ˙¥Ÿ." );
#else
            ...
            {
#ifdef JP    // ≈◊Ω∫∆ÆΩ√ »Æ¿ŒπŸ∂˜ (∞®¿⁄) 
                const bool need_by2 = (strstr( se.death_source_name, "ø° ¿««ÿ") == 0);
                snprintf( scratch, sizeof(scratch), "¿˚¿« %s%s",
                          se.death_source_name,
                          (need_by2) ? "ø° ¿««ÿ ¥ÁΩ≈¿∫ ¡◊æ˙¥Ÿ."
                                    : "ø° ¿««ÿ ¥ÁΩ≈¿∫ ¡◊æ˙¥Ÿ." );
...
#ifdef JP 
                strncat( username, "¿«", sizeof(username) );
#else
...
#ifdef JP 
            snprintf( scratch, INFO_SIZE, "%s ∞‘¿” Ω√∞£ %s (%ld≈œ)",
#else
...
static void hs_search_death(char *inbuf, struct scorefile_entry &se)
{
    ...
    // sigh.. //Ç±Ç±Ç©ÇÁâ∫ÇÃñ|ñÛÇÕÇ–Ç∆Ç‹Ç∏ïsóv
#ifdef JP /* ñÛïsópÅH */
