...
#ifdef JP 
    f << "# 경고: 이 파일은 자동 생성되는 파일입니다." << std::endl
...
void macro_add_query( void )
{
...
#ifdef JP 
    mpr( "무엇을 설정할 것인가?  (m)매크로  (k)키맵핑 ", MSGCH_PROMPT );
#else
...
#ifdef JP 
    snprintf( info, INFO_SIZE, "%s 키를 지정 하시오: ",
              (keymap ? "키맵핑" : "매크로") );
#else
    ...
    {
#ifdef JP 
        snprintf( info, INFO_SIZE, "현재의 행동: %s", 
#else
        ...
        mpr( info, MSGCH_WARN );
#ifdef JP 
        mpr( "어떻게 할건가?  (r)재설정  (c)지우기  (a)중단", MSGCH_PROMPT );
#else
