...
static void cards(unsigned char which_card)
{
    ...
    case CARD_FIEND:
#ifdef JP
        mpr("카드에 커다랗고 비늘로 덮힌 악마의 그림이 그려져 있다.");
#else
    ...
    case CARD_LICH:
#ifdef JP
        mpr( "카드에 기분이 안좋아 보이는 해골의 모습이 그려져 있다." );
#else
...
#ifdef JP
            mpr("카드에 흉측한 어보미네이션이 그려져 있다."); // Dio : abomination 그냥 발음대로가 나은것같아서..
#else
    ...
    case CARD_HEALING:
#ifdef JP
        mpr("엘릭서의 카드를 뽑았다."); // Dio 이것도 그냥 발음대로..
#else
...
#ifdef JP
                                            ? "당신 앞에 땅으로부터 솟아난다." : "당신의 발밑에 땅으로부터 솟아난다.");
#else
                                            ? "before you!" : "at your feet!");
#endif
                mpr(info);
...
#ifdef JP
        mpr( "엉터리 마법의 카드를 뽑았다." ); // Dio 나름대로 이 번역도 괜찮아서 그대로.. 더 괜찮은게 있을까요?
#else
...
#ifdef JP
        mpr("콘트랙트 브리지의 규칙이 적인 카드를 뽑았다.");
        mpr("정말 흥미롭군!"); // Dio 좀더 좋은 멘트 있으면...
#else
