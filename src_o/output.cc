...
#define BUF_SIZE 25*3*45
    ...
    lines++;
    snprintf(CUR_AD, "종족     : %s\0", species_name(you.species,you.experience_level) );
    snprintf(CUR_AD, "직업     : %s\0", you.class_name);
    snprintf(CUR_AD, "신앙     : %s\0", god_name(you.religion) );
    snprintf(CUR_AD, "\0");
    snprintf(CUR_AD, "레벨        %7d\0", you.experience_level);
    snprintf(CUR_AD, "경험치      %7d\0", you.experience);

    if (you.experience_level < 27)
    {
        int xp_needed = (exp_needed(you.experience_level+2) - you.experience) + 1;
        snprintf(CUR_AD, "경험치 목표 %7d\0", exp_needed(you.experience_level + 2) + 1);
        snprintf(CUR_AD, "필요 경험치 %7d\0", xp_needed);
    }
    else
    {
        snprintf(CUR_AD, "다음 레벨   *******\0");
        snprintf(CUR_AD, "필요 경험치 *******\0");
    }

    snprintf(CUR_AD, "남은 마법   %7d\0", player_spell_levels() );

    snprintf(CUR_AD, "소지금      %7d\0", you.gold );
...
*/
    ...
    if (you.strength == you.max_strength)
    {
        snprintf(CUR_AD, "힘     : %3d\0", you.strength);
    }
    else
    {
        snprintf(CUR_AD, "힘     : %3d (%2d)\0", you.strength, you.max_strength);
    }

    if (you.intel == you.max_intel)
    {
        snprintf(CUR_AD, "지능   : %3d\0", you.intel);
    }
    else
    {
        snprintf(CUR_AD, "지능   : %3d (%2d)\0", you.intel, you.max_intel);
    }

    if (you.dex == you.max_dex)
    {
        snprintf(CUR_AD, "민첩   : %3d\0", you.dex);
    }
    else
    {
        snprintf(CUR_AD, "민첩   : %3d (%2d)\0", you.dex, you.max_dex);
    }

    snprintf(CUR_AD, "AC     : %3d\0", player_AC() );
    snprintf(CUR_AD, "회피   : %3d\0", player_evasion() );
    snprintf(CUR_AD, "방패   : %3d\0", player_shield_class() );
    lines++;
    ...
    if (you.real_time != -1)
    {
        ...
        snprintf(CUR_AD, "게임시간 : %10s\0", buff);
        snprintf(CUR_AD, "경과 턴  : %10d\0", you.num_turns );
    }

    lines = 27;

    snprintf(CUR_AD, "화염 내성 : %s\0", itosym3( player_res_fire(calc_unid) ) );
    //snprintf(CUR_AD, " /%2d", player_res_fire() );
    snprintf(CUR_AD, "냉기 내성 : %s\0", itosym3( player_res_cold(calc_unid) ) );
    //snprintf(CUR_AD, " /%2d", player_res_cold() );
    snprintf(CUR_AD, "약화 내성 : %s\0", itosym3( player_prot_life(calc_unid) ) );
    //snprintf(CUR_AD, " /%2d", player_prot_life() );
    snprintf(CUR_AD, "독소 내성 : %s\0", itosym1( player_res_poison(calc_unid) ) );
    //snprintf(CUR_AD, " /%2d", player_res_poison() );
    snprintf(CUR_AD, "전기 내성 : %s\0", itosym1( player_res_electricity(calc_unid) ) );
    //snprintf(CUR_AD, " /%2d", player_res_electricity() );
    lines++;

    snprintf(CUR_AD, "능력 유지 : %s\0", itosym1( player_sust_abil(calc_unid) ) );
    snprintf(CUR_AD, "변이 저항 : %s\0", itosym1( wearing_amulet( AMU_RESIST_MUTATION, calc_unid) ) );
    snprintf(CUR_AD, "감속 저항 : %s\0", itosym1( wearing_amulet( AMU_RESIST_SLOW, calc_unid) ) );
    snprintf(CUR_AD, "명석함    : %s\0", itosym1( wearing_amulet( AMU_CLARITY, calc_unid) ) );
    lines++;
    lines++;
/*
    if (you.wizard)
    {
        lines++;
        snprintf(CUR_AD, "공격 속도 : %2d\0", check_weapon_speed() );
        snprintf(CUR_AD, "이동 속도 : %2d\0", player_movement_speed() );
        snprintf(CUR_AD, "마법 저항 : %2d\0", player_res_magic(calc_unid) / 10 );
        //snprintf(CUR_AD, " /%2d", (player_res_magic() + 1) / 10 );
        snprintf(CUR_AD, "은밀성 : %2d\0", check_stealth(calc_unid) / 10 );
        lines++;
    }
    else
*/
    {
        const char *e_items[] = {"무기  ", "갑옷  ", "방패  ", "머리  ", "망토  ",
                                 "장갑  ", "신발  ", "부적  ", "반지  ", "반지  "};
        ...
        for(i=0; i < NUM_EQUIP; i++)
        {
            ...
            else
            {
                ...
                {
                    if (you.attribute[ATTR_TRANSFORMATION] == TRAN_BLADE_HANDS)
                        snprintf(CUR_AD, "%s칼날의 손\0", e_items[i]);  //테스트시 확인 바람(ZK2)
                    else if (you.skills[SK_UNARMED_COMBAT])
                        snprintf(CUR_AD, "%s맨손 격투\0", e_items[i]);  //테스트시 확인 바람(ZK2)
                    else
                        snprintf(CUR_AD, "%s없음\0", e_items[i]);
                }
                else
                {
                        snprintf(CUR_AD, "%s없음\0", e_items[i]);
    ...
    lines = 52;
    snprintf(CUR_AD, "투명 보기: %s\0", itosym1( player_see_invis(calc_unid) ) );
    snprintf(CUR_AD, "결계     : %s\0", itosym1( wearing_amulet(AMU_WARDING, calc_unid)
                                               ||(you.religion == GOD_VEHUMET && you.duration[DUR_PRAYER]
                                               && (!player_under_penance() && you.piety >= 75) )
                                               ) );
    snprintf(CUR_AD, "물품 보호: %s\0", itosym1( wearing_amulet( AMU_CONSERVATION, calc_unid) ) );
    snprintf(CUR_AD, "부식 저지: %s\0", itosym1( wearing_amulet( AMU_RESIST_CORROSION, calc_unid) ) );
    ...
    if ( !wearing_amulet( AMU_THE_GOURMAND, calc_unid) )
    {
        ...
        {
        case SP_GHOUL:
            snprintf(CUR_AD, "썩은 고기 : %s\0", itosym3(3) );
            break;

        case SP_KOBOLD:
        case SP_TROLL:
            snprintf(CUR_AD, "썩은 고기 : %s\0", itosym3(2) );
            break;

        case SP_HILL_ORC:
        case SP_OGRE:
            snprintf(CUR_AD, "썩은 고기 : %s\0", itosym3(1) );
            break;
#ifdef V_FIX
        case SP_OGRE_MAGE:
            snprintf(CUR_AD, "대식가   : %s\0", itosym1(1) );
            break;
#endif
        default:
            snprintf(CUR_AD, "대식가   : %s\0", itosym1(0) );
            break;
        }
    }
    else
    {
        snprintf(CUR_AD, "대식가   : %s\0", itosym1( wearing_amulet( AMU_THE_GOURMAND, calc_unid) ) );
    }

    lines++;

    if ( scan_randarts(RAP_PREVENT_TELEPORTATION, calc_unid) )
        snprintf(CUR_AD, "랜덤이동 : %s\0", itosym1( scan_randarts(RAP_PREVENT_TELEPORTATION, calc_unid) ) );
    else
    {
      //snprintf(CUR_AD, "공간이동 : %s\0", itosym3( (player_teleport(calc_unid) + 11) / 12 ) );
        snprintf(CUR_AD, "공간이동 : %s\0", itosym1( player_teleport(calc_unid) ) );
    }

    if ( ( you.attribute[ATTR_CONTROL_TELEPORT] <= 1 )
       &&( !player_equip( EQ_RINGS, RING_TELEPORT_CONTROL, calc_unid) ) )
        snprintf(CUR_AD, "좌표이동 : %s\0", itosym1( 0 ) );
    else
        snprintf(CUR_AD, "좌표이동 : %s\0", itosym1( 1 ) );

    snprintf(CUR_AD, "공중부양 : %s\0", itosym1( player_is_levitating() ) );
    snprintf(CUR_AD, "비행제어 : %s\0", itosym1( wearing_amulet(AMU_CONTROLLED_FLIGHT, calc_unid) ) );
