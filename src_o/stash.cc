...
#define ST_MINOR_VER ((unsigned char) 2)
...
void Stash::display(ostream &os, int refx, int refy, std::string place) const {
    ...
    os << "(" << ((int) x - refx) << ", " << ((int) y - refy)
       << (place.length()? (std::string)", " + place : (std::string)"")
       << ")"
       << endl;
...
#ifdef JP
            sprintf(buf, "%d개의 금화", item.quantity);
#else
...
void ShopInfo::add_item(const string &name, const string &desc,
                    unsigned price) {
    shop_item it;
    it.name = name;
    it.desc = desc;
...
void ShopInfo::display(ostream &os) const {
    ...
    if (items.size() > 0) {
        for (unsigned i = 0; i < items.size(); ++i) {
#ifdef JP
            os << "    " << setw( 40 ) << setiosflags( ios::left )
               << items[i].name << resetiosflags( ios::left )
               << setw( 6 ) << items[i].price << " GOLD" << endl;
            if (items[i].desc.length() > 0)
                //os << "    " << items[i].desc << endl;
            {
                std::string desc;
                desc = items[i].desc.substr(0);
                //os << "    " << items[i].desc << endl;

                // trim leading whitespace
                string::size_type notwhite = desc.find_first_not_of(" \t\n");
...
#ifdef JP
    mpr(en? "이 지점에 배치된 아이템을 기록합니다."
        : "이 지점에 배치된 아이템을 무시합니다.");
#else
    ...
    if (s) {
        ...
        else
        {
            ...
            if (Options.stash_tracking == STM_EXPLICIT)
                mpr("이 지점은 이미 기록 리스트에 들어가 있습니다.");
#else
            if (Options.stash_tracking == STM_EXPLICIT)
                mpr("This square is already recorded.");
#endif
        }
    } else {
        Stash newStash(x, y);
...
#ifdef JP
            if (Options.stash_tracking == STM_EXPLICIT)
                mpr("이 지점에 배치된 아이템을 기록합니다.");
#else
            if (Options.stash_tracking == STM_EXPLICIT)
                mpr("I'll record what I see on this square.");
#endif
            for (list<Stash>::iterator iter = stashes.begin();
                    iter != stashes.end(); iter++) {
                if (newStash.abs_pos() < iter->abs_pos()) {
...
void LevelStashes::load(FILE *file) {
    ...
    for (int i = 0; i < size; ++i) {
        Stash s;
        s.load(file);
        if (!s.empty()) stashes.push_back(s);
    }
...
#ifdef JP
    os << player_title() << "["
       << you.your_name  << "]" << endl << endl;
    if (!levels.size())
        os << "기록된 위치 정보가 없다." << endl;
#else
...
void StashTracker::update_visible_stashes(
    ...
    for (int cy = 1; cy <= 17; ++cy) {
        for (int cx = 9; cx <= 25; ++cx) {
            ...
            if (x < 0 || x >= GXM || y < 0 || y >= GYM) continue;
            if (!env.show[cx - 8][cy] && !(cx == 17 && cy == 9)) continue;
