...
void cast_twisted(int power, int corps_beh, int corps_hit)
{
    ...
    {
#ifdef JP
        mpr("시체의 산은 서로 결합해서 몸부림치는 고깃덩이가 됐다!");
#else
...
bool brand_weapon(int which_brand, int power)
{
    ...
    case SPWPN_DRAINING:
#ifdef JP
        strcat(info, "은(는) 사악한 에너지로 가득 찼다.");
#else
...
#ifdef JP
        strcat(info, "은(는) 은색으로 빛나며, 매우 날카로워진 것 같다.");
#else
...
#ifdef JP
        strcat(info, "은(는) ");
        strcat( info, coinflip() ? "기묘하게 " : "이상하게 " );
#else
...
#ifdef JP
        strcat(info, "은(는) 은색으로 빛나며, 보다 무거워진 것 같다.");
#else
...
#ifdef JP
            simple_monster_message( monster, "은(는) 도주했다!" );
#else
...
void holy_word(int pow)
{
    ...
    for (int tu = 0; tu < MAX_MONSTERS; tu++)
    {
        ...
        {
#ifdef JP
            simple_monster_message(monster, "은(는) 심하게 경련했다!");
#else
...
void cast_refrigeration(int pow)
{
    ...
    for (toxy = 0; toxy < MAX_MONSTERS; toxy++)
    {
        ...
        {
#ifdef JP
            snprintf( info, INFO_SIZE, "당신은 %s을(를) 얼어붙게 했다.",
#else
...
void drain_life(int pow)
{
    ...
    {
#ifdef JP
        mpr( "당신은 몸속이 생명으로 가득 채워지는 것을 느꼈다." );
#else
...
int vampiric_drain(int pow)
{
  ...
  dirc:
    ...
    strcat(info, ptr_monam( monster, DESC_NOCAP_THE ));
#ifdef JP
    strcat(info, "에게서 생명력이 흘러들어오는 것을 느꼈다.");
#else
...
char burn_freeze(int pow, char flavour)
{
    ...
    strcat(info, ptr_monam( monster, DESC_NOCAP_THE ));
    strcat(info, "을(를) ");
    strcat(info, (flavour == BEAM_FIRE)        ? "불태웠다" :
                 (flavour == BEAM_COLD)        ? "얼렸다" :
                 (flavour == BEAM_MISSILE)     ? "부쉈다" :
                 (flavour == BEAM_ELECTRICITY) ? "감전시켰다"
                                               : "______");
#else
    strcpy(info, "You ");
                     ...
                     unsigned char unfriendly)
{
    ...
    {
#ifdef JP
        mpr("어느 방향의 지형을 소재로 소환합니까?", MSGCH_PROMPT);
#else
        ...
        || random2(100) < unfriendly)
    {
        ...
        if (summ_success >= 0)
#ifdef JP
            mpr( "정령은 소환된 것에 대하여 기뻐하지 않는 모양이다." );
#else
...
#ifdef JP
        snprintf( info, INFO_SIZE, "누군가가 당신의 부름에 응했다!" );
#else
