...
#ifdef JP
    std::string killtext = "처치한 몬스터:" EOL;
#else
...
#ifdef JP
        char buf[20];
        snprintf(buf, sizeof buf, "%4d마리의 ", n);
        return buf + name;
#else
    if (n > 1) {
        char buf[20];
...
static std::string kill_times(int kills) {
    char buf[50];
#ifdef JP
        snprintf(buf, sizeof buf, " (%d회)", kills);
#else
    switch (kills) {
      case 1:
        strcpy(buf, " (once)");
...
kill_ghost::kill_ghost(const struct monsters *mon) {
    ...
    if (mon->type == MONS_PLAYER_GHOST)
        ghost_name = "         " + ghost_description(true)+"의 유령";
    else if (mon->type == MONS_PANDEMONIUM_DEMON)
    {
        std::string buf;
        buf = ghost.name;
        ghost_name = "         판데모니움의 " + buf + " 유령"; // ...잘 모르겠네요
    }
#else
    if (mon->type == MONS_PLAYER_GHOST)
