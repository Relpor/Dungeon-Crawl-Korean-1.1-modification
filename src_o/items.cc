...
#ifdef JP
    mpr( "이 층에는 너무나 많은 아이템이 있어서 일부를 제거하겠다.", MSGCH_WARN );
#else
...
void unlink_item( int dest )
{
...
#ifdef JP
    mpr( "오류 경고: Problems unlinking item!!!", MSGCH_DANGER );
#else
    ...
    if (!linked)
#ifdef JP
        mpr("오류 경고: Item didn't seem to be linked at all.", MSGCH_DANGER);
#else
...
void item_check(char keyin)
{
    ...
    if (grid >= DNGN_ENTER_HELL && grid <= DNGN_PERMADRY_FOUNTAIN)
    {
        ...
        {
#ifdef JP
            snprintf( info, INFO_SIZE, "여기에는 %s(으)로 만들어진 내려가는 계단이 있다.",
                     (grid == DNGN_ROCK_STAIRS_DOWN) ? "바위" : "작은 돌" );
#else
        ...
        {
#ifdef JP
            snprintf( info, INFO_SIZE, "여기에는 %s(으)로 만들어진 올라가는 계단이 있다.",
                     (grid == DNGN_ROCK_STAIRS_UP) ? "바위" : "작은 돌" );
#else
...
#ifdef JP
                mpr("여기에는 얼어붙은 황무지 코큐투스로 가는 문이 있다.");
#else
...
#ifdef JP
                snprintf( info, INFO_SIZE, "여기에는 %s의 입구가 있다.", shop_name(you.x_pos, you.y_pos));
#else
...
#ifdef JP
                mpr("여기에는 미궁으로 가는 문이 있다.");
                mpr("굶주림이 기다리고 있음을 잊지 마라!");
#else
...
#ifdef JP
                mpr("여기에는 무한 공포의 어비스로 가는 일방통행 문이 있다.");
#else
...
#ifdef JP
                mpr("여기에는 판데모니움에서 탈출할 수 있는 문이 있다.");
#else
...
#ifdef JP
                mpr("여기에는 판데모니움의 다른 지역으로 가는 문이 있다.");
#else
...
#ifdef JP
                mpr("여기에는 모든 종교의 신전으로 가는 계단이 있다.");
#else
...
#ifdef JP
                mpr("여기에는 오크 광산으로 되돌아 가는 계단이 있다.");
#else
...
#ifdef JP
                mpr("여기에는 \"키쿠바쿠드하\"의 고대의 뼈로 만들어진 제단이 있다.");
#else
...
#ifdef JP
                mpr("여기에는 \"이레델렘눌\"의 현무암으로 만들어진 제단이 있다.");
#else
...
#ifdef JP
                mpr("여기에는 \"마크레브\"의 불길이 타오르는 제단이 있다.");
#else
    ...
    while (objl != NON_ITEM)
    {
        ...
        {
#ifdef JP
            strcpy(item_show[counter], "아이템이 너무 많다.");
#else
...
#ifdef JP
            strcat(item_show[counter], " 개의 금화");
#else
...
void pickup(void)
{
    ...
    {
#ifdef JP
        mpr("당신은 지금 높이로부터 바닥까지 손이 닿지 않는다.");
#else
        ...
        && !player_in_branch( BRANCH_ECUMENICAL_TEMPLE ))
    {
        ...
        {
#ifdef JP
            mpr("여기에는 휴대용 제단이 있다. 그러나 당신은 물건을 더 가질 수 없다.");
#else
...
#ifdef JP
        if (yesno("여기에는 휴대용 제단이 있다. 가질 것인가?"))
#else
    ...
    else if (mitm[o].link == NON_ITEM)      // just one item?
    {
        ...
        if (num == -1)
#ifdef JP
            mpr("이보다 많은 아이템을 가질 수 없다.");
#else
            mpr("You can't carry that many items.");
#endif
        else if (num == 0)
#ifdef JP
            mpr("무게 때문에 이제 가지고 다닐 수 없다.");
#else
...
#ifdef JP
                strcat(info, "\? (y:줍는다. n:방치 a:모두 q:중단)");
#else
                strcat(info, "\? (y,n,a,q)");
#endif
...
int move_item_to_player( int obj, int quant_got, bool quiet )
{
    ...
    if (mitm[obj].base_type == OBJ_GOLD)
    {
        ...
        {
#ifdef JP
            snprintf( info, INFO_SIZE, "%d개의 금화를 주웠다.",
                     quant_got );
#else
    ...
    for (m = Options.pick_items_start; m < ENDOFPACK; m++)
    {
        ...
        if (!is_valid_item( you.inv[m] ))
        {
                ...
                && you.char_direction == DIR_DESCENDING)
            {
                if (!quiet)
#ifdef JP
                    mpr("지금 당신이 해야 할 일은 던젼에서  탈출하는 것이다!");
#else
...
static void drop_gold(unsigned int amount)
{
    if (you.gold > 0)
    {
...
#ifdef JP
        snprintf( info, INFO_SIZE, "%d개의 금화를 버렸다.",
                 amount );
#else
        ...
        {
#ifdef JP
            mpr( "여기에는 아이템이 너무 많으므로 금화를 버릴 수 가 없다." );
#else
...
void drop(void)
{
...
#ifdef JP
    item_dropped = prompt_invent_item( "어떤 아이템을 버립니까? ", -1, true, true,
#else
    ...
    else if (item_dropped == PROMPT_GOT_SPECIAL)  // ie '$' for gold
    {
        ...
        {
#ifdef JP
            snprintf(str_pass, ITEMNAME_SIZE, "%d개의 금화", you.gold);
            mpr( str_pass );
            mpr ("수량: ");
#else
    ...
    {
#ifdef JP
        mpr( "현재 층에는 아이템이 너무 많아서 그 아이템을 버릴 수 없다." );
#else
...
#ifdef JP
    snprintf( info, INFO_SIZE, "%s을(를) 버렸다.", str_pass );
#else
...
#ifdef JP
    snprintf( info, INFO_SIZE, "움직임 횟수: %d", turns );
#else
...
void handle_time( long time_delta )
{
    ...
    if (player_in_hell() && coinflip())
    {
        ...
        mpr((temp_rand == 0) ? "\"너는 여기서 살아 나갈수 없다!\"" :
            ...
            (temp_rand == 2) ? "\"우리들을 거역한 사람은 결코 허락되지 않는다!\"" :
            (temp_rand == 3) ? "\"침입자는 여기에서는 환영받지 못한다!\"" :
            (temp_rand == 4) ? "\"너는 여기에 있을 수 있는 사람이 아니다!\"" :
            (temp_rand == 5) ? "\"빨리 꺼져라. 우물쭈물 하지 말고!\"" :
            (temp_rand == 6) ? "\"너는 벌써 우리의 것이다!\"" :
            (temp_rand == 7) ? "무서운 죽음의 예감을 느꼈다....." :
            (temp_rand == 8) ? "기괴하고 무서운 말을 들었다....." :

            (temp_rand == 9) ? ((you.species != SP_MUMMY)
                    ? "유황의 지독한 냄새를 맡았다." : "유황이 비와 같이 쏟아졌다.") :

            (temp_rand == 10) ? "무엇인가 무서운 일이 일어났다." :
            (temp_rand == 11) ? "고대의 악령에게 감시되고 있는 것을 느꼈다." :
            (temp_rand == 12) ? "미아가 된 것처럼 느꼈다." :
            (temp_rand == 13) ? "돌연, 왜소하고 약해진 것 같다." :
            (temp_rand == 14) ? "내장까지 떨리게하는 절규가 대기를 채웠다." :
            (temp_rand == 15) ? "공포에 떨었다." :
            (temp_rand == 16) ? "적대적인 존재를 느꼈다."
                              : "악마와 같은 웃음소리를 들었다!", MSGCH_TALK);
#else
    ...
    if (!you.disease)
    {
        ...
        {
#ifdef JP
            mpr("힘이 회복되는 것을 느꼈다.", MSGCH_RECOVERY);
#else
        ...
        {
#ifdef JP
            mpr("민첩성이 회복되는 것을 느꼈다. ", MSGCH_RECOVERY);
#else
        ...
        {
#ifdef JP
            mpr("지능이 회복되는 것을 느꼈다.", MSGCH_RECOVERY);
#else
    ...
    else
    {
        ...
        {
#ifdef JP
            mpr("당신의 병이 체력을 빼앗아 간다.", MSGCH_WARN);
#else
...
#ifdef JP
            snprintf( info, INFO_SIZE, "당신이 사용하고 있는 무기는 %s(이)다.", str_pass );
#else
...
#ifdef JP
                mpr( "당신의 짐이 갑자기 가벼워졌다.", MSGCH_ROTTEN_MEAT );
#else
    ...
    if (new_rotting_item)
    {
        ...
        case SP_GHOUL: //likes it
            temp_rand = random2(8);
#ifdef JP
            mpr( ((temp_rand  < 5) ? "무엇인가가 썩는 지독한 냄새를 맡았다." :
                  (temp_rand == 5) ? "썩은 고기의 지독한 냄새가 당신을 더욱 배고프게 했다." :
                  (temp_rand == 6) ? "고기가 썩는 지독한 냄새를 맡았다. 먹음직스럽다."
                                   : "우와! 뭔가 먹음직스러운 것이 당신의 짐 속에 있다."),
                MSGCH_ROTTEN_MEAT );
#else
...
#ifdef JP
            mpr( ((temp_rand  < 5) ? "무엇인가가 썩는 지독한 냄새를 맡았다." :
                  (temp_rand == 5) ? "고기 썩는 냄새가 난다." :
                  (temp_rand == 6) ? "부패하는 냄새가 난다."
                                   : "짐속에 무언가가 썩고있다."),
                MSGCH_ROTTEN_MEAT );
#else
            mpr( ((temp_rand  < 5) ? "You smell something rotten." :
...
void cmd_destroy_item( void )
{
...
#ifdef JP
    int item = prompt_invent_item( "어떤 아이템을 파괴합니까? ", -1, true, false );
#else
...
#ifdef JP
    snprintf( info, INFO_SIZE, "%s을(를) 파괴합니까?", str_pass );
#else
    ...
    if (yesno( info, true ))
    {
       //destroy it!!
#ifdef JP
        snprintf( info, INFO_SIZE, "%s을(를) 파괴했다.", str_pass );
#else
